...
Weekly Context JSON 規則：\n1. operations 必須只有 1 筆，entity=weekly_context、action=upsert。\n2. week_start 使用當週星期一 YYYY-MM-DD；key.context_id 固定為 weekly_context_<week_start>_import，例如 weekly_context_2026-08-10_import。data.week_start 必須與 key 中日期一致。\n3. payload 頂層 scenario=weekly_context_update，context_authority=UPDATE_CENTER_JSON。generated_at 與 data.updated_at 使用產生此更新包時的有效 ISO 日期時間。\n4. data.dish_category 必須正規化為且只能使用「咖哩／濃湯」「沙拉」「甜點／飲料」之一。若遊戲畫面寫「咖哩、濃湯」仍輸出「咖哩／濃湯」；若寫「點心、飲料」或「點心／飲料」則輸出「甜點／飲料」。\n5. 只整理本週可觀測事實：實際營地 camp、料理類型 dish_category、活動名稱 event_name、鍋子容量 pot_size，以及活動效果 event_effects。不得從上週資料或一般遊戲常識補值。\n6. event_effects 必須直接輸出為 JSON object，不要把 JSON 再包成字串。只寫畫面或使用者提供內容能直接支持的欄位。可使用：event_schema、event_start、event_end、mission_start、mission_end、event_camp_scope、meal_category_forced、recipe_final_energy_multiplier、extra_tasty_multiplier、sunday_extra_tasty_multiplier、sunday_pot_multiplier、new_recipe_count、cross_sleep_type_encounters、boosted_pokemon_types、shiny_encounter_possible、limited_feature。event_start/event_end 若畫面有精確時間，保留完整 ISO 日期時間，不可只截成日期。未確認欄位省略，不可自行補 1、0 或預設值。\n7. event_effects 欄位型別必須嚴格遵守：meal_category_forced 只能是 boolean true/false；cross_sleep_type_encounters 與 shiny_encounter_possible 也只能是 boolean；recipe_final_energy_multiplier、extra_tasty_multiplier、sunday_extra_tasty_multiplier、sunday_pot_multiplier 是數字；new_recipe_count 是 0 以上整數；boosted_pokemon_types 是字串陣列；event_schema、event_start、event_end、mission_start、mission_end、event_camp_scope、limited_feature 是字串。meal_category_forced 只表示「活動是否強制料理類型」，料理名稱只能放在 data.dish_category。正確例：「meal_category_forced": true」；錯誤例：「meal_category_forced": "咖哩／濃湯"」。limited_feature 必須逐字抄錄畫面名稱，不得把「扭糖機」自行改寫成「扭蛋機」等近義詞；看不清楚就省略。\n8. sunday_pot_multiplier 只有截圖／使用者本次資料明確證實時才輸出；不得因一般週日規則自行填 2。\n9. 固定三樹果營地可省略 favorite_berry_1~3，由平台公版 Camp Berry Master 自動投影。萌綠之島一般模式與 EX 動態營地：只有本週畫面實際看見全部三種喜好樹果時才一次填滿 favorite_berry_1~3；若任一名稱看不清楚或只看見 0~2 種，三欄全部省略／null。樹果名稱必須逐字依畫面文字，不得用相似字猜測；不得沿用上週。\n10. 活動若寫「所有營地」，可在 event_effects.event_camp_scope 輸出 ALL_CAMPS；若寫不論睡眠類型都能遇見其他睡眠類型，可輸出 cross_sleep_type_encounters=true；若明確列出提升出現機率的寶可夢屬性，可輸出 boosted_pokemon_types 字串陣列。\n11. base_notes 只記錄使用者明確提供且會影響本週策略的假設；遊戲公告事實應放在對應正式欄位或 event_effects，不要把未證實內容塞入 base_notes。\n12. evidence.source_image_ref 必須指向本次判讀的主要截圖；若多張圖共同支持，可另外輸出 evidence.source_image_refs 陣列。confidence 只反映本次辨識信心；名稱若有不確定，不可一邊猜測一邊給 0.98/0.99。\n13. 不得寫入任何公版 Master、料理解鎖、寶可夢個體、庫存或隊伍資料。`;

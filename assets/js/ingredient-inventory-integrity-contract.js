...
export function applyIngredientInventoryIdentityMigration(db){
  ...
  for(const [legacyName,canonicalName] of Object.entries(LEGACY_INGREDIENT_IDENTITY_RENAMES)){
    ...
    if(legacy&&!canonical){
      ...
      audit.push({legacy_name:legacyName,canonical_name:canonicalName,action:'REKEYED_LEGACY_ONLY',quantity:Number(legacy.quantity||0)});
      ...
      audit.push({legacy_name:legacyName,canonical_name:canonicalName,action:reason,legacy_quantity:Number(legacy.quantity||0),canonical_quantity:Number(canonical.quantity||0)});
    }
  ...
  db.run(`INSERT OR REPLACE INTO settings(key,value_json,updated_at)
    VALUES('ingredient_inventory_identity_migration_v12',?,datetime('now'))`,[JSON.stringify({version:INGREDIENT_INVENTORY_INTEGRITY_VERSION,audit})]);
  ...
  return {changed:audit.length>0,audit};
}
...
export function buildIngredientAbsenceCandidates({coverage,recognizedIngredientNames=[],establishedInventoryRows=[],confirmations=[]}={}){
  ...
  return (Array.isArray(establishedInventoryRows)?establishedInventoryRows:[])
    ...
    .map(row=>{
      ...
      const resolution=INGREDIENT_ABSENCE_RESOLUTIONS.includes(confirmation?.resolution)?confirmation.resolution:null;
...
export function applyIngredientAbsenceConfirmations(payload,confirmations,{sourceImageRefs=[]}={}){
  const copy=clone(payload),normalized=normalizeIngredientAbsenceConfirmations(confirmations),existing=new Set(ingredientNamesFromUpdatePackage(copy));
  ...
  for(const item of normalized){
    ...
    copy.operations.push({
      operation_id:`ABSENCE-${String(counter).padStart(3,'0')}`,

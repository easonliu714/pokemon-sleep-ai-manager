...
async function saveRecipeState(row,unlocked,level,energy){
  if(!databaseReady())throw new Error('目前為唯讀／尚未完成資料庫初始化，無法修改玩家料理狀態');
  const playerRecipeId=row.player_recipe_id||row.recipe_id;
  const before=rows('SELECT * FROM recipes WHERE recipe_id=?',[playerRecipeId])[0]||null;
  ...
  try{
    ...
    const after=rows('SELECT * FROM recipes WHERE recipe_id=?',[playerRecipeId])[0];
    const updateId=`MANUAL-RECIPE-${Date.now()}-${Math.random().toString(16).slice(2,6)}`;
    ...
    run('INSERT INTO import_changes(update_id,operation_index,entity,action,key_json,before_json,after_json,status,message) VALUES(?,?,?,?,?,?,?,?,?)',[updateId,0,'recipes','manual_update',JSON.stringify({recipe_id:playerRecipeId,master_recipe_id:row.recipe_id}),JSON.stringify(before),JSON.stringify(after),'applied','前端手動修改料理解鎖狀態']);
    commit();await persist();
  }catch(error){rollback();throw error;}
}

const authorityById=new Map(PUBLIC_RECIPE_MASTER.map(recipe=>[recipe.recipe_id,recipe]));
function recipeSummary(recipe){return (recipe?.ingredients||[]).map(item=>`${item.ingredient_name}×${item.quantity}`).join('、');}
function publicRecipeRows(){
  if(!databaseReady())return PUBLIC_RECIPE_MASTER.map(recipe=>({...recipe,unlocked:0,recipe_level:1,current_energy:null,player_recipe_id:null,player_record_exists:0,summary:recipeSummary(recipe),data_version:PUBLIC_RECIPE_MASTER_VERSION}));
  return rows('SELECT * FROM recipe_catalog_state').map(row=>{
    const authority=authorityById.get(row.recipe_id);
    return {...row,summary:authority?recipeSummary(authority):''};
  });
}
function renderRecipeCatalog(){
  const table=$('recipeTable');if(!table)return;
  const readonly=!databaseReady();
  ...
  table.innerHTML=`<thead><tr><th>分類</th><th>料理</th><th>配方</th><th>已解鎖</th><th>料理等級</th><th>目前能量</th><th>主檔版本</th><th>操作</th></tr></thead><tbody>${data.map(row=>`<tr><td>${esc(row.category)}</td><td>${esc(row.recipe_name)}</td><td>${esc(row.summary||'—')}</td><td><input class="canonical-recipe-unlocked" type="checkbox" ${row.unlocked?'checked':''} ${readonly?'disabled':''} data-id="${esc(row.recipe_id)}"></td><td><input class="inline-number canonical-recipe-level" type="number" min="1" value="${esc(row.recipe_level??1)}" ${readonly?'disabled':''} data-id="${esc(row.recipe_id)}"></td><td><input class="inline-number canonical-recipe-energy" type="number" min="0" value="${esc(row.current_energy??'')}" ${readonly?'disabled':''} data-id="${esc(row.recipe_id)}"></td><td>${esc(row.data_version||PUBLIC_RECIPE_MASTER_VERSION)}</td><td><button class="canonical-save-recipe" ${readonly?'disabled':''} data-id="${esc(row.recipe_id)}">${readonly?'唯讀':'儲存'}</button></td></tr>`).join('')}</tbody>`;
  if(readonly)return;
  table.querySelectorAll('.canonical-save-recipe').forEach(button=>button.addEventListener('click',async()=>{const id=button.dataset.id,row=data.find(item=>item.recipe_id===id),selector=CSS.escape(id),unlocked=table.querySelector(`.canonical-recipe-unlocked[data-id="${selector}"]`).checked,level=table.querySelector(`.canonical-recipe-level[data-id="${selector}"]`).value,energy=table.querySelector(`.canonical-recipe-energy[data-id="${selector}"]`).value;try{await saveRecipeState(row,unlocked,level,energy);renderRecipeCatalog();window.dispatchEvent(new CustomEvent('pokemon-sleep:data-changed',{detail:{entity:'recipes',operation:'manual_upsert'}}));}catch(error){alert(error.message);}}));
}
...
function renderView(view){
  ...
  try{
    if(view==='ingredients')renderIngredientCatalog();else if(view==='items')renderItemCatalog();else renderRecipeCatalog();
    progress('PUBLIC_CATALOG_RENDER_COMPLETED',`${view}公版資料載入完成`,'completed',{view,generation:requestedGeneration,recipe_master_version:view==='recipes'?PUBLIC_RECIPE_MASTER_VERSION:undefined,rescue_readonly:isRescueReadonly()});return true;

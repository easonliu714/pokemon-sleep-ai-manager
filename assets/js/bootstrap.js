...
function showVisibleVersion(){
  ...
  if(!badge){badge=document.createElement('span');badge.id='appVersion';badge.className='badge';badge.style.marginInlineStart='8px';badge.style.whiteSpace='nowrap';const db=document.getElementById('dbStatus');if(db?.parentElement===header)db.insertAdjacentElement('afterend',badge);else header.append(badge);}
  badge.textContent=`版本 ${APP_VERSION}`;badge.dataset.versionAuthority='bootstrap';badge.title=`Pokémon Sleep AI Manager ${APP_VERSION} / ${VERSION}`;
}
function enforceVersionAuthority(){
  const root=document.documentElement;let repaired=false;
  if(root.dataset.appVersion!==APP_VERSION){root.dataset.appVersion=APP_VERSION;repaired=true;}
  if(root.dataset.appBuild!==VERSION){root.dataset.appBuild=VERSION;repaired=true;}
  const badge=document.getElementById('appVersion');if(badge&&badge.textContent!==`版本 ${APP_VERSION}`){badge.textContent=`版本 ${APP_VERSION}`;badge.dataset.versionAuthority='bootstrap';repaired=true;}
  if(repaired)debugTrace.record('bootstrap','version_authority_repaired',{status:'completed',details:{expected_version:APP_VERSION,expected_build:VERSION}});
}
showVisibleVersion();
...
addEventListener('pagehide',()=>versionObserver.disconnect(),{once:true});

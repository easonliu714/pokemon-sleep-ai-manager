...
// Only startup authorities required to observe DB/app readiness stay on the critical path.
// The feature probes remain exact-build validation, but are loaded after business-ready.
const criticalProbes=['runtime-version.js','v0382-release-authority.js'];
...
function waitForAppReady(){if(document.documentElement.dataset.databaseReady==='true'&&document.getElementById('dbStatus')?.textContent==='App 已就緒')return Promise.resolve();return new Promise(resolve=>globalThis.addEventListener('pokemon-sleep:app-ready',()=>resolve(),{once:true}));}

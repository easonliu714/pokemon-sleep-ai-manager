...
export const UC_IMG_A_VERSION='uc-img-a-2026-08-11-b';
...
const emptyScenarioState=()=>({raw_response:'',response_prompt_revision:null,response_stale:false,last_update_id:null,last_apply_status:null});
...
export function mountUnifiedScreenshotUpdateCenter(){
  ...
  function bind(){
    ...
    root.querySelectorAll('.uc-img-scenario').forEach(panel=>{
      ...
      panel.querySelector('.uc-img-copy-prompt').onclick=async()=>{try{await navigator.clipboard.writeText(buildScreenshotScenarioPrompt(session,key));alert('Prompt 已複製；請連同對應圖片交給信賴的 AI 模型。');}catch(error){alert(`複製失敗：${error.message}`);}};
      panel.querySelector('.uc-img-response').oninput=e=>{const state=session.scenario_state[key];state.raw_response=e.target.value;state.response_prompt_revision=screenshotScenarioRevision(session,key);state.response_stale=false;state.last_apply_status=null;save();clearRt(key);};

...
export const UC_IMG_A_VERSION='uc-img-a-2026-08-11-c-dual-mode';
...
export function buildScreenshotScenarioPrompt(session,scenarioKey){
  ...
  const coverage=session.coverage?.[scenarioKey]||'PARTIAL',base=PROMPT_CATALOG[config.promptKey]?.prompt;if(!base)throw new Error(`找不到 Prompt Catalog：${config.promptKey}`);
  const map=entries.map(entry=>`  - ${entry.image_ref} = ${entry.file_name}`).join('\n');
...
export function validateScreenshotScenarioPayload(session,scenarioKey,input){
  const config=UC_IMG_A_SCENARIOS[scenarioKey];if(!config)throw new Error(`不支援的情境：${scenarioKey}`);
  let payload;try{const value=extractJsonObjectText(input);payload=typeof value==='string'?JSON.parse(value):clone(value);}catch(error){return {ok:false,payload:null,errors:[`JSON 解析失敗：${error.message}`],warnings:[],review:[],summary:{scenario:config.scenario}};}
...
export function mountUnifiedScreenshotUpdateCenter(){
  ...
  const hasImageBytes=key=>{const entries=scenarioEntries(session,key);return Boolean(entries.length&&entries.every(entry=>runtime.files.has(entry.entry_id)));};
  const panelMarkup=key=>{
    ...
    const stale=s.response_stale&&s.raw_response?'<div class="uc-img-stale">圖片分類／Coverage 已變更：目前 AI JSON 已過期，請重新分析或依最新 Prompt 貼回新的辨識 JSON。</div>':'';
    const aiError=s.last_ai_error?`<div class="uc-img-ai-error">${esc(s.last_ai_error)}</div>`:'';
    const provider=s.provider_meta?.provider?`最近來源：${esc(s.provider_meta.provider)}${s.provider_meta.model?` · ${esc(s.provider_meta.model)}`:''}${s.provider_meta.project_alias?` · ${esc(s.provider_meta.project_alias)}`:''}`:'尚無 AI 回應來源';
    return `<section class="uc-img-scenario" data-scenario="${key}"><div class="uc-img-scenario-head"><div><b>${esc(cfg.label)}</b><div class="notice">scenario=<code>${esc(cfg.scenario)}</code> · 已指定 ${count(key)} 張</div></div><label>Coverage <select class="uc-img-coverage"><option value="PARTIAL" ${session.coverage[key]==='PARTIAL'?'selected':''}>PARTIAL／部分畫面</option><option value="USER_CONFIRMED_COMPLETE" ${session.coverage[key]==='USER_CONFIRMED_COMPLETE'?'selected':''}>USER_CONFIRMED_COMPLETE／我確認完整</option></select></label></div><p class="notice">完整 Coverage 只是 Evidence；未出現項目仍不會自動歸零、鎖定或刪除。</p><div class="uc-img-ai-note"><b>雙模式 AI Intake</b><br>內部 Gemini：直接送出目前頁面的圖片 bytes，取得 Structured JSON；外部 Prompt：可交給 ChatGPT、Gemini 或其他模型。兩者最後都只會進入同一套 Review / Dry Run / Apply。</div><div class="uc-img-ai-bar"><label>分析方式 <select class="uc-img-ai-mode"><option value="internal" ${s.ai_mode==='internal'?'selected':''}>Gemini API 直接分析</option><option value="external" ${s.ai_mode==='external'?'selected':''}>外部 AI Prompt</option></select></label><button class="uc-img-internal" ${prompt&&hasImageBytes(key)?'':'disabled'}>Gemini API 直接分析</button><button class="uc-img-open-settings">API Key / 模型設定</button></div><div class="notice">${hasImageBytes(key)?'圖片 bytes：READY':'圖片 bytes：NOT AVAILABLE（重新整理後需重新選圖才能內部分析）'} · ${provider}</div>${aiError}<details ${s.ai_mode==='external'?'open':''}><summary>外部 AI Prompt／備援模式</summary><textarea class="uc-img-prompt" readonly placeholder="先指定此類型圖片">${esc(prompt)}</textarea><div class="buttons"><button class="uc-img-copy-prompt" ${prompt?'':'disabled'}>複製外部 AI Prompt</button></div></details>${stale}<label><b>AI JSON（內部產生或外部貼回）</b><textarea class="uc-img-response" placeholder="內部 Gemini 成功後會自動顯示；也可貼 raw JSON 或 JSON code fence">${esc(s.raw_response||'')}</textarea></label><div class="buttons"><button class="uc-img-copy-json" ${s.raw_response?'':'disabled'}>複製 JSON</button><button class="uc-img-copy-diagnostic" ${s.raw_response?'':'disabled'}>複製 AI 診斷包</button><button class="uc-img-parse">解析／結構檢查</button><button class="uc-img-approve" disabled>確認待覆核</button><button class="uc-img-dry" disabled>使用既有 Dry Run</button><button class="uc-img-apply danger" disabled>套用更新</button></div><div class="uc-img-result" data-result>尚未解析。</div><div class="uc-img-issues" data-issues></div><div data-preview></div></section>`;
  };
  function render(){
    root.innerHTML=`<div class="uc-img-a-head"><div><h3>從遊戲截圖更新 <small>UC.IMG-A</small></h3><p class="notice">本週環境、食材庫存、料理狀態。支援 Gemini API 直接分析與外部 AI Prompt 雙模式；圖片只留目前頁面記憶體。</p></div><span class="badge">${esc(session.session_id)}</span></div><div class="uc-img-privacy">隱私：截圖不會寫入 SQLite 或 GitHub；Gemini API Key 沿用既有 Key Vault / Project Pool，不會寫入更新 JSON。內部或外部 AI 的結果都必須經既有 Review → Dry Run → Snapshot → Apply。</div><div class="uc-img-a-toolbar buttons"><label>加入截圖 <input id="ucImgFiles" type="file" accept="image/*" multiple></label><button id="ucImgReset">建立新工作階段</button><button id="ucImgReload">重新載入資料畫面</button></div><div class="notice">圖片：${session.entries.length} 張；Weekly=${count('weekly')}、食材=${count('ingredients')}、料理=${count('recipes')}、未分類=${session.entries.filter(i=>!i.scenario_key).length}</div><div class="uc-img-a-images">${session.entries.length?session.entries.map(e=>`<article class="uc-img-card" data-entry-id="${esc(e.entry_id)}"><div class="uc-img-preview">${e.object_url?`<img src="${esc(e.object_url)}" alt="${esc(e.image_ref)}">`:'<span>圖片 bytes 未保留<br>需要時重新選取即可</span>'}</div><div class="uc-img-ref">${esc(e.image_ref)}</div><div class="uc-img-meta">${esc(e.file_name)} · ${e.file_size?`${Math.round(e.file_size/1024)} KB`:'metadata only'}</div><select class="uc-img-classify">${scenarioOptions(e.scenario_key)}</select><div class="buttons"><button class="uc-img-remove">移除</button></div></article>`).join(''):'<div class="notice">尚未加入圖片。</div>'}</div><div class="uc-img-scenarios">${scenarioKeys.map(panelMarkup).join('')}</div>`;
    bind();for(const key of scenarioKeys)renderResult(key);
  }
  function bind(){
    root.querySelector('#ucImgFiles').onchange=event=>{for(const file of [...event.target.files]){const entry=addScreenshotEntry(session,file);runtime.files.set(entry.entry_id,file);try{entry.object_url=URL.createObjectURL(file);entry.image_available=true;}catch{}}save();clearRt();render();};
  ...
  async function runInternal(key){
    ...
    if(!hasImageBytes(key))return alert('內部 Gemini 需要目前頁面的圖片 bytes。若曾重新整理，請重新選取圖片。');
  ...
  function parse(key){const state=session.scenario_state[key];if(state?.response_stale)return alert('目前 AI JSON 已過期；請依最新圖片分類／Coverage 重新產生並貼回。');const r=rt(key);r.preview=null;r.result=validateScreenshotScenarioPayload(session,key,state?.raw_response||'');renderResult(key);}
  ...
  function dry(key){const r=rt(key);if(session.scenario_state[key]?.response_stale)return alert('AI JSON 已過期，請重新辨識。');if(!r.result?.payload)return;if(r.result.errors.length||r.result.review.length)return alert('請先排除結構錯誤與待覆核項目');try{r.preview=dryRun(r.result.payload);renderResult(key);}catch(error){alert(error.message);}}
  ...
  function renderResult(key){
    ...
    if(state.response_stale){resultEl.textContent='AI JSON 已過期；請依最新 Prompt 重新辨識。';issues.innerHTML='';preview.innerHTML='';approveBtn.disabled=dryBtn.disabled=applyBtn.disabled=true;return;}

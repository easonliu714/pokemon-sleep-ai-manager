...
export function mountUnifiedScreenshotUpdateCenter(){
  ...
  const panelMarkup=key=>{const cfg=UC_IMG_A_SCENARIOS[key],s=session.scenario_state[key]||emptyScenarioState();let prompt='';try{if(count(key))prompt=buildScreenshotScenarioPrompt(session,key);}catch{}const stale=s.response_stale&&s.raw_response?'<div class="uc-img-stale">圖片分類／Coverage／Public Master 已變更：目前 AI JSON 已過期，請重新分析或依最新 Prompt 貼回新的辨識 JSON。</div>':'';const aiError=s.last_ai_error?`<div class="uc-img-ai-error">${esc(s.last_ai_error)}</div>`:'';const provider=s.provider_meta?.provider?`最近來源：${esc(s.provider_meta.provider)}${s.provider_meta.model?` · ${esc(s.provider_meta.model)}`:''}${s.provider_meta.project_alias?` · ${esc(s.provider_meta.project_alias)}`:''}${s.provider_meta.response_contract?` · ${esc(s.provider_meta.response_contract)}`:''}`:'尚無 AI 回應來源';const recognitionNote=supportsPublicMasterRecognition(cfg.scenario)?'<br><b>公版約束：</b>AI 先回傳 MATCHED / AMBIGUOUS / UNMATCHED Recognition；平台再驗證 Public Master 並編譯 Update Package。':' ';return `<section class="uc-img-scenario" data-scenario="${key}"><div class="uc-img-scenario-head"><div><b>${esc(cfg.label)}</b><div class="notice">scenario=<code>${esc(cfg.scenario)}</code> · 已指定 ${count(key)} 張</div></div><label>Coverage <select class="uc-img-coverage"><option value="PARTIAL" ${session.coverage[key]==='PARTIAL'?'selected':''}>PARTIAL／部分畫面</option><option value="USER_CONFIRMED_COMPLETE" ${session.coverage[key]==='USER_CONFIRMED_COMPLETE'?'selected':''}>USER_CONFIRMED_COMPLETE／我確認完整</option></select></label></div><p class="notice">完整 Coverage 只是 Evidence；未出現項目仍不會自動歸零、鎖定或刪除。</p><div class="uc-img-ai-note"><b>雙模式 AI Intake</b><br>內部 Gemini：直接送出目前頁面的圖片 bytes；外部 Prompt：可交給 ChatGPT、Gemini 或其他模型。兩者最後都只會進入同一套 Review / Dry Run / Apply。${recognitionNote}</div><div class="uc-img-ai-bar"><label>分析方式 <select class="uc-img-ai-mode"><option value="internal" ${s.ai_mode==='internal'?'selected':''}>Gemini API 直接分析</option><option value="external" ${s.ai_mode==='external'?'selected':''}>外部 AI Prompt</option></select></label><button class="uc-img-internal" ${prompt&&hasImageBytes(key)?'':'disabled'}>Gemini API 直接分析</button><button class="uc-img-open-settings">API Key / 模型設定</button></div><div class="notice">${esc(byteSummary(key))} · ${provider}</div>${aiError}<details ${s.ai_mode==='external'?'open':''}><summary>外部 AI Prompt／備援模式</summary><textarea class="uc-img-prompt" readonly placeholder="先指定此類型圖片">${esc(prompt)}</textarea><div class="buttons"><button class="uc-img-copy-prompt" ${prompt?'':'disabled'}>複製外部 AI Prompt</button></div></details>${stale}<label><b>AI JSON（原始辨識結果，可直接複製供稽核）</b><textarea class="uc-img-response" placeholder="內部 Gemini 成功後會自動顯示；也可貼 raw JSON 或 JSON code fence">${esc(s.raw_response||'')}</textarea></label><div class="buttons"><button class="uc-img-copy-json" ${s.raw_response?'':'disabled'}>複製 JSON</button><button class="uc-img-copy-diagnostic" ${s.raw_response?'':'disabled'}>匯出 AI 診斷包</button><button class="uc-img-parse">解析／結構檢查</button><button class="uc-img-approve" disabled>確認待覆核</button><button class="uc-img-dry" disabled>使用既有 Dry Run</button><button class="uc-img-apply danger" disabled>套用更新</button></div><div class="uc-img-result" data-result>尚未解析。</div><div class="uc-img-issues" data-issues></div><div data-preview></div></section>`;};

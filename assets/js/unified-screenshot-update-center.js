...
export function mountUnifiedScreenshotUpdateCenter(){
  if(!hasDocument)return null;const host=document.getElementById('updateCenterScreenshotMount')||document.getElementById('updateCenterDynamicContent');if(!host||document.getElementById('ucImgA'))return null;installStyle();
  let session=restoreScreenshotSession();const runtime={files:new Map(),scenario:{}};const root=document.createElement('section');root.id='ucImgA';root.className='uc-img-a';host.replaceChildren(root);

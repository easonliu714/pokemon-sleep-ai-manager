...
import {downloadUcImgDiagnosticJson} from './uc-img-diagnostic-export.js';
...
const emptyScenarioState=()=>({raw_response:'',provider_raw_response:'',response_prompt_revision:null,response_stale:false,last_update_id:null,last_apply_status:null,ai_mode:'internal',provider_meta:null,last_ai_error:null});
...
export function setScenarioAiMode(session,scenarioKey,mode){
  if(!UC_IMG_A_SCENARIOS[scenarioKey])throw new Error(`不支援的情境：${scenarioKey}`);
  if(!UC_IMG_A_MODES.includes(mode))throw new Error(`不支援 AI 模式：${mode}`);
...
export function validateScreenshotScenarioPayload(session,scenarioKey,input){
  ...
  if(isPublicMasterRecognitionPayload(sourcePayload)){
    ...
    recognition={...compiled,source_payload:sourcePayload,catalog_options:compiled.snapshot?.rows?.map(row=>row[compiled.snapshot.display_name_field]).filter(Boolean)||[]};
  ...
  return {ok:uniqueErrors.length===0&&review.length===0,payload,source_payload:sourcePayload,recognition,errors:uniqueErrors,warnings:uniqueWarnings,review,summary:{...(workflow.summary||{}),uc_img_a_version:UC_IMG_A_VERSION,session_id:session.session_id,scenario_key:scenarioKey,expected_scenario:config.scenario,assigned_image_count:assignedRefs.size,coverage,traceable_evidence:traceableEvidence,recognition_contract:Boolean(recognition),recognition_authority:recognition?.summary?.authority||null,recognition_matched_count:recognition?.summary?.matched_count??null,recognition_unresolved_count:recognition?.summary?.unresolved_count??null,recognition_ignored_count:recognition?.summary?.ignored_count??null,catalog_snapshot_id:recognition?.summary?.catalog_snapshot_id||null}};
}
...
function weeklyBerryReviewHtml(result,key){
  ...
  const payload=result?.source_payload||result?.payload;
  const observations=Array.isArray(payload?.visual_observations)?payload.visual_observations:[];
  const unresolved=observations.filter(item=>item?.status!=='VERIFIED');
  const needsTextReview=(result?.review||[]).some(item=>item?.kind==='weekly_field_confidence_missing');
  if(!unresolved.length&&!needsTextReview)return '';
  const options=weeklyBerryOptions();
  const optionHtml=`<option value="">請依原圖選擇正確公版樹果</option>${options.map(name=>`<option value="${esc(name)}">${esc(name)}</option>`).join('')}`;
  const cards=unresolved.map(item=>`<article class="uc-img-recognition-card uc-img-weekly-berry-card" data-slot="${esc(item.slot)}"><div><b>樹果 Slot ${esc(item.slot)}</b> · AI 狀態 ${esc(item.status||'UNKNOWN')} · confidence=${esc(item.confidence??'—')}</div><div class="notice">AI 候選：<b>${esc(item.candidate_name||'未能命名')}</b>。AI 候選不是 authority；請看原始遊戲截圖後，從公版樹果清單人工確認。</div><select class="uc-img-weekly-berry-select">${optionHtml}</select><div class="uc-img-recognition-actions"><button data-weekly-berry-confirm>確認這個 Slot</button></div></article>`).join('');
  const textReview=needsTextReview?'<article class="uc-img-recognition-card"><b>本週文字欄位待確認</b><div class="notice">目前只有 operation-level AI confidence，不能當成每個文字欄位的 authority。請對照原圖確認本週日期／營地／料理類型／活動文字後再繼續。</div><button data-weekly-text-confirm>我已逐欄對照原圖，確認目前文字欄位</button></article>':'';
  return `<section class="uc-img-recognition" data-weekly-visual-review><b>本週樹果／文字人工覆核</b><div class="notice">只有人工選擇公版樹果或既有 exact governed resolver 才能解除 Slot blocker；HTTP 200 / confidence 不會自動 VERIFIED。</div>${cards}${textReview}</section>`;
}

async function copyText(text){if(navigator.clipboard?.writeText){await navigator.clipboard.writeText(text);return true;}const area=document.createElement('textarea');area.value=text;area.style.position='fixed';area.style.opacity='0';document.body.appendChild(area);area.select();const ok=document.execCommand('copy');area.remove();if(!ok)throw new Error('瀏覽器未允許複製');return true;}
...
export function mountUnifiedScreenshotUpdateCenter(){
  ...
  const byteSummary=key=>{const entries=scenarioEntries(session,key);if(!entries.length)return '圖片 bytes：NOT AVAILABLE';const ready=entries.filter(entryReady).length,pending=entries.filter(entry=>entry.byte_state==='PENDING').length,failed=entries.filter(entry=>entry.byte_state==='READ_FAILED').length;if(failed)return `圖片 bytes：READ FAILED ${failed}/${entries.length}`;if(pending)return `圖片 bytes：準備中 ${ready}/${entries.length}`;if(ready===entries.length)return `圖片 bytes：READY（記憶體快照） ${ready}/${entries.length}`;return `圖片 bytes：NOT AVAILABLE ${ready}/${entries.length}`;};
  const panelMarkup=key=>{const cfg=UC_IMG_A_SCENARIOS[key],s=session.scenario_state[key]||emptyScenarioState();let prompt='';try{if(count(key))prompt=buildScreenshotScenarioPrompt(session,key);}catch{}const stale=s.response_stale&&s.raw_response?'<div class="uc-img-stale">圖片分類／Coverage／Public Master 已變更：目前 AI JSON 已過期，請重新分析或依最新 Prompt 貼回新的辨識 JSON。</div>':'';const aiError=s.last_ai_error?`<div class="uc-img-ai-error">${esc(s.last_ai_error)}</div>`:'';const provider=s.provider_meta?.provider?`最近成功來源：${esc(s.provider_meta.provider)}${s.provider_meta.model?` · ${esc(s.provider_meta.model)}`:''}${s.provider_meta.project_alias?` · ${esc(s.provider_meta.project_alias)}`:''}${s.provider_meta.response_contract?` · ${esc(s.provider_meta.response_contract)}`:''}`:'尚無成功 AI 回應來源';const recognitionNote=supportsPublicMasterRecognition(cfg.scenario)?'<br><b>公版約束：</b>AI 先回傳 MATCHED / AMBIGUOUS / UNMATCHED Recognition；平台再驗證 Public Master 並編譯 Update Package。':' ';return `<section class="uc-img-scenario" data-scenario="${key}"><div class="uc-img-scenario-head"><div><b>${esc(cfg.label)}</b><div class="notice">scenario=<code>${esc(cfg.scenario)}</code> · 已指定 ${count(key)} 張</div></div><label>Coverage <select class="uc-img-coverage"><option value="PARTIAL" ${session.coverage[key]==='PARTIAL'?'selected':''}>PARTIAL／部分畫面</option><option value="USER_CONFIRMED_COMPLETE" ${session.coverage[key]==='USER_CONFIRMED_COMPLETE'?'selected':''}>USER_CONFIRMED_COMPLETE／我確認完整</option></select></label></div><p class="notice">完整 Coverage 只是 Evidence；未出現項目仍不會自動歸零、鎖定或刪除。</p><div class="uc-img-ai-note"><b>雙模式 AI Intake</b><br>內部 Gemini：直接送出目前頁面的圖片 bytes；外部 Prompt：可交給 ChatGPT、Gemini 或其他模型。兩者最後都只會進入同一套 Review / Dry Run / Apply。${recognitionNote}</div><div class="uc-img-ai-bar"><label>分析方式 <select class="uc-img-ai-mode"><option value="internal" ${s.ai_mode==='internal'?'selected':''}>Gemini API 直接分析</option><option value="external" ${s.ai_mode==='external'?'selected':''}>外部 AI Prompt</option></select></label><button class="uc-img-internal" ${prompt&&hasImageBytes(key)?'':'disabled'}>Gemini API 直接分析</button><button class="uc-img-open-settings">API Key / 模型設定</button></div><div class="notice">${esc(byteSummary(key))} · ${provider}</div>${aiError}<details ${s.ai_mode==='external'?'open':''}><summary>外部 AI Prompt／備援模式</summary><textarea class="uc-img-prompt" readonly placeholder="先指定此類型圖片">${esc(prompt)}</textarea><div class="buttons"><button class="uc-img-copy-prompt" ${prompt?'':'disabled'}>複製外部 AI Prompt</button></div></details>${stale}${s.provider_raw_response?`<details><summary><b>Provider Raw JSON（唯讀稽核，不參與覆核修改）</b></summary><textarea class="uc-img-provider-raw" readonly>${esc(s.provider_raw_response)}</textarea></details>`:''}<label><b>Working / Normalized JSON（覆核與 Dry Run 使用）</b><textarea class="uc-img-response" placeholder="內部 Gemini 成功後由平台 normalization 產生；外部 AI 也可直接貼 raw JSON 或 JSON code fence">${esc(s.raw_response||'')}</textarea></label><div class="buttons"><button class="uc-img-copy-json" ${s.raw_response?'':'disabled'}>複製 JSON</button><button class="uc-img-copy-diagnostic" ${s.raw_response||s.last_ai_error?'':'disabled'}>匯出 AI 診斷包</button><button class="uc-img-parse">解析／結構檢查</button><button class="uc-img-approve" disabled>確認待覆核</button><button class="uc-img-dry" disabled>使用既有 Dry Run</button><button class="uc-img-apply danger" disabled>套用更新</button></div><div class="uc-img-result" data-result>尚未解析。</div><div class="uc-img-issues" data-issues></div><div data-preview></div></section>`;};
  function render(){root.innerHTML=`<div class="uc-img-a-head"><div><h3>從遊戲截圖更新 <small>UC.IMG-A</small></h3><p class="notice">本週環境、食材庫存、料理狀態。支援 Gemini API 直接分析與外部 AI Prompt 雙模式；有 Public Master 的情境先做 constrained recognition；圖片只留目前頁面記憶體。</p></div><span class="badge">${esc(session.session_id)}</span></div><div class="uc-img-privacy">隱私：Public Master 候選只包含公版資料，不含玩家庫存／解鎖／私人寶可夢；截圖不會寫入 SQLite 或 GitHub；Gemini API Key 沿用既有 Key Vault / Project Pool。只有平台 revalidate 後的 MATCHED 才能進既有 Review → Dry Run → Snapshot → Apply。</div><div class="uc-img-a-toolbar buttons"><label>加入截圖 <input id="ucImgFiles" type="file" accept="image/*" multiple></label><button id="ucImgReset">建立新工作階段</button><button id="ucImgReload">重新載入資料畫面</button></div><div class="notice">圖片：${session.entries.length} 張；Weekly=${count('weekly')}、食材=${count('ingredients')}、料理=${count('recipes')}、未分類=${session.entries.filter(i=>!i.scenario_key).length}</div><div class="uc-img-a-images">${session.entries.length?session.entries.map(e=>`<article class="uc-img-card" data-entry-id="${esc(e.entry_id)}"><div class="uc-img-preview">${e.object_url?`<img src="${esc(e.object_url)}" alt="${esc(e.image_ref)}">`:`<span>${esc(ucImgByteStateLabel(e))}<br>${e.byte_state==='READ_FAILED'?'請重新選取此圖片':'圖片 bytes 不會持久保存'}</span>`}</div><div class="uc-img-ref">${esc(e.image_ref)}</div><div class="uc-img-meta">${esc(e.file_name)} · ${e.file_size?`${Math.round(e.file_size/1024)} KB`:'metadata only'} · <span class="${e.byte_state==='READ_FAILED'?'uc-img-byte-error':''}">${esc(ucImgByteStateLabel(e))}</span></div><select class="uc-img-classify">${scenarioOptions(e.scenario_key)}</select><div class="buttons"><button class="uc-img-remove">移除</button></div></article>`).join(''):'<div class="notice">尚未加入圖片。</div>'}</div><div class="uc-img-scenarios">${scenarioKeys.map(panelMarkup).join('')}</div>`;bind();for(const key of scenarioKeys)renderResult(key);}
  function bind(){
    root.querySelector('#ucImgFiles').onchange=async event=>{const files=[...event.target.files];if(!files.length)return;const jobs=files.map(file=>{const entry=addScreenshotEntry(session,file);entry.byte_state='PENDING';entry.byte_error=null;return snapshotUcImgPickerFile(file).then(snapshot=>({ok:true,entry,snapshot})).catch(error=>({ok:false,entry,error}));});save();clearRt();render();const results=await Promise.all(jobs),failures=[];for(const result of results){const {entry}=result;if(result.ok){runtime.files.set(entry.entry_id,result.snapshot.blob);entry.byte_state='READY';entry.byte_error=null;entry.byte_snapshot_size=result.snapshot.byte_length;entry.mime_type=result.snapshot.mime_type;entry.image_available=true;try{entry.object_url=URL.createObjectURL(result.snapshot.blob);}catch{entry.object_url=null;}}else{runtime.files.delete(entry.entry_id);entry.byte_state='READ_FAILED';entry.byte_error=result.error?.message||String(result.error);entry.byte_snapshot_size=0;entry.image_available=false;entry.object_url=null;failures.push(entry);}}save();clearRt();render();if(failures.length)alert(`有 ${failures.length} 張圖片在選取後立即讀取失敗，已禁止內部 Gemini：\n${failures.map(entry=>`${entry.image_ref} ${entry.file_name}`).join('\n')}\n請重新選取失敗圖片。`);};
    ...
    root.querySelectorAll('.uc-img-scenario').forEach(panel=>{const key=panel.dataset.scenario;panel.querySelector('.uc-img-coverage').onchange=e=>{setScenarioCoverage(session,key,e.target.value);save();clearRt(key);render();};panel.querySelector('.uc-img-ai-mode').onchange=e=>{setScenarioAiMode(session,key,e.target.value);save();render();};panel.querySelector('.uc-img-internal').onclick=()=>runInternal(key);panel.querySelector('.uc-img-open-settings').onclick=()=>{document.querySelector('nav button[data-view="guide"]')?.click();setTimeout(()=>document.getElementById('aiProjectPoolSettings')?.scrollIntoView({behavior:'smooth',block:'start'}),0);};panel.querySelector('.uc-img-copy-prompt').onclick=async()=>{try{await copyText(buildScreenshotScenarioPrompt(session,key));alert('外部 AI Prompt 已複製；請連同對應圖片交給信賴的 AI 模型。');}catch(error){alert(`複製失敗：${error.message}`);}};panel.querySelector('.uc-img-copy-json').onclick=async()=>{try{await copyText(session.scenario_state[key]?.raw_response||'');alert('AI JSON 已複製。');}catch(error){alert(`複製失敗：${error.message}`);}};
      panel.querySelector('.uc-img-copy-diagnostic').onclick=async()=>{try{const r=rt(key),state=session.scenario_state[key];const validation=r.result||(state?.raw_response?validateScreenshotScenarioPayload(session,key,state.raw_response):null);const bundle=buildUcImgDiagnosticBundle({appVersion:globalThis.PokemonSleepVersionAuthority?.app_version||document.documentElement.dataset.appVersion||null,session,scenarioKey:key,config:UC_IMG_A_SCENARIOS[key],coverage:session.coverage[key],rawResponse:state?.raw_response||'',validation,providerMeta:state?.provider_meta});const filename=downloadUcImgDiagnosticJson(bundle);alert(`AI 診斷包已匯出：${filename}\n不含 API Key、圖片 bytes 或 SQLite。`);}catch(error){alert(`診斷包建立失敗：${error.message}`);}};
      panel.querySelector('.uc-img-response').oninput=e=>{const state=session.scenario_state[key];state.raw_response=e.target.value;if(state.provider_meta?.provider==='gemini'){}else if(!state.provider_raw_response)state.provider_raw_response=e.target.value;state.response_prompt_revision=screenshotScenarioRevision(session,key);state.response_stale=false;state.last_apply_status=null;state.last_ai_error=null;state.provider_meta={provider:'external_or_manual',model:null,project_alias:null,image_count:count(key),response_contract:null,platform_authority:null,completed_at:nowIso()};save();clearRt(key);};panel.querySelector('.uc-img-parse').onclick=()=>parse(key);panel.querySelector('.uc-img-approve').onclick=()=>approve(key);panel.querySelector('.uc-img-dry').onclick=()=>dry(key);panel.querySelector('.uc-img-apply').onclick=()=>apply(key);});
  }
  async function runInternal(key){const cfg=UC_IMG_A_SCENARIOS[key],entries=scenarioEntries(session,key),state=session.scenario_state[key],r=rt(key);if(r.busy)return;if(!hasImageBytes(key))return alert('內部 Gemini 只接受已完成平台記憶體快照的圖片。若顯示準備中／讀取失敗／重新整理後遺失，請重新選取圖片。');const poolData=globalThis.PokemonSleepAiProjectPool;if(!poolData?.projects?.length)return alert('尚未設定 Gemini API Key。請先到「使用說明 → AI API Key 與備援 Project」設定並測試 Key。');r.busy=true;state.last_ai_error=null;render();try{const analysis=await analyzeUcImgScenarioWithGemini({scenarioKey:key,config:cfg,entries,fileMap:runtime.files,prompt:buildScreenshotScenarioPrompt(session,key),poolData,onTrace:(event,details)=>globalThis.DebugTrace?.record?.('uc_img_gemini',event,{status:event.endsWith('failed')?'failed':event.endsWith('completed')?'completed':'started',details})});globalThis.PokemonSleepAiProjectPool={...poolData,projects:analysis.projects};state.provider_raw_response=String(analysis.raw_provider_text??'');state.raw_response=analysis.raw_json;state.response_prompt_revision=screenshotScenarioRevision(session,key);state.response_stale=false;state.last_apply_status=null;state.ai_mode='internal';state.provider_meta={provider:'gemini',model:analysis.model,project_alias:analysis.project_alias,image_count:analysis.image_count,response_contract:analysis.response_contract,platform_authority:analysis.platform_authority||null,completed_at:analysis.completed_at};state.last_ai_error=null;save();r.result=validateScreenshotScenarioPayload(session,key,analysis.payload);r.preview=null;}catch(error){state.last_ai_error=error?.message||String(error);save();alert(`Gemini 分析失敗：${state.last_ai_error}`);}finally{r.busy=false;render();}}
  ...
  function resolveRecognition(key,observationId,action,displayName=null){try{const state=session.scenario_state[key],value=extractJsonObjectText(state?.raw_response||''),raw=typeof value==='string'?JSON.parse(value):clone(value);if(!isPublicMasterRecognitionPayload(raw))throw new Error('目前 AI JSON 不是 Public Master Recognition 格式');const updated=applyPublicMasterRecognitionResolution(raw,UC_IMG_A_SCENARIOS[key].scenario,observationId,action,displayName);state.raw_response=JSON.stringify(updated,null,2);state.response_prompt_revision=screenshotScenarioRevision(session,key);state.response_stale=false;save();const r=rt(key);r.preview=null;r.result=validateScreenshotScenarioPayload(session,key,updated);renderResult(key);}catch(error){alert(`Public Master 對應失敗：${error.message}`);}}
  function resolveWeeklyBerry(key,slot,canonicalName){try{const state=session.scenario_state[key],value=extractJsonObjectText(state?.raw_response||''),raw=typeof value==='string'?JSON.parse(value):clone(value);const updated=resolveWeeklyBerryVisualCandidate(raw,Number(slot),canonicalName);state.raw_response=JSON.stringify(updated,null,2);state.response_prompt_revision=screenshotScenarioRevision(session,key);state.response_stale=false;save();const r=rt(key);r.preview=null;r.result=validateScreenshotScenarioPayload(session,key,updated);renderResult(key);}catch(error){alert(`本週樹果覆核失敗：${error.message}`);}}
  function confirmWeeklyText(key){try{const state=session.scenario_state[key],value=extractJsonObjectText(state?.raw_response||''),raw=typeof value==='string'?JSON.parse(value):clone(value);const updated=confirmWeeklyObservedTextFields(raw);state.raw_response=JSON.stringify(updated,null,2);state.response_prompt_revision=screenshotScenarioRevision(session,key);state.response_stale=false;save();const r=rt(key);r.preview=null;r.result=validateScreenshotScenarioPayload(session,key,updated);renderResult(key);}catch(error){alert(`本週文字欄位覆核失敗：${error.message}`);}}
  function approve(key){const r=rt(key);if(!r.result?.payload||!r.result.review?.length)return;const genericReview=r.result.review.filter(item=>item.kind!=='public_master_recognition');if(!genericReview.length)return alert('請使用 Public Master 對應卡逐筆處理 AMBIGUOUS / UNMATCHED 項目。');if(!confirm(`確認已人工檢查 ${genericReview.length} 筆待覆核項目？`))return;const approved=approveReviewed(r.result.payload);const state=session.scenario_state[key];state.raw_response=JSON.stringify(approved,null,2);state.response_prompt_revision=screenshotScenarioRevision(session,key);state.response_stale=false;save();r.result=validateScreenshotScenarioPayload(session,key,approved);r.preview=null;renderResult(key);}
  ...
  function renderResult(key){const panel=root.querySelector(`.uc-img-scenario[data-scenario="${key}"]`);if(!panel)return;const r=rt(key),result=r.result,state=session.scenario_state[key]||emptyScenarioState(),resultEl=panel.querySelector('[data-result]'),issues=panel.querySelector('[data-issues]'),preview=panel.querySelector('[data-preview]'),approveBtn=panel.querySelector('.uc-img-approve'),dryBtn=panel.querySelector('.uc-img-dry'),applyBtn=panel.querySelector('.uc-img-apply danger')||panel.querySelector('.uc-img-apply'),internalBtn=panel.querySelector('.uc-img-internal');if(internalBtn){internalBtn.disabled=r.busy||!hasImageBytes(key);internalBtn.textContent=r.busy?'Gemini 分析中…':'Gemini API 直接分析';}if(state.response_stale){resultEl.textContent='AI JSON 已過期；請依最新 Prompt／Public Master 重新辨識。';issues.innerHTML='';preview.innerHTML='';approveBtn.disabled=dryBtn.disabled=applyBtn.disabled=true;return;}if(!result){resultEl.textContent=state.last_apply_status==='APPLIED'?`已套用 update_id=${state.last_update_id}`:'尚未解析。';issues.innerHTML='';preview.innerHTML='';approveBtn.disabled=dryBtn.disabled=applyBtn.disabled=true;return;}const rec=result.summary.recognition_contract?` · Master：<b>${esc(result.summary.recognition_authority||'—')}</b> · MATCHED：<b>${result.summary.recognition_matched_count||0}</b> · 未解：<b>${result.summary.recognition_unresolved_count||0}</b> · 忽略：<b>${result.summary.recognition_ignored_count||0}</b>`:'';resultEl.innerHTML=`操作：<b>${result.summary.operation_count||0}</b> · 圖片：<b>${result.summary.assigned_image_count||0}</b> · coverage：<b>${esc(result.summary.coverage||'—')}</b> · Evidence：<b>${result.summary.traceable_evidence?'PASS':'FAIL'}</b> · 錯誤：<b>${result.errors.length}</b> · 警告：<b>${result.warnings.length}</b> · 待覆核：<b>${result.review.length}</b>${rec}`;const weeklyActionKinds=new Set(['weekly_berry_visual_candidate','weekly_berry_visual_observation','weekly_field_confidence_missing','weekly_berry_visual_surface_unreported']);const genericReview=result.review.filter(item=>item.kind!=='public_master_recognition'&&!weeklyActionKinds.has(item.kind));const messages=[...result.errors.map(v=>`<div class="status-conflict">錯誤：${esc(v)}</div>`),...result.warnings.map(v=>`<div>警告：${esc(v)}</div>`),...genericReview.map(v=>`<div>待覆核：${esc(v.operation_id)}／${esc(v.entity)}</div>`)].join('');issues.innerHTML=(messages||(!result.recognition?.unresolved?.length&&!result.review.length?'<div class="status-ready">結構檢查通過。</div>':''))+recognitionReviewHtml(result)+weeklyBerryReviewHtml(result,key);issues.querySelectorAll('.uc-img-recognition-card[data-observation-id]').forEach(card=>{const observationId=card.dataset.observationId,select=card.querySelector('.uc-img-rec-candidate');card.querySelector('[data-rec-action="MATCH"]').onclick=()=>{if(!select.value)return alert('請先選擇一個公版候選。');resolveRecognition(key,observationId,'MATCH',select.value);};card.querySelector('[data-rec-action="IGNORE"]').onclick=()=>{if(confirm('確認這一列是辨識誤判或非目標 UI 文字，忽略且不建立更新 operation？'))resolveRecognition(key,observationId,'IGNORE');};card.querySelector('[data-rec-action="MASTER_GAP"]').onclick=()=>{if(confirm('確認畫面確實存在此項目，但目前 Public Master 沒有可靠對應？此動作只保留 gap evidence，不會寫入玩家資料或公版 Master。'))resolveRecognition(key,observationId,'MASTER_GAP');};});issues.querySelectorAll('.uc-img-weekly-berry-card').forEach(card=>{const select=card.querySelector('.uc-img-weekly-berry-select');card.querySelector('[data-weekly-berry-confirm]').onclick=()=>{if(!select.value)return alert('請先依原圖選擇正確公版樹果。');if(confirm(`確認 Slot ${card.dataset.slot} 在原始遊戲畫面中是「${select.value}」？`))resolveWeeklyBerry(key,Number(card.dataset.slot),select.value);};});const textConfirm=issues.querySelector('[data-weekly-text-confirm]');if(textConfirm)textConfirm.onclick=()=>{if(confirm('確認你已逐欄對照原始遊戲畫面，週起始日／營地／料理類型／活動文字與目前值一致？'))confirmWeeklyText(key);};approveBtn.disabled=!genericReview.length;dryBtn.disabled=Boolean(result.errors.length||result.review.length);preview.innerHTML=previewHtml(r.preview);applyBtn.disabled=!r.preview||r.preview.conflict_count!==0||Boolean(result.errors.length||result.review.length);}

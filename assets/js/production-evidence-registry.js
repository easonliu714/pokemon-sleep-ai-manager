...
export const EVIDENCE_STATUS=Object.freeze({
  ...
  REFERENCE_EVIDENCE_IDENTIFIED:'REFERENCE_EVIDENCE_IDENTIFIED',REFERENCE_COMMUNITY_NUMERIC_EVIDENCE:'REFERENCE_COMMUNITY_NUMERIC_EVIDENCE',REFERENCE_EFFECT_TEXT_ONLY:'REFERENCE_EFFECT_TEXT_ONLY',
...
export function buildProductionEvidenceSnapshot({candidateFeatures={},weeklyContext={},productionRegistry=currentProductionAuthorityRegistry()}={}){
  ...
  const speciesRateReferenceResults=candidates.map(row=>resolveReferenceSpeciesIngredientRate(row));
  const speciesRateReferenceResolved=speciesRateReferenceResults.filter(row=>row.status==='REFERENCE_IDENTIFIED').length;
  const speciesRateReferenceReview=speciesRateReferenceResults.filter(row=>row.status==='REVIEW_REQUIRED').length;

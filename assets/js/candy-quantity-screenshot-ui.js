...
function mount(){
  ...
  function render(){
    ...
    section.innerHTML=`<h3>糖果截圖庫存覆核 <small>P0-B5 / v0.4.27.55.1</small></h3>
      ...
      ${reviewHtml()}
      ${gapHtml()}
      ${state.result?.ok&&operationCount>0?`<div class="status-ready">B5 confirmation Gate PASS：${operationCount} 筆 operation 可進 Dry-Run；${gapCount} 筆 Public Master gap 僅保留 evidence，不寫入。</div>`:''}

...
export const EVALUATION_LIFECYCLE_VERSION='evaluation-lifecycle-2026-08-09-a';
...
export function inspectEvaluationLifecycle({now=new Date(),pokemonIds=null,force=false}={}){
  ...
  const weeklyContext=latestWeeklyContext(),profile=getActiveStrategyGoalProfile(),epoch=localWeekStart(now);
  if(!profile)return Object.freeze({
    ...
    context_id:weeklyContext.context_id||null,goal_profile_id:null,refresh_required:false,write_required:false,auto_refresh_allowed:false,
    target_count:0,refresh_count:0,reused_count:0,stale_count:0,write_performed:false,player_rows_modified:false,
  });
  if(!weeklyContext.context_id||!String(weeklyContext.week_start||'').trim())return Object.freeze({
    ...
    context_id:weeklyContext.context_id||null,goal_profile_id:profile.goal_profile_id||null,refresh_required:false,write_required:false,auto_refresh_allowed:false,
    target_count:0,refresh_count:0,reused_count:0,stale_count:0,write_performed:false,player_rows_modified:false,
  });
  if(!weeklyContextMatchesEpoch(weeklyContext,epoch))return Object.freeze({
    ...
    stored_week_start:String(weeklyContext.week_start||''),context_id:weeklyContext.context_id||null,goal_profile_id:profile.goal_profile_id||null,
  ...
  return Object.freeze({
    ...
    context_id:weeklyContext.context_id||null,goal_profile_id:profile.goal_profile_id||null,auto_refresh_allowed:true,

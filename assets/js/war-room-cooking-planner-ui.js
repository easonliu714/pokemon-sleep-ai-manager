...
const OBJECTIVE_LABEL=Object.freeze({unlock_recipes:'優先解鎖新料理',preserve_resources:'保留食材餘裕',continuous_meals:'連續多餐'});
const WARNING_LABEL=Object.freeze({TARGET_MEALS_NOT_REACHED:'未達目標餐數',SHARED_INVENTORY_CONTENTION:'存在共享食材競爭',USABLE_INVENTORY_EXHAUSTED:'部分可用庫存耗盡',LOW_BUFFER_AFTER_PLAN:'部分食材剩餘不足再做一餐'});
...
function planCard(plan,index){
  return `<article class="war-team-card g7-cooking-plan ${index?'alternative':''}">
    ...
    <div class="war-team-summary"><span>模擬餐數<b>${plan.completed_meals}/${plan.target_meals}</b></span><span>新料理機會<b>${plan.unlock_count}</b></span><span>下一餐仍可選<b>${plan.next_executable_count}</b></span><span>保留後可用總量<b>${plan.remaining_usable_total}</b></span></div>
...
export function renderWarRoomCookingPlanner(root=document.getElementById('warroomCookingPlanner')){
  ...
  try{
    ...
    root.innerHTML=`<div class="panel war-team-optimizer-panel g7-cooking-panel">
      <div class="war-team-toolbar"><div><h3>G7 料理資源競爭／多餐模擬</h3><p class="notice">同一道料理的 READY 判定仍由既有 Recipe Strategy 負責；本區只回答多道 READY 料理是否會搶同一份實體庫存。所有結果都是模擬，不修改食材庫存或料理狀態。</p></div><button type="button" data-g7-refresh>重新計算</button></div>

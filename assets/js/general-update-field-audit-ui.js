...
const FIELD_META = {
  ...
  carry_limit:['持有上限'], favorite_berry:['樹果種類'], sleep_hours:['一起睡覺的時間（換算小時）',v=>v==null?'—':`${v} 小時`], sleep_time_text:['一起睡覺的時間'],
  evolution_sleep_hours_required:['進化所需一起睡覺的時間',v=>v==null?'—':`${v} 小時`],
  obtained_at:['入手日期'], registered_at:['登錄日期'], core_role:['核心定位'], recommendation:['培養建議'], item_advice:['道具建議'], scenarios:['適用情境'], status:['狀態'],
};
const DETAIL_FIELDS = ['level','sp','specialty','type','main_skill','main_skill_level','nature','nature_bonus','nature_penalty','helper_seconds','carry_limit','favorite_berry','sleep_time_text','obtained_at'];
...
const SLEEP_FIELDS = new Set(['sleep_hours','sleep_time_text']);
...
function semanticValue(record, field){
  if(field==='sleep_time_text'){
    if(meaningful(record?.sleep_time_text)) return record.sleep_time_text;
    if(meaningful(record?.sleep_hours)) return `${record.sleep_hours} 小時`;
    return null;
  }
  return record?.[field];
}
...
function ensurePanel() {
  ...
  panel.innerHTML = `
    ...
    <div id="profileAuditDryRunState" class="notice"></div>
    <details class="field-audit-details"><summary>進階：資料庫欄位決策明細</summary>
      <div id="fieldAuditSummary">載入 JSON 後即建立唯讀差異預覽；正式 Dry Run 仍遵守 update_id 與人工確認 Gate。</div>
...
function traceDryRunEligibility(reason) {
  ...
  debugTrace.record('update_center','dry_run_eligibility_changed',{status:'completed',details:{enabled,reason,workflow_error_count:workflowErrorCount(),applied_update_id:appliedUpdateId,update_id:loadedPayload?.update_id||null}});
}
function refreshAppliedState(){
  appliedUpdateId = Boolean(loadedPayload?.update_id && Number(scalar('SELECT COUNT(*) FROM import_batches WHERE update_id=?',[loadedPayload.update_id])||0)>0);
}
function confirmationProgress(){
  ...
  const confirmed=items.filter(item=>item.status==='user_confirmed_not_visible'&&item.confirmed_by_user===true).length;
  return {items,confirmed,total:items.length};
}
function enforceDryRunGate(){
  refreshAppliedState();
  const {confirmed,total}=confirmationProgress();
  const errors=workflowErrorCount();
  if(appliedUpdateId && $('dryRunBtn')) $('dryRunBtn').disabled=true;
  const target=$('profileAuditDryRunState');
  if(!target)return;
  if(appliedUpdateId){
    target.innerHTML=`<b>唯讀比對模式</b>：Update ID <code>${esc(loadedPayload?.update_id||'')}</code> 已套用過；下方仍可核對目前 SQLite 與此 JSON，但正式 Dry Run／Apply 會被重複更新保護阻擋。`;
    return;
  }
  if(total && confirmed<total){
    target.innerHTML=`<b>Dry Run 尚未解鎖</b>：請先完成 ${confirmed}/${total} 項「目前未顯示」人工確認。`;
    return;
  }
  if(errors){
    target.innerHTML=`<b>Dry Run 尚未解鎖</b>：人工確認已完成，但仍有 ${errors} 項結構／流程錯誤需要排除。`;
    return;
  }
  target.innerHTML=Boolean($('dryRunBtn')&&!$('dryRunBtn').disabled)
    ? '<b>✓ 已可執行 Dry Run</b>：本次 payload 已與主更新中心同步。'
    : '<b>等待主更新中心重新驗證</b>：若狀態未立即更新，請重新按「檢查結構」。';
}
...
function reviewPayload() {
  ...
  return {
    ...loadedPayload,
    update_id:`REVIEW-ONLY-${loadedPayload.update_id||'NO-UPDATE-ID'}`,
...
function refreshReviewPreview() {
  ...
  try {
    ...
    debugTrace.record('update_center','human_diff_review_ready',{status:'completed',details:{source_update_id:loadedPayload?.update_id||null,review_update_id:payload.update_id,operation_count:reviewPreview.operation_count,conflict_count:reviewPreview.conflict_count,applied_update_id:appliedUpdateId}});
  } catch (error) {
    debugTrace.record('update_center','human_diff_review_unavailable',{status:'blocked',details:{source_update_id:loadedPayload?.update_id||null,reason:error.message}});
  }
}
...
function pokemonVisibleDiffCount(change){
  const audit=(change?.field_audit||[]).filter(a=>!META_FIELDS.has(a.field));
  const regular=audit.filter(a=>!SLEEP_FIELDS.has(a.field)&&!same(a.existing,a.effective)).length;
  const sleepChanged=audit.some(a=>SLEEP_FIELDS.has(a.field)&&!same(a.existing,a.effective));
  return regular+(sleepChanged?1:0);
}
function visibleDiffCount(changes){
  let count=0;
  for(const change of changes){
    if(change.entity==='pokemon') count += pokemonVisibleDiffCount(change);
...
function packageDiffStats(){
  const ids=new Set((reviewPreview?.changes||[]).filter(c=>c.entity==='pokemon').map(pokemonIdOf).filter(Boolean));
  let changedPokemon=0,visibleFields=0,evidence=0;
  ids.forEach(id=>{const changes=changesForPokemon(id),count=visibleDiffCount(changes);if(count)changedPokemon+=1;visibleFields+=count;evidence+=evidenceDiffCount(changes);});
  return {pokemonCount:ids.size,changedPokemon,visibleFields,evidence};
}
...
function abilityGrid(change){
  ...
  return `<div class="review-detail-grid">${DETAIL_FIELDS.map(field=>{
    const oldValue=semanticValue(before,field),newValue=semanticValue(after,field),changed=!same(oldValue,newValue);
    return `<div class="review-detail-card ${changed?'changed':'unchanged'}"><b>${esc(fieldLabel(field))}</b><div class="review-value-current">${htmlValue(field,newValue)}</div>${changed?`<div class="review-value-diff"><span class="before">${htmlValue(field,oldValue)}</span><span class="arrow">→</span><span class="after">${htmlValue(field,newValue)}</span></div>`:'<small>未變更</small>'}</div>`;
  }).join('')}</div>`;
}
...
function semanticPokemonPreview(pokemonId,pokemonLabel){
  ...
  if(!reviewPreview || !changes.length) return '<p class="profile-preview-empty">暫時無法建立資料庫差異預覽；請核對 JSON 身份欄位。</p>';
...
async function synchronizeCanonicalPayload(reason='confirmation_change') {
  ...
  debugTrace.record('update_center','canonical_payload_rebuilt',{status:'completed',details:{reason,update_id:loadedPayload.update_id||null,confirmation_count:confirmations.length,confirmed_count:confirmedCount}});
  try{
    ...
    refreshAppliedState();
    if(appliedUpdateId && $('dryRunBtn')) $('dryRunBtn').disabled=true;
    debugTrace.record('update_center','main_state_payload_reloaded',{status:'completed',details:{reason,update_id:loadedPayload.update_id||null,confirmation_count:confirmations.length,confirmed_count:confirmedCount,applied_update_id:appliedUpdateId}});
    debugTrace.record('update_center','workflow_validation_completed',{status:'completed',details:{reason,error_count:workflowErrorCount(),dry_run_enabled:Boolean($('dryRunBtn')&&!$('dryRunBtn').disabled),applied_update_id:appliedUpdateId}});
    traceDryRunEligibility('canonical_handshake');
    return true;
  }catch(error){ debugTrace.record('update_center','canonical_payload_handshake_failed',{status:'failed',details:{reason,update_id:loadedPayload?.update_id||null},error}); throw error; }
...
function renderConfirmations(){
  ...
  if(!items.length){
    ...
    target.innerHTML='<p>此更新包沒有「目前未顯示」槽位需要人工確認。</p>';
    enforceDryRunGate();
    return;
  }
  ...
  target.innerHTML=`<p class="notice update-review-instruction">以下不是 JSON 原始欄位，而是「目前 SQLite → 匯入後」的人類可讀能力預覽。紅／綠差異才是本次會改變的玩家資料；「一起睡覺的時間」只算一個語意欄位。</p><div class="profile-confirmation-groups">${[...groups.entries()].map(([key,entries])=>{
  ...
  target.querySelectorAll('[data-profile-confirmation]').forEach(checkbox=>checkbox.addEventListener('change',async()=>{
    ...
    debugTrace.record('update_center','profile_confirmation_checkbox_changed',{status:'completed',details:{index,confirmed:checkbox.checked,update_id:loadedPayload.update_id||null}});
  ...
  $('acceptProfileAuditBtn')?.addEventListener('click',async()=>{
    ...
    debugTrace.record('update_center','profile_audit_confirmed',{status:'completed',details:{update_id:loadedPayload.update_id||null,confirmation_count:items.length,empty_slots_preserved:true}});
...
function bind(){
  ...
  $('jsonFile')?.addEventListener('change',async event=>{
    ...
    try{
      loadedPayload=JSON.parse(await file.text()); loadedFileName=file.name; refreshAppliedState(); refreshReviewPreview();
      debugTrace.record('update_center','json_file_loaded',{status:'completed',details:{update_id:loadedPayload.update_id||null,applied_update_id:appliedUpdateId,confirmation_count:Array.isArray(loadedPayload?.profile_audit_confirmations)?loadedPayload.profile_audit_confirmations.length:0}});
      renderConfirmations();
      if(reviewPreview) renderAudit(reviewPreview);
      else $('fieldAuditSummary').textContent='JSON 已載入，但唯讀差異預覽尚未建立；請檢查身份欄位。';
      $('fieldAuditTable').innerHTML='';
      queueMicrotask(()=>{enforceDryRunGate();traceDryRunEligibility('json_file_loaded');});
    }
    catch{ loadedPayload=null; reviewPreview=null; appliedUpdateId=false; }
  });
  $('dryRunBtn')?.addEventListener('click',()=>{
    enforceDryRunGate();
    const blocked=Boolean($('dryRunBtn')?.disabled); debugTrace.record('update_center',blocked?'dry_run_blocked':'dry_run_started',{status:blocked?'blocked':'started',details:{update_id:loadedPayload?.update_id||null,workflow_error_count:workflowErrorCount(),applied_update_id:appliedUpdateId}});
    setTimeout(()=>{
      if(!loadedPayload)return;
      refreshReviewPreview();
      if(reviewPreview){renderAudit(reviewPreview);renderConfirmations();debugTrace.record('update_center','dry_run_review_synced',{status:'completed',details:{source_update_id:loadedPayload.update_id||null,operation_count:reviewPreview.operation_count,conflict_count:reviewPreview.conflict_count,formal_dry_run_blocked:blocked}});}
      else debugTrace.record('update_center','dry_run_blocked',{status:'blocked',details:{reason:'review_preview_unavailable',update_id:loadedPayload.update_id||null,workflow_error_count:workflowErrorCount()}});
    },0);
  },true);
}

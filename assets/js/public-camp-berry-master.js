export const PUBLIC_CAMP_BERRY_VERSION='public-camp-berry-2026-08-10-a';

const source=(source_name,source_ref)=>Object.freeze({
  source_type:'verified_public_reference',source_name,source_ref,verified_at:'2026-08-10',data_version:PUBLIC_CAMP_BERRY_VERSION,
});
const FIXED=source('Serebii Pokémon Sleep research-area reference','research-area-favourite-berry-verified-2026-08-10');
...
export const PUBLIC_CAMP_BERRY_MASTER=Object.freeze([
  ...
  Object.freeze({camp_name:'黃金舊發電廠',berry_policy:'FIXED_3',favorite_berries:Object.freeze(['葡萄果','墨莓果','靛莓果']),main_berry_pool:Object.freeze([]),sub_berry_pool_policy:null,...FIXED}),
...
export function resolveCampFavoriteBerries(campName,observed=[]){
  ...
  const clean=[...new Set((Array.isArray(observed)?observed:[]).map(value=>String(value??'').normalize('NFKC').trim()).filter(Boolean))].slice(0,3);

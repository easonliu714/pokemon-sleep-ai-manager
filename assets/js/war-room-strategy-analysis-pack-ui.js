...
export const STRATEGY_ANALYSIS_PACK_UI_VERSION='strategy-analysis-pack-ui-2026-08-10-a';
...
function ensurePanel(){
  ...
  panel.innerHTML=`<h3>外部 AI Strategy Analysis Pack</h3>
    <p class="notice">由本機 deterministic 資料產生 provider-neutral 分析包。資源數量以目前 SQLite 為準；Pokémon stable local ID 會改成 <code>cand_XXX</code>；不包含 API Key、raw SQLite、原始截圖或完整 OCR。外部 AI 回覆只作策略建議，沒有直接 Apply 路徑。</p>
    ...
    <details><summary>分享資料摘要</summary><pre id="strategyAnalysisPreview" style="white-space:pre-wrap;max-height:22rem;overflow:auto"></pre></details>`;
  host.appendChild(panel);bind(panel);return panel;
}
function setButtons(enabled){for(const id of ['strategyAnalysisCopyBtn','strategyAnalysisJsonBtn','strategyAnalysisMdBtn'])if(el(id))el(id).disabled=!enabled;const share=el('strategyAnalysisShareBtn');if(share)share.disabled=!enabled||typeof navigator.share!=='function';}
function render(){
...
async function share(){
  ...
  try{
    const files=[new File([JSON.stringify(currentResult.pack,null,2)],'pokemon_sleep_strategy_analysis_pack.json',{type:'application/json'}),new File([currentResult.markdown],'pokemon_sleep_strategy_analysis_pack.md',{type:'text/markdown'})];
    if(navigator.canShare?.({files}))await navigator.share({title:'Pokémon Sleep Strategy Analysis Pack',text:'可供外部 AI 分析的本機可信資源包',files});
...
function invalidate(){if(!currentResult)return;stale=true;render();}
function install(){ensurePanel();document.addEventListener('click',event=>{if(event.target.closest?.('[data-view="warroom"]'))setTimeout(()=>{ensurePanel();render();},0);},true);globalThis.addEventListener?.('pokemon-sleep:data-changed',invalidate);document.addEventListener('pokemon-sleep-data-refreshed',invalidate);render();}

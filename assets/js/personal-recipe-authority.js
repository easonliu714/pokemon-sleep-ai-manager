export const PERSONAL_RECIPE_AUTHORITY_VERSION='g51-personal-recipe-authority-2026-09-26-a';
...
export function assertPlayerOwnedRecipe(row,{publicRecipeIds=[]}={}){
  const recipeId=text(row?.recipe_id);
  if(!recipeId)throw new Error('recipe_id is required');
  const publicIds=new Set((publicRecipeIds||[]).map(String));
  if(publicIds.has(recipeId))throw new Error('public recipe authority is read-only from personal recipe CRUD');
  if(row?.source&&text(row.source)!==PLAYER_RECIPE_SOURCE)throw new Error('recipe is not player-owned');
  return true;
}

export function buildPersonalRecipeMutationPlan({action,draft,before=null,publicRecipeIds=[]}={}){
  ...
  assertPlayerOwnedRecipe(candidate,{publicRecipeIds});

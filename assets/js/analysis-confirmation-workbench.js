...
function ensureRoot(){let root=document.getElementById('analysisConfirmationWorkbench');if(root){root.dataset.analysisConfirmationReady='true';return root;}const updates=document.getElementById('updates');if(!updates)return null;const heading=document.createElement('h3');heading.textContent='AI／OCR 結果確認';heading.id='analysisConfirmationHeading';root=document.createElement('section');root.id='analysisConfirmationWorkbench';root.className='panel';root.dataset.analysisConfirmationReady='true';const anchor=document.getElementById('updateCenterDynamicContent');anchor?.insertAdjacentElement('afterend',heading);heading.insertAdjacentElement('afterend',root);return root;}

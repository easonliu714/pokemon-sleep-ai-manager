...
function ensureRoot(){let root=document.getElementById('analysisConfirmationWorkbench');if(root)return root;const updates=document.getElementById('updates');if(!updates)return null;const mount=document.getElementById('updateCenterAnalysisMount')||document.getElementById('updateCenterDynamicContent')||updates;root=document.createElement('section');root.id='analysisConfirmationWorkbench';root.dataset.pageOwner='updates-analysis';mount.appendChild(root);const shell=document.getElementById('updateCenterAnalysisStaticShell');if(shell){shell.dataset.hydrationState='ready';shell.classList.remove('loading-placeholder');}return root;}

...
export function buildAiConsentQueue(items,{selectedIds=[],model='gemini-3.6-flash',projectAlias=null}={}){
  ...
  const queue=eligible.filter(item=>selected.has(itemId(item))).map(item=>({
    item_id:itemId(item),
    ...
    manual_duplicate_override:duplicateCandidate(item),
...
export function recordAiConsentTrace(queue,{confirmed=false}={}){
  globalThis.DebugTrace?.record?.('ai_review','ai_review_consent_prepared',{status:confirmed?'completed':'blocked',details:{selected_count:queue?.selected_count||0,manual_duplicate_count:queue?.manual_duplicate_count||0,model:queue?.model||null,project_alias:queue?.project_alias||null,contains_api_key:false}});
}

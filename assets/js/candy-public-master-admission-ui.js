...
import {begin,commit,isDatabaseReady,isRescueReadonly,persist,rollback,run} from './database.js';
import {replayCandyRecognitionAgainstCurrentMaster} from './candy-quantity-confirmation-authority.js';

export const CANDY_PUBLIC_MASTER_ADMISSION_UI_VERSION='candy-public-master-admission-ui-2026-09-01-b';
...
async function persistAdmissionRow(row){
  if(!isDatabaseReady()||isRescueReadonly())throw new Error('玩家 SQLite 尚未處於可寫狀態；Public Candy identity 尚不可建立');
  begin();
  try{
...
async function directAdmit(card){
  try{
    ...
    await persistAdmissionRow(admission.row);
    ...
    if(!resolved||resolved.status!=='MATCHED'||clean(resolved.canonical_name)!==clean(observation.observed_text))throw new Error('Local admission 已建立，但 exact replay 未形成同名 MATCH；已停止，不會寫玩家 quantity');
    ...
    alert(`Public Candy identity 已${admission.status==='CREATED'?'建立':'存在'}並儲存：${admission.row.candy_name}\n\n同一筆 observation 已重新對應。請接著核對遊戲畫面並確認 quantity；Gemini Raw JSON 不會被改寫。`);
  }catch(error){
...
function wire(){
  ...
  section.querySelectorAll('[data-kind="identity"]').forEach(card=>{
    ...
    button.title='一次確認後建立並儲存本機 Public Candy identity，立即重播同一筆 observation；不寫入玩家 quantity。';

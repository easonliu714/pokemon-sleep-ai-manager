export const STRATEGY_ANALYSIS_PACK_VERSION='strategy-analysis-pack-2026-08-10-a';
export const STRATEGY_ANALYSIS_PROMPT_VERSION='strategy-analysis-prompt-2026-08-10-a';
...
export function buildEphemeralCandidateResolver(candidates=[]){
  ...
  return Object.freeze({
    ...
    ref_to_stable:refToStable,
    count:sorted.length,
  });
}

function sanitizeCandidate(row,ref){
  return Object.freeze({
    candidate_ref:ref,
    ...
    missing_inputs:Object.freeze(unique(row?.missing_inputs)),
    failed_constraints:Object.freeze(unique(row?.failed_constraints)),
    reasons:Object.freeze(unique(row?.reasons)),
  });
}
...
function sanitizeTeam(team,resolver){
  ...
  const slots=(team.slots||[]).map(slot=>{
    ...
    return Object.freeze({
      ...
      favorite_berry_match:slot?.favorite_berry_match??null,weekly_ingredient_overlap:Object.freeze(unique(slot?.weekly_ingredient_overlap)),
      reasons:Object.freeze(unique(slot?.reasons)),
    });
  });
  return Object.freeze({
    team_status:text(team.team_status)||null,
    slots:Object.freeze(slots),
    satisfied_constraints:Object.freeze(unique(team.satisfied_constraints)),
    missing_constraints:Object.freeze(unique(team.missing_constraints)),
    warnings:Object.freeze(unique(team.warnings)),
    recipe_coverage:stable(team.recipe_coverage||{}),
...
function sanitizeResources(resourceSnapshot,{recipeStrategy,recipeDiscovery,teamOptimization}={}){
  ...
  const ingredients=ingredientSource.filter(row=>ingredientNames.size===0?Boolean(row.player_record_exists):ingredientNames.has(text(row.ingredient_name))).map(row=>Object.freeze({
    ingredient_name:text(row.ingredient_name),quantity:Number(row.quantity||0),available:Number(row.available??row.quantity??0),player_record_exists:Boolean(row.player_record_exists),
  }));
  ...
  return Object.freeze({
    ...
    source_fingerprint:text(resourceSnapshot?.fingerprint)||null,
...
function sanitizeWeeklyContext(weeklyContext={}){
  return Object.freeze({
    context_id:text(weeklyContext.context_id)||null,week_start:text(weeklyContext.week_start)||null,camp:text(weeklyContext.camp)||null,dish_category:text(weeklyContext.dish_category)||null,
    favorite_berries:Object.freeze([weeklyContext.favorite_berry_1,weeklyContext.favorite_berry_2,weeklyContext.favorite_berry_3].map(text).filter(Boolean)),
...
function sanitizeGoalProfile(goalProfile){
  ...
  return Object.freeze({
    goal_profile_id:text(goalProfile.goal_profile_id)||null,profile_name:text(goalProfile.profile_name)||null,primary_goal:text(goalProfile.primary_goal)||null,
...
function sanitizeRecipeStrategy(recipeStrategy){
  return Object.freeze({
    projection_status:text(recipeStrategy?.projection_status)||null,input_fingerprint:text(recipeStrategy?.input_fingerprint)||null,
    candidates:Object.freeze((recipeStrategy?.candidates||[]).slice(0,10).map(row=>Object.freeze({
      ...
      total_ingredients:number(row.total_ingredients),requirements:Object.freeze((row.requirements||[]).map(req=>Object.freeze({
        ingredient_name:text(req.ingredient_name),required:Number(req.required||req.quantity||0),current:Number(req.current||0),strategy_shortage:Number(req.strategy_shortage||0),
      }))),
    }))),
  });
}
...
function sanitizeRecipeDiscovery(recipeDiscovery,resolver){
  ...
  return Object.freeze({
    projection_status:text(recipeDiscovery?.projection_status)||null,input_fingerprint:text(recipeDiscovery?.input_fingerprint)||null,
    summary:stable(recipeDiscovery?.summary||{}),
...
function deriveMissingRules({candidateScoring,teamOptimization,recipeDiscovery,resourceSnapshot,weeklyContext}={}){
  ...
  const dimensions=['intrinsic_score','weekly_fit_score','roster_marginal_value_score','training_roi_score'];
  for(const dimension of dimensions)if(!scoreCandidates.some(row=>number(row?.[dimension])!==null))missing.add(dimension);
...
export function buildStrategyAnalysisPack({
  ...
  const candidateRows=selected.map(row=>sanitizeCandidate(row,resolver.stable_to_ref.get(candidateStableId(row))));
  ...
  const deterministicResults=Object.freeze({
    team_optimization:Object.freeze({
      projection_status:text(teamOptimization?.projection_status)||null,input_fingerprint:text(teamOptimization?.input_fingerprint)||null,
  ...
  const base={
    ...
    privacy_manifest:Object.freeze({api_key_in_pack:false,raw_sqlite_in_pack:false,raw_screenshot_in_pack:false,raw_ocr_in_pack:false,stable_pokemon_ids_in_pack:false,private_notes_in_pack:false,ephemeral_candidate_refs:true}),
...
export function strategyAnalysisPackMarkdown(pack){
  const lines=[
    ...
    '','> 此檔案不包含 API Key、raw SQLite、原始截圖、完整 OCR 或 stable Pokémon local ID。AI 回覆只作建議，不可直接 Apply。','',
  ];
  return lines.join('\n');
}

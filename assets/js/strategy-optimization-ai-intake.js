import {normalizeOptimizationAiResponse} from './strategy-optimization-ai-contract.js';

export const STRATEGY_OPTIMIZATION_AI_INTAKE_VERSION='strategy-optimization-ai-intake-2026-08-12-a';
const text=(value,max=1200)=>String(value??'').normalize('NFKC').trim().slice(0,max);
const uniq=(values,max=40)=>[...new Set((Array.isArray(values)?values:[]).map(value=>text(value,180)).filter(Boolean))].slice(0,max);

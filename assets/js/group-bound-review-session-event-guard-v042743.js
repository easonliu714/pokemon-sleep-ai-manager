import {
  ...
  GROUP_BOUND_REVIEW_SESSION_REASON,
  installGroupBoundReviewSessionCache,
} from './group-bound-review-session-cache-v042743.js';

export const GROUP_BOUND_REVIEW_EVENT_GUARD_VERSION='v0.4.27.43-group-bound-review-event-guard-2026-08-27-a';
...
export function installGroupBoundReviewEventGuard(scope=globalThis){
  ...
  const canonicalize=(event,eventType)=>{
    ...
    }else if(eventType==='merged'&&detail.revision?.analysis_type==='ai'){
      api.model.activate(id,session?.draft||{});
    ...
    trace(scope,'v042743_review_event_canonicalized',{event_type:eventType,group_id:id,reason:detail.reason||null,status:'completed',phase:session.phase,merge_status:result?.status||null,conflict_count:session.draft?.conflicts?.length||0,capture_phase_authority:true});
  };

  // Capture-phase canonicalization runs before older bubble listeners even though this
  // successor module is installed later. DOM consumers therefore only see the Group
  // session cache, never a temporarily conflicted core draft.
  ...
  scope.addEventListener?.('pokemon-sleep:unified-analysis-stage',event=>{
    ...
    queueMicrotask(()=>{
      ...
      trace(scope,'v042743_exact_group_ai_sealed',{...progress,group_id:group.id,status:'completed',phase:sealed?.phase||seeded?.phase||null,exact_assigned_source_completion:true});

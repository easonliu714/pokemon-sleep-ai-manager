...
export const CANDY_QUANTITY_CONFIRMATION_AUTHORITY_VERSION='candy-quantity-confirmation-authority-2026-08-31-a';
...
export function validateCandyQuantityGovernedRecognition(payload,input='candies',{allowedImageRefs=[]}={}){
  ...
  const pending=pendingCandyQuantityReviews(payload);
  const baseIds=new Set((base.unresolved||[]).map(item=>item.observation_id));
  const unresolved=[...(base.unresolved||[]),...pending.filter(item=>!baseIds.has(item.observation_id))];
  return {
    ...base,
    ...
    candy_quantity_confirmation:{
      ...
      confirmed_count:(payload?.observations||[]).filter(isCandyQuantityExplicitlyConfirmed).length,
      screenshot_quantity_is_candidate_only:true,
    },
  };
}

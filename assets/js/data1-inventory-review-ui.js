import {applyInventoryReviewBatch,buildInventoryReviewPackage,filterInventoryItems,summarizeInventoryReview} from './data1-inventory-review.js';
...
const CATEGORY_OPTIONS=['unclassified','pokemon','recipe','ingredient','item','capacity','account'];
...
export function createInventoryReviewWorkbench({manifest,onChange=()=>{}}={}){
  const root=document.createElement('section');root.className='data1-review-workbench';
  let current=manifest;
  let selected=new Set();
  let filters={status:'all',category:'all',query:''};

  const render=()=>{
    const summary=summarizeInventoryReview(current);
    const items=filterInventoryItems(current,filters);
    root.innerHTML=`
      <h4>私人清點覆核工作台</h4>
      <div class="data1-review-summary">
        <div><strong>總數</strong><br>${summary.total}</div><div><strong>已覆核</strong><br>${summary.reviewed}</div><div><strong>待處理</strong><br>${summary.pending}</div><div><strong>進度</strong><br>${summary.progress_percent}%</div>
      </div>
      ...
      <div id="data1ReviewFeedback" role="status" aria-live="polite"></div>`;
    ...
    root.querySelector('#data1ReviewQuery').addEventListener('input',event=>{filters.query=event.target.value;render();});

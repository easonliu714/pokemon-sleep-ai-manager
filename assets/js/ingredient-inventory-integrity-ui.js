...
function establishedRows(){
  try{return rows(`SELECT ingredient_name,quantity,unlocked,unlock_state,player_record_exists FROM ingredient_catalog_state WHERE player_record_exists=1 ORDER BY ingredient_name`);}catch{return [];}
}
...
function ensureStyle(){
  ...
  style.textContent=`.${REVIEW_CLASS}{margin:10px 0;padding:12px;border:1px solid #e3c87a;border-radius:10px;background:#fffaf0}.ingredient-absence-card{padding:10px;margin-top:8px;border:1px solid #ead9a4;border-radius:9px;background:#fff}.ingredient-absence-actions{display:flex;gap:7px;flex-wrap:wrap;margin-top:8px}.ingredient-absence-actions button{flex:1 1 150px}.ingredient-absence-resolved{color:#1f7a5a;font-weight:750}.ingredient-absence-blocked{color:#8a5b00;font-weight:700}.ingredient-integrity-badge{display:inline-block;margin-left:6px;font-size:.75rem;color:#687d74}.ingredient-absence-semantics{font-size:.86rem;color:#5c6d65;margin-top:4px}@media(max-width:700px){.${REVIEW_CLASS}{padding:10px}.ingredient-absence-actions{display:grid;grid-template-columns:1fr}.ingredient-absence-actions button{width:100%}}`;
  document.head.appendChild(style);
}
function resolutionLabel(value){
  if(value==='CONFIRMED_EXHAUSTED')return '已確認用罄 → quantity=0，維持已解鎖';
  if(value==='PRESERVE_EXISTING_NOT_CAPTURED')return '照片未拍到 → 保留既有值';
  if(value==='CONFIRMED_NOT_UNLOCKED')return '已確認尚未解鎖';
  if(value==='CONFIRMED_UNLOCKED_ZERO')return '已確認已解鎖，目前庫存為 0';
  if(value==='PRESERVE_UNLOCK_UNKNOWN')return '本次無法判斷 → 保留待確認';
  return '待確認';
}
function actionButtons(item){
  if(item.review_kind==='UNLOCK_STATE_REVIEW')return `<div class="ingredient-absence-actions"><button data-resolution="CONFIRMED_NOT_UNLOCKED">尚未解鎖</button><button data-resolution="CONFIRMED_UNLOCKED_ZERO">已解鎖，目前為 0</button><button data-resolution="PRESERVE_UNLOCK_UNKNOWN">本次無法確認 → 保留待確認</button></div>`;
  return `<div class="ingredient-absence-actions"><button data-resolution="CONFIRMED_EXHAUSTED">已用罄 → 更新為 0</button><button data-resolution="PRESERVE_EXISTING_NOT_CAPTURED">照片沒拍到 → 保留 ${item.previous_quantity}</button><button data-resolution="AI_REVIEW">AI 漏判／辨識錯誤</button></div>`;
}
function render(){
...
function commitResolution(panel,state,item,resolution){
  ...
  if(resolution==='CONFIRMED_EXHAUSTED'&&!confirm(`確認「${item.ingredient_name}」已解鎖但目前用罄？系統將以明確 quantity=0 覆蓋既有 ${item.previous_quantity}，並維持 unlocked=1。`))return;
  ...
  if(resolution==='CONFIRMED_NOT_UNLOCKED'&&!confirm(`確認「${item.ingredient_name}」目前尚未解鎖？系統只會記錄 unlocked=0，不會把圖片缺席偽造成 quantity=0 觀測。`))return;
  if(resolution==='CONFIRMED_UNLOCKED_ZERO'&&!confirm(`確認「${item.ingredient_name}」已解鎖，而且目前庫存確實為 0？系統會記錄 unlocked=1，既有 quantity=0 保持不變。`))return;
  if(resolution==='PRESERVE_UNLOCK_UNKNOWN'&&!confirm(`本次先不判定「${item.ingredient_name}」是否解鎖？系統會保留 UNKNOWN；本次可繼續套用其他已確認庫存，未來完整庫存仍可能再次詢問。`))return;
  const refs=[...new Set([
    ...imageRefs(),

...
const EXECUTOR_SCHEMA='pokemon-sleep-ai-review-executor/1.0';
const PROMPT_VERSION='pokemon-sleep-image-review/1.0';
...
export async function executeAiReviewQueue({queue,inventory,poolData,resolveImage,onProgress=()=>{},onTrace=()=>{}}={}){
  ...
  for(let index=0;index<queue.items.length;index++){
    const queued=queue.items[index];const id=itemId(queued);const source=(inventory?.items||[]).find(item=>itemId(item)===id)||queued;const model=poolData.model||queue.model;const cacheKey=await hashCacheKey({sha256:source.sha256||id,model,promptVersion:PROMPT_VERSION});const cached=readAiResultCache(cacheKey);
    ...
    const result={item_id:id,source_image_ref:itemPath(source),model,project_alias:outcome.used_alias,analysis:extractJson(outcome.payload),completed_at:new Date().toISOString(),prompt_version:PROMPT_VERSION};writeAiResultCache(cacheKey,result);results.push(result);onProgress({current:index+1,total:queue.items.length,cached:false,project_alias:outcome.used_alias});
  }
  return {schema:EXECUTOR_SCHEMA,status:'completed',completed:results.length,total:queue.items.length,results,projects,prompt_version:PROMPT_VERSION};
}

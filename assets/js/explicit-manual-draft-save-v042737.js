...
export function revisionAuthorityFromDetail(detail={}){
  ...
  return {group_id:text(detail?.group_id)||null,analysis_id:text(revision?.analysis_id)||null,revision_no:revision?.revision_no==null?null:Number(revision.revision_no),source_image_ref:text(revision?.source_image_ref)||null};
}
...
export function revisionAuthorityMatches(expected={},state={}){
  ...
  if(expectedAnalysis!==actualAnalysis||expectedRevision!==actualRevision)return {ok:false,status:'STALE_REVISION',expected_analysis_id:expectedAnalysis,actual_analysis_id:actualAnalysis||null,expected_revision_no:expectedRevision,actual_revision_no:actualRevision};
...
export function classifyFormSnapshot({expected={},current={},touched_keys=[]}={}){
  ...
  return {clean:manual.length===0&&unauthorized.length===0,manual_dirty:manual.length>0,unauthorized_drift:unauthorized.length>0,manual_keys:manual,unauthorized_keys:unauthorized};
}

export function confirmationActionPolicy({action='',classification={}}={}){
  const manualDirty=Boolean(classification?.manual_dirty),systemDrift=Boolean(classification?.unauthorized_drift);
  return {allowed:!manualDirty,block_reason:manualDirty?'UNSAVED_MANUAL_CHANGES':null,discard_system_drift:systemDrift,prepare_authoritative_apply:action==='applyConfirmedAnalysis'&&systemDrift&&!manualDirty};
}
...
export function snapshotForm(root,draft={}){
  ...
  for(const node of root?.querySelectorAll?.('[data-field]')||[]){const key=nodeKey(node);if(!key)continue;expected[key]=expectedForField(draft,node.dataset.field,node);current[key]=nodeValue(node);}
  for(const node of root?.querySelectorAll?.('[data-check]')||[]){const key=nodeKey(node);if(!key)continue;expected[key]=expectedForCheck(draft,node.dataset.check);current[key]=Boolean(node.checked);}
  return {expected,current};
}

export function projectAuthoritativeForm(root,draft={}){
  const projected=[];
  for(const node of root?.querySelectorAll?.('[data-field]')||[]){const key=nodeKey(node);if(!key)continue;const expected=expectedForField(draft,node.dataset.field,node);if(!same(nodeValue(node),expected)){setNodeValue(node,expected);projected.push(key);}}
  for(const node of root?.querySelectorAll?.('[data-check]')||[]){const key=nodeKey(node);if(!key)continue;const expected=expectedForCheck(draft,node.dataset.check);if(Boolean(node.checked)!==Boolean(expected)){node.checked=Boolean(expected);projected.push(key);}}
  return projected;
}
...
function rebuildIngredientLevel(out,root,level){
  ...
  const name=text(root.querySelector(`[data-field="ingredient_name_${level}"]`)?.value);const qtyNode=root.querySelector(`[data-field="ingredient_qty_${level}"]`);const quantity=qtyNode?.value===''||qtyNode==null?null:Number(qtyNode.value);
  if(!name)rows.delete(Number(level));else rows.set(Number(level),{unlock_level:Number(level),ingredient_name:name,quantity:Number.isFinite(quantity)?quantity:null});
...
function rebuildSubskillLevel(out,root,level){
  ...
  const name=text(root.querySelector(`[data-field="subskill_name_${level}"]`)?.value);const unlocked=Boolean(root.querySelector(`[data-check="sub_unlock_${level}"]`)?.checked);
  if(!name)rows.delete(Number(level));else rows.set(Number(level),{unlock_level:Number(level),subskill_name:name,is_unlocked:unlocked?1:0});
...
export function applyManualPatch(rawDraft={},root,manualKeys=[]){
  const out=clone(rawDraft)||{},ingredientLevels=new Set(),subskillLevels=new Set();
  for(const key of manualKeys||[]){
    const field=scalarFieldName(key);let match=field?.match(/^ingredient_(?:name|qty)_(\d+)$/);if(match){ingredientLevels.add(Number(match[1]));continue;}
    ...
    if(!field)continue;const node=root?.querySelector?.(`[data-field="${field}"]`);if(node)out[field]=nodeValue(node);
  }
  for(const level of ingredientLevels)rebuildIngredientLevel(out,root,level);for(const level of subskillLevels)rebuildSubskillLevel(out,root,level);return out;
}

function compatibleIngredient(a,b){
...
function compatibleSubskill(a,b){return text(a?.subskill_name)===text(b?.subskill_name);}
export function mergeRowsByUnlockLevel(collections=[],kind='ingredients'){
  const slots=new Map(),conflicted=new Map();
  const compatible=kind==='subskills'?compatibleSubskill:compatibleIngredient;
  for(const rows of collections||[]){
    for(const raw of Array.isArray(rows)?rows:[]){
      const level=Number(raw?.unlock_level);if(!Number.isFinite(level)||!text(kind==='subskills'?raw?.subskill_name:raw?.ingredient_name))continue;
      const row=clone(raw);
      if(conflicted.has(level)){const list=conflicted.get(level);if(!list.some(v=>same(v,row)))list.push(row);continue;}
      const existing=slots.get(level);
      if(!existing){slots.set(level,row);continue;}
      if(!compatible(existing,row)){slots.delete(level);conflicted.set(level,[existing,row]);continue;}
      if(kind==='ingredients'&&existing.quantity==null&&row.quantity!=null)existing.quantity=row.quantity;
      if(kind==='subskills')existing.is_unlocked=(Number(existing.is_unlocked)||Number(row.is_unlocked))?1:0;
    }
  }
  return {rows:[...slots.values()].sort((a,b)=>Number(a.unlock_level)-Number(b.unlock_level)),conflicts:[...conflicted.entries()].map(([unlock_level,candidates])=>({field:`${kind}@${unlock_level}`,unlock_level,status:'REVIEW_REQUIRED_CROSS_IMAGE_CONFLICT',candidates}))};
}

export function repairPartialCollectionsFromRevisions(revisions=[],normalizeRevision=value=>value,draft={}){
  const normalized=(revisions||[]).map(revision=>normalizeRevision(revision)).filter(Boolean);
  const ingredients=mergeRowsByUnlockLevel(normalized.map(row=>row.ingredients||[]),'ingredients');
  const subskills=mergeRowsByUnlockLevel(normalized.map(row=>row.subskills||[]),'subskills');
  const out=clone(draft)||{};
  if(normalized.some(row=>Array.isArray(row.ingredients)&&row.ingredients.length))out.ingredients=ingredients.rows;
  if(normalized.some(row=>Array.isArray(row.subskills)&&row.subskills.length))out.subskills=subskills.rows;
  out.v042740_partial_collection_conflicts=[...ingredients.conflicts,...subskills.conflicts];
  return out;
}

export function shouldRestoreReviewFocus(beforeActiveGroupId,afterActiveGroupId,state={}){
  const before=text(beforeActiveGroupId),after=text(afterActiveGroupId);
  if(!before||!after||before===after)return false;
  const group=(state?.groups||[]).find(row=>text(row?.id)===before);
  return Boolean(group&&group.status!=='closed');
}

export function rebindVisibleAuthorityForSave(consistency,expected={}){
  if(!consistency||typeof consistency.getState!=='function')return {ok:false,status:'CONSISTENCY_NOT_READY'};
  const state=consistency.getState(),group=(state?.groups||[]).find(row=>text(row?.id)===text(expected?.group_id));
  if(!group)return {ok:false,status:'GROUP_NOT_FOUND'};
  const latest=group.latest_revision||{};
  if(text(latest.analysis_id)!==text(expected?.analysis_id)||Number(latest.revision_no)!==Number(expected?.revision_no))return {ok:false,status:'STALE_REVISION'};
  const er=text(expected?.source_image_ref),ar=text(latest?.source_image_ref);
  if(er&&ar&&er!==ar)return {ok:false,status:'SOURCE_IMAGE_MISMATCH'};
  if(text(state.active_group_id)!==text(expected.group_id))consistency.selectGroup?.(expected.group_id,{reason:'v042740_explicit_save_rebind_visible_authority'});
...
export function installExplicitManualDraftSave(scope=globalThis){
  ...
  const setStatus=(message,kind='')=>{lastStatus={message,kind};const node=statusNode();if(node){node.className=`notice${kind?` ${kind}`:''}`;node.textContent=message;}};
  const findGroup=(groupId,state=consistency.getState())=>(state?.groups||[]).find(row=>text(row?.id)===text(groupId))||null;
  const repairDraft=(detail={})=>{
    const state=consistency.getState(),group=findGroup(detail.group_id,state),base=detail?.draft?clone(detail.draft):clone(group?.draft||{});
    if(!group||typeof consistency.normalizeRevision!=='function')return base;
    const repaired=repairPartialCollectionsFromRevisions(group.revisions||[],consistency.normalizeRevision,base);
    const count=repaired.v042740_partial_collection_conflicts?.length||0;
    if(count)trace(scope,'v042740_partial_collection_conflicts_retained',{group_id:group.id,conflict_count:count,status:'review_required'});
    return repaired;
  };
  const currentClassification=()=>{const currentRoot=root();if(!currentRoot||!authoritativeDraft)return {clean:true,manual_dirty:false,unauthorized_drift:false,manual_keys:[],unauthorized_keys:[]};const snapshots=snapshotForm(currentRoot,authoritativeDraft);return classifyFormSnapshot({...snapshots,touched_keys:[...touched]});};
  const traceSystemDrift=state=>{if(!state.unauthorized_drift)return;const signature=state.unauthorized_keys.join('|');if(signature===lastDriftSignature)return;lastDriftSignature=signature;trace(scope,'v042738_system_dom_drift_isolated',{group_id:authority?.group_id||null,fields:state.unauthorized_keys,status:'isolated',write_authority:false,navigation_blocked:false});};
  const refreshDirtyStatus=()=>{const state=currentClassification();if(state.manual_dirty){setStatus('有尚未儲存的人工修改。請先儲存或還原，才能切換寶可夢或確認處置。','pending');return state;}traceSystemDrift(state);if(!lastStatus||['pending','error'].includes(lastStatus.kind))setStatus(state.unauthorized_drift?'目前沒有尚未儲存的人工修改；系統顯示差異不具寫入權，切換時會直接丟棄。':'目前沒有尚未儲存的人工修改。','');return state;};
  const mount=()=>{const area=confirmation();if(!area)return false;let panel=area.querySelector('#manualDraftAuthorityV042738');if(!panel){panel=scope.document.createElement('section');panel.id='manualDraftAuthorityV042738';panel.className='notice';panel.innerHTML='<strong>人工修改 Authority</strong><br><span>切換寶可夢不會自動保存表單。只有按「儲存人工修改」才會永久覆寫目前草稿。</span><div class="buttons"><button type="button" id="saveManualAnalysisDraftV042738">儲存人工修改</button><button type="button" id="revertManualAnalysisDraftV042738" class="secondary">還原目前草稿</button></div><div id="manualDraftStatusV042738" class="notice"></div>';const nav=area.querySelector('.analysis-review-navigation');(nav||area.querySelector('header'))?.insertAdjacentElement('afterend',panel);panel.querySelector('#saveManualAnalysisDraftV042738').onclick=saveManual;panel.querySelector('#revertManualAnalysisDraftV042738').onclick=()=>revertManual('user_revert');}if(lastStatus)setStatus(lastStatus.message,lastStatus.kind);else refreshDirtyStatus();return true;};
  const acceptDetail=(detail={},reason='selected')=>{if(isLegacyCorrectedConfirmationEvent(detail))return false;authority=revisionAuthorityFromDetail(detail);authoritativeDraft=repairDraft(detail);touched.clear();lastStatus=null;lastDriftSignature='';mount();refreshDirtyStatus();trace(scope,'v042738_manual_authority_refreshed',{reason,group_id:authority.group_id,analysis_id:authority.analysis_id,revision_no:authority.revision_no,status:'completed',partial_merge:true});return true;};
  const scheduleAuthoritativeRender=(detail={},reason='core_event')=>{const expected=revisionAuthorityFromDetail(detail),token=++projectionSequence;if(!expected.group_id||!expected.analysis_id||expected.revision_no==null)return token;queueMicrotask(()=>{if(token!==projectionSequence)return;const match=revisionAuthorityMatches(expected,consistency.getState());if(!match.ok||text(authority?.group_id)!==text(expected.group_id)){trace(scope,'v042739_authoritative_render_skipped',{reason,status:'blocked',match_status:match.status||null,expected_group_id:expected.group_id,active_group_id:consistency.getState()?.active_group_id||null});return;}const projected=projectAuthoritativeForm(root(),authoritativeDraft||{});trace(scope,'v042739_authoritative_render_committed',{reason,status:'completed',group_id:expected.group_id,analysis_id:expected.analysis_id,revision_no:expected.revision_no,field_count:projected.length,fields:projected,legacy_dom_writer_count:0,single_confirmation_authority:true});refreshDirtyStatus();});return token;};
  function revertManual(reason='manual_revert'){if(!authority?.group_id)return false;touched.clear();lastDriftSignature='';lastStatus={message:'已還原為目前群組的已儲存草稿。',kind:'success'};consistency.selectGroup?.(authority.group_id,{reason:`${reason}_v042738`});trace(scope,'v042738_manual_draft_reverted',{group_id:authority.group_id,reason,status:'completed'});return true;}
  function saveManual(){
  ...
  const markTouched=event=>{const currentRoot=root();if(!currentRoot||!currentRoot.contains?.(event.target))return;if(event.isTrusted===false)return;const key=nodeKey(event.target);if(!key)return;touched.add(key);refreshDirtyStatus();};
  const suppressLegacyCorrectedEvent=event=>{if(!isLegacyCorrectedConfirmationEvent(event?.detail||{}))return;event.preventDefault?.();event.stopImmediatePropagation?.();event.stopPropagation?.();trace(scope,'v042739_legacy_corrected_event_suppressed',{status:'blocked',group_id:event?.detail?.group_id||null,event_type:event?.type||null,legacy_source:'v042718_form_authority_corrected',single_confirmation_authority:true});};
  const preserveReviewFocusOnRevision=()=>{const before=consistency.getState()?.active_group_id||null;if(!before)return;queueMicrotask(()=>{const state=consistency.getState(),after=state?.active_group_id||null;if(!shouldRestoreReviewFocus(before,after,state))return;consistency.selectGroup?.(before,{reason:'v042740_background_revision_preserve_review_focus'});trace(scope,'v042740_background_revision_focus_restored',{status:'completed',preserved_group_id:before,background_group_id:after});});};

  if(scope.document){
    scope.addEventListener?.('pokemon-sleep:analysis-revision-saved',preserveReviewFocusOnRevision,true);
    ...
    scope.addEventListener?.('pokemon-sleep:analysis-confirmation-group-selected',event=>{const detail=event.detail||{};if(isLegacyCorrectedConfirmationEvent(detail))return;if(acceptDetail(detail,'group_selected'))scheduleAuthoritativeRender(detail,'group_selected');});
    scope.addEventListener?.('pokemon-sleep:analysis-confirmation-merged',event=>{const detail=event.detail||{};if(isLegacyCorrectedConfirmationEvent(detail))return;if(authority?.group_id&&text(detail.group_id)!==text(authority.group_id))return;const hadDirty=currentClassification().manual_dirty;if(acceptDetail(detail,'revision_merged'))scheduleAuthoritativeRender(detail,'revision_merged');if(hadDirty)setStatus('分析 revision 已更新；先前未儲存的人工修改沒有寫回，請重新覆核。','pending');});

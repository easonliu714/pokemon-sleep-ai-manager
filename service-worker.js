const CACHE = 'pokemon-sleep-ai-v0.3.40-data1b-inventory-review';
const ASSETS = [
  ...
  './assets/js/identity-import-wizard.js','./assets/js/identity-import-pipeline.js','./assets/js/pokemon-screenshot-grouping.js','./assets/js/pokemon-zip-manifest.js','./assets/js/pokemon-zip-adapter.js','./assets/js/data1-zip-inventory.js','./assets/js/data1-inventory-review.js','./assets/js/data1-inventory-review-ui.js','./assets/js/jszip-loader.js','./assets/js/android-import-file-picker.js','./assets/js/screenshot-observation-bridge.js','./assets/js/identity-import-apply-operation.js','./assets/js/identity-import-transaction.js','./assets/js/identity-import-wizard-entry.js',
...
self.addEventListener('fetch', (event) => {
  ...
  if (networkFirst) {
    event.respondWith(fetch(event.request, {cache: 'no-store'}).then((response) => {const copy = response.clone();caches.open(CACHE).then((cache) => cache.put(event.request, copy));return response;}).catch(() => caches.match(event.request).then((hit) => hit || caches.match('./index.html'))));
    return;
  }

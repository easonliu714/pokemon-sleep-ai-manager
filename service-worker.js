...
const APP_BUILD = '20260806-v0393-post-migration-startup-isolation';
const CACHE = 'pokemon-sleep-ai-v0.3.93-v0393-post-migration-startup-isolation';
...
const ASSETS = [
  ...
  './assets/js/bootstrap.js','./assets/js/runtime-version.js','./assets/js/debug-trace-manager.js','./assets/js/app.js','./assets/js/database.js','./assets/js/storage.js','./assets/js/sqlite-load-worker.js','./assets/js/v0390-worker-load-control.js','./assets/js/backup-truth-restore.js','./assets/js/data-consistency-multicapture.js','./assets/js/public-catalog-workbench.js','./assets/js/v0389-rescue-catalog-import.js',
...
self.addEventListener('activate',(event)=>{
  event.waitUntil(
    caches.keys()
      ...
      .then((clients)=>Promise.all(clients.map((client)=>client.postMessage({type:'pokemon-sleep-version-activated',app_version:APP_VERSION,build:APP_BUILD}))))
  );
});
self.addEventListener('fetch',(event)=>{
  if(event.request.method!=='GET')return;
  const url=new URL(event.request.url);const sameOrigin=url.origin===self.location.origin;
  const versionAuthorityAsset=sameOrigin&&(url.pathname.endsWith('/index.html')||url.pathname.endsWith('/bootstrap.js')||url.pathname.endsWith('/service-worker.js')||url.pathname.endsWith('/v0389-rescue-catalog-import.js')||url.pathname.endsWith('/v0390-worker-load-control.js')||url.pathname.endsWith('/sqlite-load-worker.js'));
  const networkFirst=sameOrigin&&(event.request.mode==='navigate'||url.pathname.endsWith('.js')||url.pathname.endsWith('.html'));
  if(versionAuthorityAsset||networkFirst){event.respondWith(fetch(event.request,{cache:'no-store'}).then((response)=>{const copy=response.clone();caches.open(CACHE).then((cache)=>cache.put(event.request,copy));return response;}).catch(()=>caches.match(event.request).then((hit)=>hit||caches.match('./index.html'))));return;}

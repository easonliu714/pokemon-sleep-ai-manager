...
assert.match(live,/pokemon-sleep-update-center-live-debug\/1\.3/);assert.match(live,/除錯紀錄已匯出/);assert.match(live,/live_debug_exported/);assert.match(live,/document\.body\.appendChild\(link\)/);assert.match(live,/DISPLAY_ENTRIES=8/);assert.match(live,/RENDER_DELAY_MS=1500/);assert.match(live,/function omitted/);assert.doesNotMatch(live,/takeOverSingleItemAdvancedReview/);
...
console.log(JSON.stringify({ok:true,gate:'G13.2G lightweight AI review and export feedback compatibility',version:'v0.3.70',compatibility:'bounded-deferred-live-debug'}));

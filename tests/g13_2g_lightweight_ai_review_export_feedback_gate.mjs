...
assert.ok(wizard.indexOf("mode:'lightweight'")<wizard.indexOf('createOcrRegionAiReviewPanel({inventory:subset'),'lightweight list must precede advanced panel');
assert.doesNotMatch(wizard,/slot\.replaceChildren\(createOcrRegionAiReviewPanel\(\{inventory:current\.fileResult\.inventory/);
assert.match(live,/pokemon-sleep-update-center-live-debug\/1\.2/);

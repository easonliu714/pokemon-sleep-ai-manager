...
assert.match(picker,/image_fingerprint_progress/);assert.match(picker,/recordProgress/);assert.match(picker,/ocr_started/);assert.match(picker,/ocr_total/);assert.match(picker,/dispatchBatch\('completed'/);assert.match(picker,/PokemonSleepOCR\?\.cancel/);assert.match(picker,/contains|SHA-256|統一/);
...
console.log(JSON.stringify({ok:true,gate:'G13.2C compatibility',version:'v0.3.75'}));
...
const hotfix=fs.readFileSync(files[0],'utf8'),shell=fs.readFileSync(files[1],'utf8'),index=fs.readFileSync(files[2],'utf8'),worker=fs.readFileSync(files[3],'utf8'),bootstrap=fs.readFileSync('assets/js/bootstrap.js','utf8');
...
const eager=/data1d1-ocr-region-direct-minimal-hotfix\.js/.test(index);
const pageAware=/updates:Object\.freeze\(\[[\s\S]*'data1d1-ocr-region-direct-minimal-hotfix\.js'/.test(bootstrap);
assert.ok(eager||pageAware,'direct minimal OCR must remain reachable via eager or page-aware Update Center authority');
if(pageAware){assert.match(bootstrap,/single_flight:true/);assert.match(bootstrap,/global_deferred_sweep:false/);}

...
requireTokens(source.diagnostic,['真實 OCR 單張覆核','loadSelectedOcrOnlyReview','ocr_only_review_started','ocr_only_review_completed','forced_ocr_started','forced_ocr_completed','單張圖片 AI 分析','standalone_single_image_ai_started','standalone_single_image_ai_completed','createSingleItemOcrRegionAiReviewPanel','executePreparedAiPayload','accept="image/png,image/jpeg,image/webp,image/avif"'],'diagnostic');
assert.doesNotMatch(source.diagnostic,/buildAiConsentQueue|validateAiConsent/,'diagnostic_must_use_real_executor_not_legacy_queue_builder');
requireTokens(source.hotfix,["executePreparedAiPayload","HOTFIX_VERSION='v0.3.72'","ultra_minimal_ai_hotfix_ready"],'hotfix');
requireTokens(source.shell,['ultra-minimal-ai-shell','ultra_minimal_ai_shell_completed','contains_image_bytes:false','contains_api_key:false','Promise.resolve(root)','強制執行單張 AI 分析'],'shell');
...
console.log(JSON.stringify({ok:true,gate:'G13.2M OCR vs single-image AI diagnostics',version:'v0.3.72',real_ocr:true,real_ai:true}));

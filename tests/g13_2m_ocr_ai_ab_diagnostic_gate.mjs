...
const files={
  diagnostic:'assets/js/data1d1-ocr-ai-ab-diagnostic.js',
  hotfix:'assets/js/data1d1-ocr-region-direct-minimal-hotfix.js',
  liveDebug:'assets/js/update-center-live-debug.js'
};
...
requireTokens(source.diagnostic,[
  'OCR-only 單張覆核',
  'loadSelectedOcrOnlyReview',
  'ocr_only_review_started',
  'ocr_only_review_completed',
  'ocr_only_review_saved',
  '單張圖片 AI 分析（獨立診斷）',
  'standalone_single_image_ai_started',
...
requireTokens(source.hotfix,[
  "import './data1d1-ocr-ai-ab-diagnostic.js?v=20260803-g13-2m-live-debug-deferred-render'",
  "HOTFIX_VERSION='v0.3.70'",
  "diagnostics_mode:'deferred_bounded_render'"
],'hotfix');
requireTokens(source.liveDebug,[
  "const DISPLAY_ENTRIES=8",
  "const RENDER_DELAY_MS=1500",
  "if(type==='function')return '[function omitted]'",
  "event==='identity_import_files_selected'?summarizeIdentityImport(detail):safe(detail)",
  'detail_text'
],'live_debug');
...
console.log(JSON.stringify({ok:true,gate:'G13.2M OCR vs standalone single-image AI A/B diagnostics',version:'v0.3.70'}));

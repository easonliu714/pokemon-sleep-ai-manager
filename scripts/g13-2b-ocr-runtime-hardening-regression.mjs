...
for(const token of ['PokemonSleepOCR?.cancel','正在停止 OCR','itemTimeoutMs:30000','finalize_started'])assert.ok(picker.includes(token),`picker:${token}`);
...
console.log(JSON.stringify({ok:true,gate:'G13.2B OCR runtime hardening compatibility',timeout_ms:30000,stall_ms:20000,worker_recovery:true,abort_terminate:true,runtime_monitor:true,version:'v0.3.75'}));
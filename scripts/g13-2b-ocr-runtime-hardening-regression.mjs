...
for(const token of ['DEFAULT_TIMEOUT_MS=30000','DEFAULT_STALL_MS=20000','ocr_timeout','ocr_stalled','terminateWorker','workerGeneration','ocr_runtime_retrying','async cancel','active_job'])assert.match(runtime,new RegExp(token));
for(const token of ['signal,timeoutMs','throwIfCancelled','itemTimeoutMs=DEFAULT_ITEM_TIMEOUT_MS','ocr-item-started','ocr-item-failed','setTimeout\(resolve,0\)'])assert.match(classifier,new RegExp(token));
for(const token of ['PokemonSleepOCR?.cancel','正在強制停止 OCR','itemTimeoutMs:30000','失敗'])assert.match(picker,new RegExp(token.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));
for(const token of ['OCR Runtime Monitor','exportOcrRuntimeMonitor','active_job','ocr_runtime_job_failed','pokemon_sleep_ocr_runtime_'])assert.match(monitor,new RegExp(token));

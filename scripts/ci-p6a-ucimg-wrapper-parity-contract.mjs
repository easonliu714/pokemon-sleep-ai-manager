...
export const CI_P6A_UCIMG_WRAPPER_PARITY_VERSION='ci-p6a-ucimg-wrapper-retirement-2026-08-16-d-p6b-successor-aware';
...
const ORIGINAL_SUCCESSOR='.github/workflows/uc-img-a.yml';
const CURRENT_SUCCESSOR='.github/workflows/screenshot-pipeline-regression.yml';
...
const successorPath=fs.existsSync(ORIGINAL_SUCCESSOR)?ORIGINAL_SUCCESSOR:CURRENT_SUCCESSOR;
assert.equal(fs.existsSync(successorPath),true,'P6A behavioral successor missing after later screenshot-domain convergence');
...
assert.match(successor,/push:/m,'P6A successor must preserve push coverage');
assert.ok(successor.includes("- main"),'P6A successor push coverage must include main');
assert.ok(successor.includes("- 'hotfix/**'"),'P6A successor must preserve hotfix/** coverage');
assert.match(successor,/workflow_dispatch:/m,'P6A successor must preserve manual dispatch');

if(successorPath===ORIGINAL_SUCCESSOR){
  for(const token of [
  ...
  ])assert.ok(successor.includes(token),`P6A UC.IMG trigger union lost predecessor path: ${token}`);
}else{
  assert.match(successor,/pull_request:\s*\n/m,'P6B successor deliberately widens P6A path coverage to all PRs');
}
...
assert.ok(actual.length<=P6A_SIDE_BY_SIDE_PARITY_PROOF.workflow_count_after_p6a_retirement,'later convergence must not exceed the P6A-retired workflow baseline after P6B retirement');
...
console.log(JSON.stringify({
  ...
  original_successor_workflow:'uc-img-a.yml',
  current_successor_workflow:successorPath.split('/').at(-1),

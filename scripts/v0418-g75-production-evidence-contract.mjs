...
if(slotSuccessor){assert.equal(slotRow.authority_status,'ACTIVE_VERIFIED');assert.equal(slotRow.runtime_numeric_activation,true);assert.equal(slotRow.evidence_status,EVIDENCE_STATUS.ACTIVE_VERIFIED_LOCAL_NUMERIC_CONTRACT);assert.deepEqual(slotRow.blocking_reasons,[]);}else{assert.equal(slotRow.runtime_numeric_activation,false);}

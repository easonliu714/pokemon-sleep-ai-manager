...
assert.ok(['v0.4.18','v0.4.19'].includes(appVersion),`unexpected G7.5 release/successor version ${appVersion}`);
if(appVersion==='v0.4.19')assert.ok(version.includes("// app_version: 'v0.4.18'"),'v0.4.19 must retain v0.4.18 lineage bridge');
const berryStrengthSuccessor=appVersion==='v0.4.19';

const candidateFeatures={candidates:[
  {pokemon_id:'private-a',type:'火',level:30,helper_seconds:2100,main_skill:'能量填充S',unlocked_ingredients:[{ingredient_name:'火辣香草',quantity:2}]},
  {pokemon_id:'private-b',type:'水',level:40,helper_seconds:2200,main_skill:'食材獲取S',unlocked_ingredients:[{ingredient_name:'哞哞鮮奶',quantity:2}]},
  {pokemon_id:'private-c',type:'妖精',level:50,helper_seconds:2300,main_skill:'活力全體療癒S',unlocked_ingredients:[{ingredient_name:'甜甜蜜',quantity:2}]},
]};
...
assert.equal(first.summary.active_numeric_dimension_count,berryStrengthSuccessor?1:0);
assert.equal(first.summary.blocked_numeric_dimension_count,berryStrengthSuccessor?6:7);

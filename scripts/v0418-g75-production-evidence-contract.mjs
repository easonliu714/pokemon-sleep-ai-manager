...
const ingredientEvidence=(level,ingredient)=>[
  {unlock_level:1,ingredient_name:ingredient,quantity:2},
  ...(level>=30?[{unlock_level:30,ingredient_name:ingredient,quantity:3}]:[]),
  ...(level>=60?[{unlock_level:60,ingredient_name:ingredient,quantity:4}]:[]),
];
const candidate=(pokemon_id,type,level,main_skill,favorite_berry_match,ingredient,specialty)=>({pokemon_id,type,level,helper_seconds:2200,main_skill,favorite_berry_match,specialty,unlocked_ingredients:ingredientEvidence(level,ingredient),...subskillEvidence(level)});
...
if(slotSuccessor){assert.equal(registry.rules.ingredient_slot_distribution.rule_version,'ingredient-slot-distribution-v1');assert.equal(registry.rules.ingredient_slot_distribution.runtime_numeric_activation,true);assert.equal(first.summary.ingredient_slot_distribution_resolved_candidate_count,3);}
const expectedActive=(baseOutputSuccessor?3:favoriteMultiplierSuccessor?2:berryStrengthSuccessor?1:0)+(slotSuccessor?1:0);
...
const slotRow=byDimension.get('ingredient_slot_distribution');
if(slotSuccessor){assert.equal(slotRow.authority_status,'ACTIVE_VERIFIED');assert.equal(slotRow.runtime_numeric_activation,true);assert.equal(slotRow.evidence_status,EVIDENCE_STATUS.ACTIVE_VERIFIED_LOCAL_NUMERIC_CONTRACT);assert.deepEqual(slotRow.blocking_reasons,[]);assert.equal(slotRow.coverage.observed_count,3);}else{assert.equal(slotRow.runtime_numeric_activation,false);}
...
if(slotSuccessor){const incompleteSlot=incomplete.rules.find(row=>row.dimension==='ingredient_slot_distribution');assert.equal(incompleteSlot.coverage.observed_count,0);assert.ok(incompleteSlot.blocking_reasons.includes('INCOMPLETE_INGREDIENT_SLOT_DISTRIBUTION_INPUT_COVERAGE'));}

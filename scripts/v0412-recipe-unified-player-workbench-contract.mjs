...
assert.equal(projection.total_count,total);
assert.equal(projection.unlocked_count,13);
assert.equal(projection.locked_count,total-13);
assert.equal(projection.partition_complete,true);
assert.deepEqual(projection.duplicate_recipe_ids,[]);
assert.equal(new Set([...projection.unlocked,...projection.locked].map(row=>row.recipe_id)).size,total,'each canonical recipe_id must appear in exactly one section');

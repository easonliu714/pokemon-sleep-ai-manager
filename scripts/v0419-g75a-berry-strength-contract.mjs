...
assert.equal(appVersion,'v0.4.19');
assert.ok(version.includes("// app_version: 'v0.4.18'"),'v0.4.19 must retain v0.4.18 lineage bridge');
...
const anchors={
  '柿仔果':[28,37,47,57,73,94,120,154],
  '蘋野果':[27,36,46,56,71,91,116,148],
  '橙橙果':[31,40,50,63,81,104,133,170],
  '葡萄果':[25,34,44,54,65,84,107,137],
  '金枕果':[30,39,49,61,79,101,129,165],
  '莓莓果':[32,41,51,65,84,107,137,176],
  '櫻子果':[27,36,46,56,71,91,116,148],
  '零餘果':[32,41,51,65,84,107,137,176],
  '勿花果':[29,38,48,59,76,97,124,159],
  '椰木果':[24,33,43,53,63,80,103,132],
  '芒芒果':[26,35,45,55,68,87,112,143],
  '木子果':[24,33,43,53,63,80,103,132],
  '文柚果':[30,39,49,61,79,101,129,165],
  '墨莓果':[26,35,45,55,68,87,112,143],
  '番荔果':[35,44,56,72,92,117,150,192],
  '異奇果':[31,40,50,63,81,104,133,170],
  '靛莓果':[33,42,53,68,86,111,142,181],
  '桃桃果':[26,35,45,55,68,87,112,143],
};
for(const [berry,values] of Object.entries(anchors)){
  anchorLevels.forEach((level,index)=>assert.equal(berryStrengthAtLevel(berry,level),values[index],`${berry} Lv.${level} anchor mismatch`));
}
...
assert.equal(registry.rules.favorite_berry_multiplier.status,'NOT_YET_VERIFIED');
...
assert.deepEqual(registry.active_verified_dimensions,['berry_energy_per_berry']);

const candidateFeatures={candidates:[
  {pokemon_id:'private-fire',type:'火',level:30,helper_seconds:2100,main_skill:'能量填充S',unlocked_ingredients:[{ingredient_name:'火辣香草',quantity:2}]},
  {pokemon_id:'private-water',type:'水',level:60,helper_seconds:2200,main_skill:'食材獲取S',unlocked_ingredients:[{ingredient_name:'哞哞鮮奶',quantity:2}]},
  {pokemon_id:'private-dragon',type:'龍',level:70,helper_seconds:2300,main_skill:'活力全體療癒S',unlocked_ingredients:[{ingredient_name:'甜甜蜜',quantity:2}]},
]};
...
assert.equal(snapshot.summary.active_numeric_dimension_count,1);
assert.equal(snapshot.summary.blocked_numeric_dimension_count,6);
...
const member=projectMemberProductionEvidence({pokemon_id:'p-electric',type:'電',level:70,helper_seconds:2500,main_skill:'能量填充S',unlocked_ingredients:[]},{productionRegistry:registry});
...
const invalidMember=projectMemberProductionEvidence({pokemon_id:'p-invalid',type:'電',level:71,helper_seconds:2500,unlocked_ingredients:[]},{productionRegistry:registry});
...
const teamFeatures={candidates:[1,2,3,4,5].map((n,index)=>({pokemon_id:`p${n}`,type:['火','水','電','草','龍'][index],level:30,helper_seconds:2400+index*50,unlocked_ingredients:[]}))};
...
console.log(JSON.stringify({
  status:'PASS',gate:'V0419_G75A_BERRY_STRENGTH_NUMERIC_MASTER',app_version:appVersion,

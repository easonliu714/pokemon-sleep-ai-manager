...
function signature(recipe){
  return [...(recipe.ingredients||[])].map(row=>`${row.ingredient_name}=${Number(row.quantity)}`).sort((a,b)=>a.localeCompare(b,'zh-Hant')).join('|');
}
...
assert.match(PUBLIC_RECIPE_MASTER_VERSION,/^public-recipe-master-2026-08-(?:09-b|11-[a-z]|12-[a-z])$/,'canonical recipe master successor version invalid');
assert.match(PUBLIC_RECIPE_CANONICAL_NAME_VERSION,/^public-recipe-zh-tw-names-2026-08-(?:09-a|11-[a-z]|12-[a-z])$/,'canonical name successor version invalid');
...
assert.ok(formulaChanges.length<=1,'only the explicitly verified 親子愛咖哩 formula may differ among the historical 76 baseline');
...
for(const override of PUBLIC_RECIPE_ZH_TW_NAME_OVERRIDES){
  ...
  const base=baseById.get(override.recipe_id),canonical=canonicalById.get(override.recipe_id);
  ...
  const legacyAlias=PUBLIC_RECIPE_ALIASES.find(row=>row.recipe_id===override.recipe_id&&row.alias_type==='legacy_recipe_name'&&row.alias_value===override.legacy_public_name&&row.source_type==='pre_v043_public_recipe_name_compatibility');
  assert.ok(legacyAlias,`missing legacy-name alias for ${override.recipe_id}`);assert.equal(Boolean(legacyAlias.is_auto_replace_safe),true);
  const canonicalResolution=resolvePublicRecipeName(override.canonical_name_zh_tw);
  assert.deepEqual(canonicalResolution,{recipe_id:override.recipe_id,recipe_name:override.canonical_name_zh_tw,resolution:'CANONICAL_EXACT',requires_review:false,commit_allowed:true});
  const legacyResolution=resolvePublicRecipeName(override.legacy_public_name);
  assert.equal(legacyResolution?.recipe_id,override.recipe_id);assert.equal(legacyResolution?.recipe_name,override.canonical_name_zh_tw);assert.equal(legacyResolution?.resolution,'LEGACY_NAME_ALIAS_SAFE');assert.equal(legacyResolution?.requires_review,false);assert.equal(legacyResolution?.commit_allowed,true);
}

const conflictById=new Map(PUBLIC_RECIPE_FORMULA_CONFLICT_REVIEWS.map(row=>[row.recipe_id,row]));
assert.ok(conflictById.has('curry_dizzy_punch'));assert.ok(conflictById.has('curry_parent_child'));
for(const row of conflictById.values()){assert.equal(row.status,'FORMULA_CONFLICT_REVIEW');assert.equal(row.auto_apply,false);}
...
}else assert.equal(canonicalById.get('curry_dizzy_punch').recipe_name,'暈眩拳辣味咖哩');
...
}else assert.equal(signature(canonicalById.get('curry_parent_child')),signature(baseById.get('curry_parent_child')));

assert.match(PUBLIC_RECIPE_PROVENANCE_VERSION,/^public-recipe-provenance-2026-08-(?:09-b|11-[a-z]|12-[a-z])$/,'recipe provenance successor version invalid');
...
for(const row of PUBLIC_RECIPE_PROVENANCE)assert.equal(row.recipe_name_zh_tw,canonicalById.get(row.recipe_id)?.recipe_name,`provenance name drifted for ${row.recipe_id}`);

const screenshotAliasRows=PUBLIC_RECIPE_ALIASES.filter(row=>row.source_type==='pre_v043_public_recipe_name_compatibility');
assert.ok(screenshotAliasRows.length>=33);
for(const recipeId of historicalOverrideIds)assert.ok(screenshotAliasRows.some(row=>row.recipe_id===recipeId),`historical legacy alias missing: ${recipeId}`);
...
for(const file of collectJsFiles(path.join(root,'assets/js'))){const relative=path.relative(root,file).replaceAll('\\','/');if(relative==='assets/js/public-recipe-canonical-authority.js')continue;const source=fs.readFileSync(file,'utf8');if(/from\s+['"]\.\/public-recipe-master\.js['"]/.test(source))directRawImports.push(relative);}
...
process.stdout.write(`${JSON.stringify({status:'PASS',gate:'R2.2_RECIPE_CANONICAL_ZH_TW_NAME_ALIAS_CONTRACT_SUCCESSOR_AWARE',base_master_version:BASE_PUBLIC_RECIPE_MASTER_VERSION,canonical_master_version:PUBLIC_RECIPE_MASTER_VERSION,canonical_name_version:PUBLIC_RECIPE_CANONICAL_NAME_VERSION,active_recipe_count:PUBLIC_RECIPE_MASTER.length,historical_stable_recipe_id_count:baseById.size,current_recipe_id_count:canonicalById.size,original_v043_renamed_baseline:historicalOverrideIds.size,current_renamed_recipe_count:renamed,legacy_public_name_alias_count:screenshotAliasRows.length,historical_formula_conflict_audit_count:PUBLIC_RECIPE_FORMULA_CONFLICT_REVIEWS.length,current_historical_formula_changes:formulaChanges,direct_runtime_raw_master_imports:directRawImports,historical_release_marker_preserved:true},null,2)}\n`);

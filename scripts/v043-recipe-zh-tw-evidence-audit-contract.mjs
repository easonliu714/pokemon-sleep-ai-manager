...
const report = runAuditFromFiles({
  privateFile: fixture,
  publicMasterFile: publicMaster,
  requireCount: 5,
});
...
assert.deepEqual(report.category_counts, {
  '咖哩／濃湯': 4,
  '甜點／飲料': 1,
});
...
assert.equal(corn.match_basis, 'CATEGORY_INGREDIENT_SIGNATURE');
...
const warmMilk = byObservedName.get('溫熱哞哞鮮奶');
assert.ok(warmMilk);
assert.equal(warmMilk.category, '甜點／飲料');
assert.equal(warmMilk.classification, 'EXACT_NAME');
...
for (const forbidden of [
  'synthetic_private_001',
  'synthetic_private_002',
  'synthetic_private_003',
  'synthetic_private_004',
  'synthetic_private_005',
  'SYNTHETIC_PRIVATE_UPDATE_DO_NOT_LEAK',
  'SYNTHETIC_PRIVATE_NOTE_SHOULD_NOT_LEAK',
  'SYNTHETIC_SCREENSHOT_DO_NOT_LEAK.png',
  'recipe_level',
  'current_energy',
  'source_image_ref',
  'operation_id',

...
for(const token of ['AES-GCM','indexedDB','ciphertext'])assert.match(vault,new RegExp(token));assert.match(vault,/generateKey\([^)]*AES-GCM[\s\S]*?false/);
...
for(const source of [vault,settings,controller]){assert.doesNotMatch(source,/DebugTrace[^\n]*project\.key/);assert.doesNotMatch(source,/DebugTrace[^\n]*\bkeys\s*:/);assert.doesNotMatch(source,/DebugTrace[^\n]*AIza/);}
assert.match(settings,/fingerprints:/);assert.match(settings,/key_count:/);

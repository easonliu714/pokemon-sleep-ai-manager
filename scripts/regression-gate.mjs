...
async function migrationStaticGate(){
  ...
  const migrations=await text('assets/js/migrations.js');
  for(const field of ['recipe_level','current_energy','updated_at','notes']){
    assert.match(migrations,new RegExp(`addColumnIfMissing\\(db,'recipes','${field}'`),`missing recipe migration field: ${field}`);
  }
  for(const version of [1,2,3,4,5]){
    assert.match(`${schema}\n${migrations}`,new RegExp(`schema_migrations[^\\n]*${version}|VALUES\\(${version},`),`migration ${version} is not registered`);
  }
  ...
  console.log('PASS migration structure: schema versions 1-5 and lifecycle hooks');
}
...
async function migrationFixtureGate(){
  ...
  assert.deepEqual(queryRows(fresh,'SELECT version FROM schema_migrations ORDER BY version').map(row=>row.version),[1,2,3,4,5]);
  for(const field of ['recipe_level','current_energy','updated_at','notes']){
    assert.ok(queryRows(fresh,"PRAGMA table_info('recipes')").some(row=>row.name===field),`fresh DB missing recipes.${field}`);
  }
  ...
  legacy.run(`
    ...
    CREATE TABLE pokemon(
      pokemon_id TEXT PRIMARY KEY,species TEXT NOT NULL,original_label TEXT,nickname TEXT,
      nickname_halfwidth_units INTEGER,nickname_valid INTEGER NOT NULL DEFAULT 1,level INTEGER,sp INTEGER,
      specialty TEXT,type TEXT,nature TEXT,nature_bonus TEXT,nature_penalty TEXT,main_skill TEXT,
      main_skill_level INTEGER,helper_seconds INTEGER,carry_limit INTEGER,favorite_berry TEXT,rating TEXT,
      ai_score REAL,status TEXT NOT NULL DEFAULT 'active',core_role TEXT,recommendation TEXT,item_advice TEXT,
      scenarios TEXT,is_favorite INTEGER NOT NULL DEFAULT 0,is_main INTEGER NOT NULL DEFAULT 0,
      obtained_at TEXT,last_updated_at TEXT NOT NULL,source_update_id TEXT
    );
  ...
  legacy.run(DDL);
  applyAllMigrations(legacy);
  const exported=legacy.export();
  legacy.close();

  const restored=new SQL.Database(exported);
  assert.equal(scalar(restored,'PRAGMA integrity_check'),'ok','restored legacy DB integrity check failed');
  applyAllMigrations(restored);
  assert.equal(scalar(restored,'PRAGMA integrity_check'),'ok','post-restore migration integrity check failed');
  ...
  assert.deepEqual(queryRows(restored,'SELECT version FROM schema_migrations ORDER BY version').map(row=>row.version),[1,2,3,4,5]);
  restored.close();
  fresh.close();
  console.log(`PASS SQLite fixtures: fresh integrity=ok; legacy restore integrity=ok; personal rows ${JSON.stringify(before)} preserved`);
}
...
async function knowledgeGate(){
  ...
  for(const [type,berry] of Object.entries(expected)) assert.ok(master.includes(`['${type}','${berry}']`),`berry mapping mismatch: ${type}→${berry}`);
...
async function serviceWorkerGate(){
  ...
  for(const asset of ['shared-master-schema.js','shared-master-data.js','shared-knowledge-ui.js']) assert.ok(sw.includes(asset),`service-worker cache missing ${asset}`);
...
await syntaxGate();
await migrationStaticGate();
await migrationFixtureGate();
await knowledgeGate();

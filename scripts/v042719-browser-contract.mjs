...
try{
  ...
  const missingScript=await page.evaluate(async()=>{
    ...
    const body=await response.text();return {status:response.status,bodyStart:body.slice(0,80)};
  });
  assert.equal(missingScript.status,504,'missing offline script must fail as asset');

...
const candidateIntakeSource=fs.readFileSync('assets/js/ingredient-probability-independent-candidate-intake.js','utf8');
assert.ok(candidateIntakeSource.includes('READY_FOR_LINEAGE_REVIEW'),'E3C-7C5 lineage-review readiness state missing');
assert.ok(candidateIntakeSource.includes('ready_for_lineage_review_implies_source_admission:false'),'E3C-7C5 must not bypass source admission');
assert.ok(candidateIntakeSource.includes('MODEL_FIT_OR_REVERSE_ENGINEERED_NOT_ACTIVATION_GRADE_REFERENCE'),'E3C-7C5 model-fit rejection missing');

...
const recipeStrategy={projection_status:'READY',input_fingerprint:'recipe-fixture',candidates:[{
  recipe_id:'evidence-test',recipe_name:'Evidence Test',category:'curry',unlocked:true,total_ingredients:7,candidate_status:'COOK_NOW_UNLOCKED',hard_constraint_status:'PASS',pot_fit:true,total_raw_shortage:6,total_strategy_shortage:6,
  requirements:[
    {ingredient_name:'Apple',required:1,owned:10,usable:10,safe_reserve:0,raw_shortage:0,strategy_shortage:0},
    {ingredient_name:'Milk',required:1,owned:0,usable:0,safe_reserve:0,raw_shortage:1,strategy_shortage:1},
    {ingredient_name:'Honey',required:5,owned:0,usable:0,safe_reserve:0,raw_shortage:5,strategy_shortage:5},
  ],
}]};
const portfolio={planner_version:'recipe-portfolio-fixture',projection_status:'READY',input_fingerprint:'portfolio-fixture',recipe_strategy_fingerprint:'recipe-fixture',objective:'unlock_recipes',context:{max_meals:3},summary:{alternative_count:0},contention:{contention_edge_count:0,oversubscribed_ingredient_count:0},alternatives:[],missing_inventory_observations:[{recipe_id:'evidence-test',ingredients:['Honey']}],player_data_write:false,inventory_mutation:false,public_master_write:false,gemini_used:false};
...
assert.equal(byName.get('Apple').quantity_state,'OBSERVED_QUANTITY');
...
const req=pack.deterministic_results.recipe_strategy.candidates[0].requirements.find(row=>row.ingredient_name==='Honey');
assert.ok(req,'Honey requirement fixture missing');
...
assert.equal(pack.integrity_manifest.empty_collection_not_zero,true);

...
for(const file of activeFiles){
  ...
  if(text.includes(authority.app_version)||text.includes(authority.app_build)||text.includes(authority.cache_name)){
    throw new Error(`hardcoded_current_authority:${file}`);
  }
}

const bootstrap=fs.readFileSync(path.join(root,'assets/js/bootstrap.js'),'utf8');
const worker=fs.readFileSync(path.join(root,'service-worker.js'),'utf8');
for(const [name,text] of [['bootstrap',bootstrap],['service-worker',worker]]){
  if(!/version-authority\.js/.test(text))throw new Error(`authority_import_missing:${name}`);
}
...
console.log(JSON.stringify({ok:true,authority,active_files:activeFiles},null,2));

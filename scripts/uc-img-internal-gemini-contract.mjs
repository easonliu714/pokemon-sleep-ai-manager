...
import {UC_IMG_GEMINI_ADAPTER_VERSION,UC_IMG_DIAGNOSTIC_SCHEMA,analyzeUcImgScenarioWithGemini,buildUcImgDiagnosticBundle,buildUcImgGeminiSchema,extractGeminiJsonText,safeAttemptFailureMessage,safeProviderErrorDetail} from '../assets/js/uc-img-gemini-adapter.js';
...
const promptSafetyAdapter=['uc-img-gemini-2026-08-15-c-prompt-safety-contract','uc-img-gemini-2026-08-17-d-live-recovery-schema','uc-img-gemini-2026-08-19-v042713-feature-model-capability','uc-img-gemini-2026-09-02-v042755-compact-public-master-schema'].includes(UC_IMG_GEMINI_ADAPTER_VERSION);
const liveSemanticAdapter=['uc-img-gemini-2026-08-17-d-live-recovery-schema','uc-img-gemini-2026-08-19-v042713-feature-model-capability','uc-img-gemini-2026-09-02-v042755-compact-public-master-schema'].includes(UC_IMG_GEMINI_ADAPTER_VERSION);
const schema=buildUcImgGeminiSchema(UC_IMG_A_SCENARIOS.ingredients,'ingredients');
assert.deepEqual(schema.properties.schema.enum,[PUBLIC_MASTER_RECOGNITION_SCHEMA]);assert.deepEqual(schema.properties.authority.enum,['ingredient_master']);assert.deepEqual(schema.properties.scenario.enum,['ingredient_inventory_update']);assert.deepEqual(schema.properties.observations.items.properties.status.enum,['MATCHED','AMBIGUOUS','UNMATCHED']);assert.equal('enum' in schema.properties.observations.items.properties.canonical_key.properties.ingredient_name,false,'provider canonical key must not expand Public Master enum');assert.equal('enum' in schema.properties.observations.items.properties.canonical_name,false,'provider canonical name must stay structural');assert.equal('enum' in schema.properties.observations.items.properties.candidate_names.items,false,'provider candidate names must stay structural');assert.equal('ingredient_id' in schema.properties.observations.items.properties.canonical_key.properties,false);assert.equal('capacity_observations' in schema.properties,false,'ingredient recognition must not inherit recipe pot-capacity observations');
...
const providerErrorMessage='Request contains an invalid argument.';
const surfacedFailure=safeAttemptFailureMessage({attempts:[{status:'FAILED',error_class:'request_rejected',http_status:400,error_message:providerErrorMessage}]});
assert.equal(surfacedFailure,'Gemini 分析暫停：request_rejected (HTTP 400) — Request contains an invalid argument.');
const secret='AIza1234567890abcdefghijklmnop';
const redacted=safeProviderErrorDetail(`bad\nhttps://example.invalid/?key=${secret} token=${secret}`);
assert.equal(redacted.includes(secret),false,'provider detail must redact API keys');
assert.ok(redacted.includes('[redacted]')||redacted.includes('[redacted-api-key]'));
assert.ok(safeProviderErrorDetail('x'.repeat(500)).length<=240,'provider detail must remain bounded');
assert.equal(safeProviderErrorDetail('\u0000\u0001 Request\n contains\t invalid '),'Request contains invalid');
...
let captured=null;const fakeExecute=async args=>{captured=args;return {ok:true,payload:providerPayload,projects:args.projects,used_alias:'Project A',used_model:args.preferredModel||args.model,model_fallback_used:false,attempts:[]};};const poolData={model:'gemini-test-flash',projects:[{alias:'Project A',key:'AIza-SECRET-MUST-NOT-LEAK',priority:1,enabled:true}]};
...
assert.equal(captured.images.length,2);assert.equal(captured.images[0].imageRef,'image-001');assert.equal(captured.images[0].fileName,'ingredient-1.png');assert.equal(captured.images[1].imageRef,'image-002');assert.equal(captured.images[1].fileName,'ingredient-2.jpg');assert.deepEqual(captured.responseJsonSchema,schema);assert.equal(captured.preferredModel,'gemini-test-flash');assert.equal(captured.feature,'uc_img_ingredients');assert.equal(analysis.model,'gemini-test-flash');assert.equal(analysis.preferred_model,'gemini-test-flash');assert.equal(analysis.project_alias,'Project A');assert.equal(analysis.response_contract,'public-master-recognition');assert.deepEqual(analysis.payload,recognition);
...
const adapterSource=fs.readFileSync(new URL('../assets/js/uc-img-gemini-adapter.js',import.meta.url),'utf8');for(const forbidden of ['applyPayload','dryRun','importer.js','indexedDB','localStorage'])assert.equal(adapterSource.includes(forbidden),false,`Gemini adapter must not own ${forbidden}`);assert.equal(adapterSource.includes('buildRecipePotCapacityPromptInstruction'),false,'Recipe pot contract must remain owned by Public Master Recognition, not duplicated in adapter');assert.ok(adapterSource.includes('buildCompactPublicMasterProviderSchemaForInput'),'Issue #420 runtime must use compact provider schema');assert.ok(adapterSource.includes('safeProviderErrorDetail'),'Issue #420 runtime must surface bounded redacted provider error detail');if(promptSafetyAdapter){assert.ok(adapterSource.includes('appendScreenshotPromptSafety'));assert.ok(adapterSource.includes('SCREENSHOT_PROMPT_SAFETY_VERSION'));}

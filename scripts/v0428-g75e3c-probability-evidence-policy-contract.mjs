...
import {currentPublicSpeciesFormRosterReadiness} from '../assets/js/public-pokemon-species-form-roster-readiness.js';
...
for(const required of ['VERSIONED_LOCAL_ACTIVATION_MASTER','COMPLETE_CURRENT_SPECIES_FORM_COVERAGE','EXPLICIT_SOURCE_COMMIT_AND_PATH_PER_ROW','FORM_SAFE_CANONICAL_IDENTITY_PER_ROW','NO_SOURCE_DECLARED_SUSPICIOUS_OR_MODEL_FIT_VALUES','INDEPENDENT_CURRENT_CROSSCHECK_PER_ACTIVATION_ROW_OR_EXPLICIT_REVIEW_HOLD','DISCREPANCY_REPORT_ZERO_UNRESOLVED_ACTIVATION_CONFLICTS','NATURE_AND_SUBSKILL_COMPOSITION_ORDER_CONTRACT_ACCEPTED','UNKNOWN_OR_AMBIGUOUS_FORM_FAILS_CLOSED','NO_RUNTIME_NETWORK_FETCH','NO_PLAYER_OR_SQLITE_WRITE','NO_AI_NUMERIC_AUTHORITY'])assert.ok(policy.requirements.includes(required),`activation requirement missing: ${required}`);

const roster=currentPublicSpeciesFormRosterReadiness();
assert.ok(['pokemon-sleep-public-species-form-roster-readiness/1.0','pokemon-sleep-public-species-form-roster-readiness/1.1'].includes(roster.schema));
const rosterReady=roster.activation_coverage_denominator_ready===true;
if(rosterReady){
  assert.equal(roster.authority_status,'ACTIVE_GOVERNED_CATALOG_REFERENCE');
  assert.equal(roster.expected_current_species_form_count,242);
  assert.equal(roster.roster_row_count,242);
  ...
  assert.equal(roster.safety.announced_pending_entries_auto_include,false);
  assert.equal(roster.safety.roster_values_imply_ingredient_probability,false);
}else{
...
const mewResult=evaluateIngredientProbabilityActivationRow({source_key:'MEW',base_ingredient_probability:0.2,source_commit:INGREDIENT_PROBABILITY_PINNED_SOURCE_COMMIT,source_path:'common/src/types/pokemon/all-pokemon.ts',canonical_species_form_id:'mew',evidence_class:INGREDIENT_PROBABILITY_EVIDENCE_CLASS.COMMUNITY_RESEARCH_DERIVED,independent_current_crosscheck_count:99});
assert.equal(mewResult.status,'EXCLUDED_FROM_ACTIVATION');
assert.equal(mewResult.eligible_for_numeric_activation,false);
assert.equal(mewResult.evidence_class,INGREDIENT_PROBABILITY_EVIDENCE_CLASS.SOURCE_DECLARED_SUSPICIOUS);

const readyFixture={source_key:'BULBASAUR',base_ingredient_probability:0.257,source_commit:INGREDIENT_PROBABILITY_PINNED_SOURCE_COMMIT,source_path:'common/src/types/pokemon/ingredient-pokemon.ts',canonical_species_form_id:'neroli:bulbasaur',form_identity_ambiguous:false,evidence_class:INGREDIENT_PROBABILITY_EVIDENCE_CLASS.COMMUNITY_RESEARCH_DERIVED,independent_current_crosscheck_count:1,unresolved_numeric_conflict:false};
assert.equal(evaluateIngredientProbabilityActivationRow(readyFixture).status,'ACTIVATION_ROW_EVIDENCE_READY');
...
const currentDenominatorAudit=evaluateIngredientProbabilityActivationMaster({rows:[readyFixture],expected_current_species_form_count:roster.expected_current_species_form_count});
assert.equal(currentDenominatorAudit.complete_current_species_form_coverage,false);
assert.equal(currentDenominatorAudit.activation_decision,'HOLD_ACTIVATION_MASTER_INCOMPLETE_OR_UNRESOLVED','having a governed denominator must not make an incomplete rate master ready');
if(rosterReady)assert.equal(currentDenominatorAudit.expected_current_species_form_count,242);else assert.equal(currentDenominatorAudit.expected_current_species_form_count,null);
const containsExcluded=evaluateIngredientProbabilityActivationMaster({rows:[readyFixture,{...readyFixture,source_key:'MEW',canonical_species_form_id:'neroli:mew'}],expected_current_species_form_count:2});
...
const syntheticReadyMaster=evaluateIngredientProbabilityActivationMaster({rows:[readyFixture,{...readyFixture,source_key:'IVYSAUR',canonical_species_form_id:'neroli:ivysaur'}],expected_current_species_form_count:2});
...
// Synthetic readiness validates the evaluator only. Current project still lacks a 242-row accepted probability activation master.
assert.notEqual(currentDenominatorAudit.activation_decision,'READY_FOR_EXPLICIT_AUTHORITY_PROMOTION');
...
console.log(JSON.stringify({status:'PASS',gate:'V0428_G75E3C_PROBABILITY_ACTIVATION_EVIDENCE_POLICY_SUCCESSOR_AWARE',policy_id:INGREDIENT_PROBABILITY_ACTIVATION_POLICY_ID,pinned_source_commit:INGREDIENT_PROBABILITY_PINNED_SOURCE_COMMIT,mew_source_declared_suspicious_excluded:true,public_species_form_roster_authority_status:roster.authority_status,public_roster_denominator_ready:rosterReady,expected_current_species_form_count:roster.expected_current_species_form_count,current_probability_master_complete:false,active_numeric_dimensions:active,production_numeric_activation:'4/7',ingredient_probability_status:registry.rules.ingredient_probability_per_help.status,species_reference_status:reference.status,overall_numeric_model_status:registry.numeric_rate_model_status,missing_is_zero:false,runtime_network_fetch:false,ai_numeric_authority:false},null,2));

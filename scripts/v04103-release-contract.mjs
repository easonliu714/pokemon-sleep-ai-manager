...
assert.ok(versionAtLeast(appVersion,'v0.4.10.3'),`unexpected v0.4.10.3 successor: ${appVersion}`);
if(exactReleases[appVersion])assert.deepEqual([version.match(/app_build:\s*'([^']+)'/)?.[1],version.match(/cache_name:\s*'([^']+)'/)?.[1]],exactReleases[appVersion]);
else{
  assert.ok(version.includes("// app_version: 'v0.4.13.2'"),'successor must retain latest known historical bridge');

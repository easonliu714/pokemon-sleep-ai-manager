...
for(const token of ['zero-write preflight','WEEKLY_CONTEXT_EPOCH_MISMATCH','write_performed','player_rows_modified'])assert.ok(lifecycleUi.includes(token)||lifecycleUi.includes(token.replace('zero-write preflight','zero-write preflight')),`lifecycle UI transparency missing: ${token}`);
...
process.stdout.write(`${JSON.stringify({
  ...
  unrelated_target_reused:changed.reused_count,archived_snapshot_staled:true,force_refresh_explicit:true,goal_profile_trigger:true,weekly_context_trigger:true,
  pokemon_scoped_trigger:true,generic_refresh_preflight:true,ingredient_item_do_not_trigger_pokemon_evaluation:true,player_rows_modified:false,gemini_used:false,
},null,2)}\n`);

...
function constExpression(source,identifier){const escaped=identifier.replace(/[.*+?^${}()|[\]\\]/g,'\\$&'),regex=new RegExp(`(?:^|\\n)\\s*const\\s+${escaped}\\b[^=]*=`,'g'),match=regex.exec(source);if(!match)return null;const start=regex.lastIndex,end=findExpressionEnd(source,start);return source.slice(start,end).trim();}
function directPokedexNumber(expression){const match=String(expression||'').match(/\bpokedexNumber\s*:\s*(\d+)\b/);return match?Number(match[1]):null;}
function resolvedPokedexNumber(expression,source,seen=new Set()){
  const direct=directPokedexNumber(expression);if(Number.isInteger(direct))return Object.freeze({pokedex_number:direct,provenance:'DIRECT_EXPORT'});
  const spreadIds=[...String(expression||'').matchAll(/\.\.\.\s*([A-Za-z_$][A-Za-z0-9_$]*)/g)].map(match=>match[1]);
  for(const identifier of spreadIds){if(seen.has(identifier))continue;const shared=constExpression(source,identifier);if(!shared)continue;const nested=resolvedPokedexNumber(shared,source,new Set([...seen,identifier]));if(nested)return Object.freeze({pokedex_number:nested.pokedex_number,provenance:'SHARED_OBJECT_SPREAD',shared_source_key:identifier});}
  return null;
}
...
for(const roster of PUBLIC_SPECIES_FORM_ROSTER_ROWS){
  const declaration=declarations.get(roster.source_key);if(!declaration){failures.push(`${roster.source_key}:EXPORT_MISSING`);continue;}
  const sourceText=neroli.get(declaration.source_path)?.text||'';
  const dexResolution=resolvedPokedexNumber(declaration.expression,sourceText);
  if(!Number.isInteger(dexResolution?.pokedex_number)||dexResolution.pokedex_number<=0){failures.push(`${roster.source_key}:POKEDEX_NUMBER_MISSING`);continue;}
  sourceRows.push({...roster,pokedex_number:dexResolution.pokedex_number,pokedex_provenance:dexResolution.provenance,pokedex_shared_source_key:dexResolution.shared_source_key||null,source_blob_sha:declaration.blob_sha});
}
...
const pokedexProvenanceCounts=sourceRows.reduce((out,row)=>{out[row.pokedex_provenance]=(out[row.pokedex_provenance]||0)+1;return out;},{});

const artifact={schema:'pokemon-sleep-public-species-form-zh-tw-identity-source-audit/1.0',generated_at:new Date().toISOString(),neroli_commit:NEROLI_COMMIT,pokeapi_commit:POKEAPI_COMMIT,roster_row_count:242,source_key_pokedex_mapped_count:sourceRows.length,pokedex_provenance_counts:pokedexProvenanceCounts,base_identity_count:identities.filter(row=>!row.form_override).length,form_override_count:identities.filter(row=>row.form_override).length,unresolved_base_groups:unresolved,extraction_failures:failures,duplicate_identity_names:duplicateIdentityNames,policy:{exact_display_name_only:true,fuzzy_auto_match:false,private_player_data_used:false,ingredient_candidate_generation:false,source_key_guess_from_ai:false,ambiguous_non_equivalent_forms_review_required:true},source_files:{neroli:Object.fromEntries([...neroli].map(([file,row])=>[file,row.blob_sha])),pokeapi:Object.fromEntries([...pokeapi].map(([file,row])=>[file,row.blob_sha]))},source_key_rows:sourceRows.map(row=>({source_key:row.source_key,pokedex_number:row.pokedex_number,pokedex_provenance:row.pokedex_provenance,pokedex_shared_source_key:row.pokedex_shared_source_key,canonical_species_form_id:row.canonical_species_form_id})),identities};

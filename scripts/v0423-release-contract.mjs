...
assert.equal(authority.app_version,'v0.4.23');
...
assert.ok(authoritySource.includes("// app_build: '20260813-v04221-recipe-formula-authority-audit'"),'v0.4.22.1 predecessor build lineage missing');
...
assert.equal(profile.schema,'pokemon-sleep-production-modifier-profile/1.0');
...
assert.deepEqual({direction:natureHelp.direction,dimension:natureHelp.dimension,numeric_status:natureHelp.numeric_status},{direction:'UP',dimension:'helper_interval_seconds',numeric_status:'NOT_YET_VERIFIED'});
assert.deepEqual({direction:natureIngredient.direction,dimension:natureIngredient.dimension,numeric_status:natureIngredient.numeric_status},{direction:'DOWN',dimension:'ingredient_probability_per_help',numeric_status:'NOT_YET_VERIFIED'});
...
const pack=buildStrategyOptimizationPack({
  strategyContextResult:contextResult,
  candidateScoring:{candidates:[candidate]},
  teamOptimization:{primary:{team_status:'READY',input_fingerprint:'team:v0423',slots:[{pokemon_id:candidate.pokemon_id}]}},
...
assert.equal(STRATEGY_OPTIMIZATION_PACK_VERSION,'strategy-optimization-pack-2026-08-14-a');
...
console.log(JSON.stringify({
  status:'PASS',gate:'V0423_G75E1_STRUCTURAL_PRODUCTION_MODIFIER_RELEASE',app_version:authority.app_version,app_build:authority.app_build,cache_name:authority.cache_name,
  predecessor:'v0.4.22.1',modifier_registry_version:PRODUCTION_MODIFIER_STRUCTURAL_VERSION,optimization_pack_version:STRATEGY_OPTIMIZATION_PACK_VERSION,
  active_numeric_dimension_count:registry.active_verified_dimensions.length,overall_numeric_model_status:registry.numeric_rate_model_status,
  nature_structural_routing:true,berry_finding_s_delegated:true,helping_bonus_team_scope:true,unknown_modifier_fail_closed:true,modifier_numeric_activation:false,
  candidate_ref_payload:true,stable_ids_in_payload:false,recipe_render_hydration_write_free:true,recipe_observed_energy_preserved:true,

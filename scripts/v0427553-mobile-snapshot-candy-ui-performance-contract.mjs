...
const memoryStorage={value:new Map(),getItem(key){return this.value.get(key)??null;},setItem(key,value){this.value.set(key,String(value));}};
...
// the exact release-version whitelist advances through .55.3.3.9.
assert.match(versionSource,/app_version: 'v0\.4\.27\.55\.3(?:\.[12]|\.3(?:\.[123456789])?)?'/);
...
assert.match(globalThis.PokemonSleepVersionAuthority?.app_version||'',/^v0\.4\.27\.55\.3(?:\.[12]|\.3(?:\.[123456789])?)?$/);

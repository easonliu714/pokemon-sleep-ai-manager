...
import {resolvePublicSpeciesFormSourceKeys} from '../assets/js/public-species-form-zh-tw-identity.js';
...
assert.equal(POKEMON_VISUAL_EVIDENCE_VERSION,'pokemon-visual-evidence-2026-08-15-b-species-ingredient-candidates');
...
// Type and Berry must be two independent direct observations.
...
// Ingredient canonical identity remains exact; legacy alias is suggestion-only.
...
// Public Species↔Ingredient authority is now active, slot-specific and consistency-only.
...
assert.equal(drifloonCorn.auto_rewrite_player_observation,false);
...
assert.equal(dratiniAvocado.reason,'SPECIES_INGREDIENT_VISUAL_CONFLICT');
assert.equal(dratiniAvocado.auto_rewrite_player_observation,false);
const pumpkin=evaluateIngredientEvidence(ev('INGREDIENT_VISUAL','窩心洋芋','ability.png',0.99,60),{species:'南瓜精'});
assert.equal(pumpkin.status,'MATCH','size is not guessed; four size source keys are candidate-equivalent');
...
// Test injection remains available for pure unit fixtures but never creates an observed value.
const injected=evaluateIngredientEvidence(ev('INGREDIENT_VISUAL','萌綠玉米','ability.png',0.99,1),{species:'fixture',speciesIngredientCandidates:['萌綠玉米','純粹油']});
assert.equal(injected.status,'MATCH');
assert.deepEqual(injected.generated_player_values,[]);

// Finite skill vocabulary is exact-only.
...
const legacyIngredientBlocked=evaluatePokemonVisualEvidence({
  species:'老翁龍',type:ev('TYPE_VISUAL','龍'),berry:ev('BERRY_VISUAL','番荔果'),
  ingredients:[ev('INGREDIENT_VISUAL','特選酪梨','ability.png',0.99,60)],
});
...
// Observation schema: visual evidence is supplied separately and never synthesized from profile values.
const observation={
  incoming_ref:'electric-001',requested_action:'resolve_on_import',identity:{},
  ...
  visual_evidence:{contract_version:POKEMON_VISUAL_EVIDENCE_VERSION,public_relation_may_generate_player_observation:false,
...
const bridge=buildObservationPayloadFromScreenshotGroups([{group_key:'electric-001',header:{name:'電龍',level:30,sp:1000},images:[{path:'ability.png'},{path:'subskills.png'}]}],{'electric-001':{aiObservation:observation}});
...
// Manifest ↔ executable operation binding remains mandatory.
...
// Ingredient package can now pass only when direct icon, species, slot level, public candidate and operation all agree.
...
assert.equal(ingredientPackagePreflight.status,'MATCH');
assert.equal(ingredientPackagePreflight.safe_for_sqlite_apply,true);
const ingredientMismatch=structuredClone(ingredientPackage);ingredientMismatch.update_id='VISUAL-INGREDIENT-CONFLICT';ingredientMismatch.pokemon_visual_evidence_manifest=buildPokemonVisualEvidenceManifest([{
  ...ingredientManifest.observations[0],ingredients:[ev('INGREDIENT_VISUAL','嫩亮酪梨','ability.png',0.99,1)],
}]);ingredientMismatch.operations[1].data.ingredient_name='嫩亮酪梨';
const ingredientMismatchPreflight=evaluatePokemonVisualUpdatePackage(ingredientMismatch);
assert.equal(ingredientMismatchPreflight.status,'CONFLICT');
assert.throws(()=>assertPokemonVisualUpdatePackageSafe(ingredientMismatch),/POKEMON_VISUAL_UPDATE_PREFLIGHT_BLOCKED:CONFLICT/);

// Legacy packages remain compatible; 75 re-audit contract remains manifest-required.
...
console.log(JSON.stringify({status:'PASS',gate:'POKEMON_VISUAL_EVIDENCE_CONSISTENCY_AND_UPDATE_PREFLIGHT',schema:POKEMON_VISUAL_EVIDENCE_SCHEMA,contract_version:POKEMON_VISUAL_EVIDENCE_VERSION,public_relation_generates_player_observation:false,type_berry_independent_observation:true,type_berry_conflict_fail_closed:true,ingredient_current_canonical:'嫩亮酪梨',legacy_avocado_requires_review:true,species_ingredient_public_authority:'ACTIVE_EXACT_SLOT_SPECIFIC_CONSISTENCY_ONLY',unknown_species_review_required:true,ambiguous_equivalent_forms_without_guess:true,finite_skill_vocabulary_exact:true,screenshot_bridge_manifest:true,operation_binding_required:true,legacy_update_package_compatible:true,reaudit_manifest_required:true,safe_sqlite_apply_requires_match:true},null,2));

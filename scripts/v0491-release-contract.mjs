...
assert.equal(version.match(/app_version:\s*'([^']+)'/)?.[1],'v0.4.9.1');
...
assert.ok(version.includes("// app_version: 'v0.4.9'"),'v0.4.9.1 must retain v0.4.9 legacy parser bridge');
assert.ok(version.includes("// app_version: 'v0.4.8.5'"),'v0.4.9.1 must retain v0.4.8.5 legacy parser bridge');
...
])assert.ok(pack.includes(token),`v0.4.9.1 pack contract missing ${token}`);
assert.equal(pack.includes('current:Number(req.current??req.available??0)'),false,'v0.4.9.1 must not fabricate recipe current=0');

const ui=read('assets/js/war-room-strategy-analysis-pack-ui.js');
for(const token of [
  '分享提醒','只提供給你信賴的 AI 模型／服務','不建議公開張貼','離線使用','SQLite/WASM','分享 Prompt',
  'shareStrategyPromptText','COPIED_FALLBACK','Permission denied',
]){
  if(token==='Permission denied')continue;
  assert.ok(ui.includes(token),`v0.4.9.1 UI missing ${token}`);
}
assert.equal(ui.includes('new File('),false,'v0.4.9.1 share path must not require file sharing');
assert.equal(ui.includes('navigator.canShare'),false,'v0.4.9.1 share path must not depend on file canShare');
assert.equal(ui.includes('applyPayload('),false,'v0.4.9.1 UI must not contain direct Apply');

let copied='';
const permissionDenied=Object.assign(new Error('Permission denied'),{name:'NotAllowedError'});
const fallback=await shareStrategyPromptText('trusted prompt',{
  shareFn:async()=>{throw permissionDenied;},
  writeTextFn:async value=>{copied=value;},
});
assert.equal(fallback.status,'COPIED_FALLBACK');
assert.equal(copied,'trusted prompt');
let copiedOnAbort=false;
const aborted=await shareStrategyPromptText('prompt',{
  shareFn:async()=>{throw Object.assign(new Error('cancel'),{name:'AbortError'});},
  writeTextFn:async()=>{copiedOnAbort=true;},
});
assert.equal(aborted.status,'ABORTED');
assert.equal(copiedOnAbort,false);
assert.equal((await shareStrategyPromptText('prompt')).status,'UNAVAILABLE');
...
assert.equal(migrations.includes('VALUES(10,'),false,'v0.4.9.1 must not add SQLite migration 10');
const sw=read('service-worker.js');
assert.ok(sw.includes("importScripts('./assets/js/version-authority.js')"));
assert.ok(sw.includes("url.pathname.endsWith('.js')"),'offline contract remains online-load-once dynamic JS runtime caching');

console.log(JSON.stringify({
  status:'PASS',gate:'V0.4.9.1_RELEASE_CONTRACT',app_version:'v0.4.9.1',
  pack_version:STRATEGY_ANALYSIS_PACK_VERSION,prompt_version:STRATEGY_ANALYSIS_PROMPT_VERSION,sharing_notice_version:STRATEGY_ANALYSIS_SHARING_NOTICE_VERSION,
  recipe_semantic_integrity:true,candidate_reference_closure:true,trusted_ai_notice:true,offline_prerequisite_visible:true,
  android_share_permission_fallback:true,file_share_required:false,direct_apply_path:false,sqlite_migration_added:false,
},null,2));

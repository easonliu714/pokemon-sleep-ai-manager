...
currentPhase='worker-isolation-lifecycle';
{
  ...
  assert.match(authority,/app_version\s*:\s*'v\d+(?:\.\d+){2,}'/);assert.match(bootstrap,/const APP_VERSION=authority\.app_version/);assert.match(bootstrap,/const VERSION=authority\.app_build/);assert.match(sw,/cache_name:CACHE/);assert.match(index,/src="\.\/assets\/js\/bootstrap\.js"/);assert.doesNotMatch(index,/bootstrap\.js\?v=/);

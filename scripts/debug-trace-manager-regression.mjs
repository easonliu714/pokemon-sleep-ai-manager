...
const paths=['assets/js/debug-trace-manager.js','assets/js/bootstrap.js','assets/js/android-import-file-picker.js','assets/js/identity-import-wizard-entry.js','assets/js/data1-zip-inventory.js','assets/js/data1-inventory-review.js','assets/js/data1-inventory-review-ui.js','assets/js/data1-image-fingerprint.js','assets/js/data1d-ocr-first-classifier.js','assets/js/data1d-local-ocr-runtime.js','service-worker.js'];
...
const [tracePath,bootstrapPath,pickerPath,wizardPath,inventoryPath,reviewPath,reviewUiPath,fingerprintPath,ocrClassifierPath,ocrRuntimePath,workerPath]=paths;
const trace=fs.readFileSync(tracePath,'utf8');const bootstrap=fs.readFileSync(bootstrapPath,'utf8');const picker=fs.readFileSync(pickerPath,'utf8');const wizard=fs.readFileSync(wizardPath,'utf8');const inventory=fs.readFileSync(inventoryPath,'utf8');const review=fs.readFileSync(reviewPath,'utf8');const reviewUi=fs.readFileSync(reviewUiPath,'utf8');const fingerprint=fs.readFileSync(fingerprintPath,'utf8');const ocrClassifier=fs.readFileSync(ocrClassifierPath,'utf8');const ocrRuntime=fs.readFileSync(ocrRuntimePath,'utf8');const worker=fs.readFileSync(workerPath,'utf8');
...
if(!bootstrap.includes("APP_VERSION = 'v0.3.43'"))throw new Error('app_version_not_bumped');
if(!bootstrap.includes('20260802-data1d-local-ocr-runtime'))throw new Error('build_not_bumped');
for(const token of ['data1-zip-inventory.js','data1-inventory-review.js','data1-inventory-review-ui.js','data1-image-fingerprint.js','data1d-ocr-first-classifier.js','data1d-local-ocr-runtime.js'])if(!bootstrap.includes(token))throw new Error(`module_not_probed:${token}`);

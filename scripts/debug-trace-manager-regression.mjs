...
requireTokens(source.reviewUi,['createInventoryReviewWorkbench','duplicate_gate_decision_applied','fingerprint_manifest_exported','review_package_exported'],'review_ui');
requireTokens(source.fingerprint,['sha256Hex','SHA-256','enrichInventoryWithFingerprints','duplicate_group_id'],'fingerprint');
requireTokens(source.classifier,['PokemonSleepOCR','chi_tra+eng','classification_status','classification_confidence','requires_review','ocr_first_ai_opt_in_only','recognizeRegion','was_cancelled'],'ocr_classifier');
requireTokens(source.runtime,['Tesseract','5.1.1','chi_tra','eng','ocr_runtime_loading','ocr_runtime_progress','ocr_runtime_ready','ocr_runtime_failed','offline_after_first_load','workerPromise'],'ocr_runtime');
requireTokens(source.runtimeUi,['ocr_runtime_loading','ocr_runtime_progress','ocr_runtime_ready','ocr_runtime_failed','pokemon-sleep:ocr-cancel-requested','ocr_preprocess_completed','canvas','grayscale','threshold'],'ocr_ui');
requireTokens(source.reviewPackage,['OCR_REVIEW_SCHEMA','buildOcrReviewQueue','buildPrivateOcrReviewPackage','downloadPrivateOcrReviewPackage','contains_image_bytes:false','contains_ocr_full_text:false'],'ocr_review_package');
requireTokens(source.regionConsent,['OCR_REGION_PRESETS','buildRegionConfig','buildAiConsentQueue','validateAiConsent','explicit_consent_required','contains_image_bytes:false','contains_api_key:false'],'ocr_region_ai_consent');
requireTokens(source.regionUi,['createOcrRegionAiReviewPanel','ocrRegionPreset','ocrAiConsent','ocrAiUploadAck','prepareAiReviewBtn','ai_review_queue_ready'],'ocr_region_ui');
requireTokens(source.thumbnail,['OcrThumbnailUrlPool','URL.createObjectURL','URL.revokeObjectURL','releaseAll','normalizeRegionConfidence','buildRegionConfidenceSummary'],'ocr_thumbnail_confidence');
...
if(/fetch\s*\(|XMLHttpRequest/.test(source.regionConsent)||/fetch\s*\(|XMLHttpRequest/.test(source.regionUi)||/fetch\s*\(|XMLHttpRequest/.test(source.thumbnail)||/fetch\s*\(|XMLHttpRequest/.test(source.overlayBootstrap))throw new Error('ocr_local_modules_must_not_upload_directly');
if(/localStorage/.test(source.aiSettings)||/localStorage/.test(source.thumbnail)||/localStorage/.test(source.overlayBootstrap))throw new Error('browser_secret_or_ocr_module_must_not_use_local_storage');

...
const paths=['assets/js/debug-trace-manager.js','assets/js/bootstrap.js','assets/js/android-import-file-picker.js','assets/js/identity-import-wizard-entry.js','assets/js/data1-zip-inventory.js','assets/js/data1-inventory-review.js','assets/js/data1-inventory-review-ui.js','service-worker.js'];
...
const [tracePath,bootstrapPath,pickerPath,wizardPath,inventoryPath,reviewPath,reviewUiPath,workerPath]=paths;
const trace=fs.readFileSync(tracePath,'utf8');const bootstrap=fs.readFileSync(bootstrapPath,'utf8');const picker=fs.readFileSync(pickerPath,'utf8');const wizard=fs.readFileSync(wizardPath,'utf8');const inventory=fs.readFileSync(inventoryPath,'utf8');const review=fs.readFileSync(reviewPath,'utf8');const reviewUi=fs.readFileSync(reviewUiPath,'utf8');const worker=fs.readFileSync(workerPath,'utf8');
...
for(const token of ['createInventoryReviewWorkbench','inventory_review_batch_applied','reviewed_manifest_exported','review_package_exported'])if(!reviewUi.includes(token))throw new Error(`review_ui_contract_missing:${token}`);
...
if(!bootstrap.includes("APP_VERSION = 'v0.3.40'"))throw new Error('app_version_not_bumped');
if(!bootstrap.includes('20260801-data1b-inventory-review'))throw new Error('build_not_bumped');
if(!bootstrap.includes('data1-zip-inventory.js'))throw new Error('inventory_not_probed');
if(!bootstrap.includes('data1-inventory-review.js')||!bootstrap.includes('data1-inventory-review-ui.js'))throw new Error('review_modules_not_probed');
...
if(!worker.includes("'./assets/js/data1-inventory-review.js'")||!worker.includes("'./assets/js/data1-inventory-review-ui.js'"))throw new Error('review_modules_not_precached');
if(!worker.includes('v0.3.40-data1b-inventory-review'))throw new Error('service_worker_cache_not_bumped');
if(/\.content\b|\.payload\b/.test(trace.match(/function safeFile[\s\S]*?\n}/)?.[0]||''))throw new Error('file_content_must_not_be_exported');
console.log(JSON.stringify({ok:true,trace_schema:'pokemon-sleep-debug-trace/1.1',version:'v0.3.40',checks:requiredTraceTokens.length+pickerTokens.length+24}));

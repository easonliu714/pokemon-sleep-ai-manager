...
const paths=['assets/js/debug-trace-manager.js','assets/js/bootstrap.js','assets/js/android-import-file-picker.js','assets/js/identity-import-wizard-entry.js','assets/js/data1-zip-inventory.js','assets/js/data1-inventory-review.js','assets/js/data1-inventory-review-ui.js','assets/js/data1-image-fingerprint.js','assets/js/data1d-ocr-first-classifier.js','service-worker.js'];
...
const [tracePath,bootstrapPath,pickerPath,wizardPath,inventoryPath,reviewPath,reviewUiPath,fingerprintPath,ocrClassifierPath,workerPath]=paths;
const trace=fs.readFileSync(tracePath,'utf8');const bootstrap=fs.readFileSync(bootstrapPath,'utf8');const picker=fs.readFileSync(pickerPath,'utf8');const wizard=fs.readFileSync(wizardPath,'utf8');const inventory=fs.readFileSync(inventoryPath,'utf8');const review=fs.readFileSync(reviewPath,'utf8');const reviewUi=fs.readFileSync(reviewUiPath,'utf8');const fingerprint=fs.readFileSync(fingerprintPath,'utf8');const ocrClassifier=fs.readFileSync(ocrClassifierPath,'utf8');const worker=fs.readFileSync(workerPath,'utf8');
...
const pickerTokens=['IMAGE_ACCEPT','ZIP_ACCEPT','tech2dImageInput','tech2dZipInput','選擇圖片','選擇 ZIP','single_zip_per_batch_required','一次只能選擇一個 ZIP','import_source_inspection','file_selection_received','image_fingerprint_progress','buildPrivateZipInventory','enrichInventoryWithFingerprints'];
...
if(!bootstrap.includes("APP_VERSION = 'v0.3.42'"))throw new Error('app_version_not_bumped');
if(!bootstrap.includes('20260801-data1d-ocr-first'))throw new Error('build_not_bumped');
for(const token of ['data1-zip-inventory.js','data1-inventory-review.js','data1-inventory-review-ui.js','data1-image-fingerprint.js','data1d-ocr-first-classifier.js'])if(!bootstrap.includes(token))throw new Error(`module_not_probed:${token}`);
for(const token of ['./assets/js/debug-trace-manager.js','./assets/js/data1-zip-inventory.js','./assets/js/data1-inventory-review.js','./assets/js/data1-inventory-review-ui.js','./assets/js/data1-image-fingerprint.js','./assets/js/data1d-ocr-first-classifier.js'])if(!worker.includes(token))throw new Error(`module_not_precached:${token}`);
if(!worker.includes('v0.3.42-data1d-ocr-first'))throw new Error('service_worker_cache_not_bumped');
if(/\.content\b|\.payload\b/.test(trace.match(/function safeFile[\s\S]*?\n}/)?.[0]||''))throw new Error('file_content_must_not_be_exported');
console.log(JSON.stringify({ok:true,trace_schema:'pokemon-sleep-debug-trace/1.1',version:'v0.3.42',checks:requiredTraceTokens.length+pickerTokens.length+48}));

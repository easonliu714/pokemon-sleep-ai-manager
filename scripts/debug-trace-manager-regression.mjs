...
const paths=['assets/js/debug-trace-manager.js','assets/js/bootstrap.js','assets/js/android-import-file-picker.js','assets/js/identity-import-wizard-entry.js','service-worker.js'];
...
const trace=fs.readFileSync(paths[0],'utf8');
const bootstrap=fs.readFileSync(paths[1],'utf8');
const picker=fs.readFileSync(paths[2],'utf8');
const wizard=fs.readFileSync(paths[3],'utf8');
const worker=fs.readFileSync(paths[4],'utf8');
...
const pickerTokens=['IMAGE_ACCEPT','ZIP_ACCEPT','tech2dImageInput','tech2dZipInput','選擇圖片','選擇 ZIP','single_zip_per_batch_required','一次只能選擇一個 ZIP','import_source_inspection','file_selection_received','zip_inspection_completed'];
...
if(!bootstrap.includes("APP_VERSION = 'v0.3.38'"))throw new Error('app_version_not_bumped');
if(!bootstrap.includes('20260801-core-debug2-import-ux'))throw new Error('build_not_bumped');
if(!worker.includes("'./assets/js/debug-trace-manager.js'"))throw new Error('debug_trace_not_precached');
if(!worker.includes('v0.3.38-core-debug2-import-ux'))throw new Error('service_worker_cache_not_bumped');
if(/\.content\b|\.payload\b/.test(trace.match(/function safeFile[\s\S]*?\n}/)?.[0]||''))throw new Error('file_content_must_not_be_exported');
console.log(JSON.stringify({ok:true,trace_schema:'pokemon-sleep-debug-trace/1.1',version:'v0.3.38',checks:requiredTraceTokens.length+pickerTokens.length+8}));

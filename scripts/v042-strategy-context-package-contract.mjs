...
const weeklyContext={week_start:'2026-08-10',camp:'天青沙灘',dish_category:'甜點／飲料',favorite_berry_1:'橙橙果',favorite_berry_2:'葡萄果',favorite_berry_3:'桃桃果',pot_size:57,event_name:'測試活動',event_effects:'SECRET_EVENT_TEXT：技能發動率提升'};
...
assert(STRATEGY_CONTEXT_PACKAGE_VERSION==='strategy-context-2026-08-09-a','package_version');
...
for(const forbidden of ['private-pokemon-id-001','private-instance-id-001','SECRET_NICKNAME','SECRET_NOTES','SHOULD_NEVER_APPEAR','private-goal-id'])assert(!payloadText.includes(forbidden),`private_value_leaked:${forbidden}`);
for(const key of ['pokemon_id','pokemon_instance_id','nickname','notes','api_key','source_image_ref','observed_json','ocr_text'])assert(!payloadText.includes(`\"${key}\"`),`private_key_leaked:${key}`);
...
console.log(JSON.stringify({status:'PASS',schema:'pokemon-sleep-strategy-context-package-contract/1.0',candidate_refs_ephemeral:true,stable_pokemon_ids_in_payload:false,event_text_default:false,event_text_explicit_optin:true,inventory_minimized:true,unknown_gemini_refs_rejected:true,direct_apply_allowed:false,preview_calls_provider:false,player_data_write:false},null,2));

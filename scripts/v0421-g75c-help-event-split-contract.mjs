...
assert.ok(['v0.4.21','v0.4.22'].includes(appVersion),`unexpected G7.5C release/successor ${appVersion}`);
if(appVersion==='v0.4.22')assert.ok(version.includes("// app_version: 'v0.4.21'"),'v0.4.22 must retain v0.4.21 lineage bridge');
...
if(baseOutputSuccessor){assert.deepEqual(registry.rules.berry_output_per_help.missing_inputs,[]);assert.ok(String(registry.rules.berry_output_per_help.scope).includes('REGULAR_BERRY_RESULT_HELP'));}
else assert.deepEqual(registry.rules.berry_output_per_help.missing_inputs,['verified_base_berry_output_per_berry_result_help_rule']);
...
const candidateFeatures={candidates:[
  makeCandidate('private-fire','火',30,'能量填充S',true,'火辣香草','樹果'),
  makeCandidate('private-water','水',60,'食材獲取S',true,'哞哞鮮奶','食材'),
  makeCandidate('private-electric','電',70,'活力全體療癒S',false,'甜甜蜜','技能'),
]};
...
assert.ok(['pokemon-sleep-production-evidence-snapshot/1.2','pokemon-sleep-production-evidence-snapshot/1.3'].includes(snapshot.schema));
...
assert.equal(snapshot.summary.active_numeric_dimension_count,baseOutputSuccessor?3:2);
assert.equal(snapshot.summary.blocked_numeric_dimension_count,baseOutputSuccessor?4:5);
...
if(baseOutputSuccessor){
  assert.equal(berryOutputRow.authority_status,'ACTIVE_VERIFIED');
  assert.equal(berryOutputRow.evidence_status,EVIDENCE_STATUS.ACTIVE_VERIFIED_LOCAL_NUMERIC_CONTRACT);
  assert.equal(berryOutputRow.runtime_numeric_activation,true);
  assert.equal(berryOutputRow.coverage.observed_count,3);
  assert.deepEqual(berryOutputRow.blocking_reasons,[]);
}else{
...
const member=projectMemberProductionEvidence(makeCandidate('p-fire','火',30,'能量填充S',true,'火辣香草','樹果'),{productionRegistry:registry});
...
if(baseOutputSuccessor){assert.equal(member.berry_output_per_help_status,'ACTIVE_VERIFIED');assert.equal(member.berry_output_per_regular_berry_result_help,2);assert.equal(member.favorite_adjusted_berry_energy_per_regular_berry_result_help,224);}
assert.equal(member.berry_rate_status,'NOT_YET_VERIFIED');assert.equal(member.berry_energy_per_hour,null);
...
assert.equal(objective.objective_status,'NUMERIC_MODEL_NOT_ACTIVE');assert.equal(objective.objective_score,null);assert.equal(objective.components.berry_energy_per_hour,null);

const ui=read('assets/js/production-evidence-ui.js');for(const token of ['G7.5 產能模型','evidence-metric-grid','已啟用數值','待補數值','事件分流','進階 Evidence / JSON','完整 Snapshot JSON'])assert.ok(ui.includes(token),`compact decision UI token missing ${token}`);assert.equal(ui.includes('安全邊界：</b>局部 ACTIVE_VERIFIED'),false,'long safety paragraph must not remain in default result UI');assert.ok(ui.indexOf('進階 Evidence / JSON')<ui.indexOf('Evidence · <code>'),'verbose Evidence/Authority should live behind advanced details');
for(const file of ['assets/js/help-event-split-contract.js','assets/js/production-authority-registry.js','assets/js/production-evidence-registry.js']){const source=read(file);for(const forbidden of ['INSERT INTO','UPDATE pokemon','UPDATE ingredient_inventory','DELETE FROM','applyPayload(','dryRun(','fetch('])assert.equal(source.includes(forbidden),false,`${file} owns forbidden mutation/network path: ${forbidden}`);}
const sw=read('service-worker.js');assert.ok(sw.includes("'./assets/js/help-event-split-contract.js'"),'first-offline precache missing help event split contract');if(baseOutputSuccessor)assert.ok(sw.includes("'./assets/js/base-berry-output-contract.js'"),'v0.4.22 first-offline precache missing base berry output contract');

console.log(JSON.stringify({status:'PASS',gate:'V0421_G75C_HELP_EVENT_SPLIT_GOVERNANCE_SUCCESSOR_AWARE',app_version:appVersion,help_event_split_status:registry.rules.help_event_split.status,berry_output_numeric_status:registry.rules.berry_output_per_help.status,active_numeric_dimension_count:snapshot.summary.active_numeric_dimension_count,blocked_numeric_dimension_count:snapshot.summary.blocked_numeric_dimension_count,structural_verified_dimension_count:snapshot.summary.structural_verified_dimension_count,berry_energy_per_hour:null,overall_numeric_model_status:registry.numeric_rate_model_status,compact_decision_ui:true,verbose_evidence_collapsed:true,base_output_successor:baseOutputSuccessor,player_write:false,sqlite_write:false,runtime_network_fetch:false,ai_numeric_authority:false},null,2));

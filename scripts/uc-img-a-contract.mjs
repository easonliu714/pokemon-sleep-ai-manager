...
// UC.IMG-A is a historical behavior contract; newer successor builds keep the same prefix
// but legitimately advance the embedded authority date.
...
const ingredientPayload=basePayload('ingredient_inventory_update',[op('ingredient_inventory',{ingredient_name:'甜甜蜜'},{quantity:113},'image-002')]);let result=validateScreenshotScenarioPayload(session,'ingredients',ingredientPayload);assert.equal(result.errors.length,0,result.errors.join('\n'));assert.equal(result.review.length,0);assert.equal(result.summary.traceable_evidence,true);assert.equal(result.summary.coverage,'USER_CONFIRMED_COMPLETE');assert.equal(result.payload.operations.length,1);

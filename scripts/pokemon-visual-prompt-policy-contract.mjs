...
assert.equal(SCREENSHOT_PROMPT_SAFETY_VERSION,'screenshot-prompt-safety-2026-08-15-a');
assert.equal(POKEMON_VISUAL_PROMPT_POLICY_VERSION,'pokemon-visual-prompt-policy-2026-08-15-a');
...
for(const edge of ['TYPE_VISUAL->BERRY_VISUAL','BERRY_VISUAL->TYPE_VISUAL','PUBLIC_CANDIDATES->INGREDIENT_VISUAL','EDITABLE_HEADER_NAME->CANONICAL_SPECIES','FILENAME->PLAYER_OBSERVATION'])assert.ok(POKEMON_VISUAL_FORBIDDEN_INFERENCE_EDGES.includes(edge));
...
for(const token of [POKEMON_VISUAL_PROMPT_POLICY_VERSION,POKEMON_VISUAL_EVIDENCE_VERSION,'observation_basis','inference_used','profile.header_name_text','特選蘋果','好眠番茄','Lv30 / Lv60'])assert.ok(externalPrompt.includes(token),`external prompt missing ${token}`);

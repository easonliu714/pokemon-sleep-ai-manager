...
assert.match(bootstrap,/APP_VERSION = 'v0\.3\.39'/);assert.match(bootstrap,/20260801-data1a-zip-inventory/);assert.match(bootstrap,/data1-zip-inventory\.js/);assert.match(bootstrap,/debug-trace-manager\.js/);assert.match(bootstrap,/identity-import-wizard-entry\.js/);assert.match(bootstrap,/pokemon-screenshot-grouping\.js/);assert.match(bootstrap,/pokemon-zip-manifest\.js/);assert.match(bootstrap,/identity-import-transaction\.js/);assert.match(bootstrap,/jszip-loader\.js/);assert.match(bootstrap,/android-import-file-picker\.js/);
assert.match(sw,/pokemon-sleep-ai-v0\.3\.39-data1a-zip-inventory/);assert.match(sw,/data1-zip-inventory\.js/);assert.match(sw,/debug-trace-manager\.js/);assert.match(sw,/identity-import-pipeline\.js/);assert.match(sw,/pokemon-screenshot-grouping\.js/);assert.match(sw,/pokemon-zip-manifest\.js/);assert.match(sw,/identity-import-transaction\.js/);assert.match(sw,/identity-import-wizard-entry\.js/);assert.match(sw,/jszip@3\.10\.1\/dist\/jszip\.min\.js/);assert.match(sw,/android-import-file-picker\.js/);
...
assert.match(wizardEntry,/pokemon-sleep:identity-import-files-selected/);assert.match(wizardEntry,/tech2dFilePickerSlot/);assert.match(wizardEntry,/tech2d-file-picker-actions/);assert.match(wizardEntry,/匯出私人清點 Manifest/);assert.match(wizardEntry,/min-height:44px/);
console.log('PASS identity guard, DATA.1 private ZIP inventory manifest, Android split picker, offline PWA, and Debug Trace contracts');

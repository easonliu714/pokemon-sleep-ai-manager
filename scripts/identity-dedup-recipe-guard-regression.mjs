...
// Debug Trace Manager bumps the shell version while preserving all TECH.2D contracts below.
assert.match(bootstrap,/APP_VERSION = 'v0\.3\.37'/);
assert.match(bootstrap,/20260801-debug-trace-manager/);
assert.match(bootstrap,/debug-trace-manager\.js/);
...
assert.match(bootstrap,/jszip-loader\.js/);
assert.match(bootstrap,/android-import-file-picker\.js/);
assert.match(sw,/pokemon-sleep-ai-v0\.3\.37-debug-trace-manager/);
assert.match(sw,/debug-trace-manager\.js/);
...
assert.match(sw,/android-import-file-picker\.js/);
...
assert.match(picker,/accept=ACCEPT|input\.accept=ACCEPT/);
assert.match(picker,/input\.multiple=true/);
assert.match(picker,/mixed_zip_and_images_not_allowed/);
assert.match(picker,/single_zip_per_batch_required/);
assert.match(picker,/createAndroidImportFilePicker/);
assert.match(wizardEntry,/pokemon-sleep:identity-import-files-selected/);
assert.match(wizardEntry,/tech2dFilePickerSlot/);
assert.match(wizardEntry,/min-height:44px/);
console.log('PASS profile completeness identity guard, TECH.2D Android ZIP/offline PWA, and Debug Trace shell contracts');

...
import {PUBLIC_RECIPE_MASTER_VERSION} from '../assets/js/public-recipe-master.js';
...
// Simulate two known pre-v0.4.2 public-master shapes and one unrecognized row.
...
db.run(`INSERT INTO recipe_master(recipe_id,category,recipe_name,base_energy,total_ingredients,source_type,source_name,source_ref,verified_at,data_version)
  VALUES(?,?,?,?,?,?,?,?,?,?)`,['curry_dream_eater','咖哩／濃湯','絕對睡眠奶油咖哩',9010,55,'legacy','shared-22','legacy:shared',null,'legacy-shared']);
db.run(`INSERT INTO recipe_master(recipe_id,category,recipe_name,base_energy,total_ingredients,source_type,source_name,source_ref,verified_at,data_version)
  VALUES(?,?,?,?,?,?,?,?,?,?)`,['future_unknown_recipe','沙拉','未辨識保留料理',1,1,'future','unknown-fixture','fixture:unknown',null,'future-version']);

// Player state intentionally uses the old IDs/names and must survive unchanged.
...
db.run(`INSERT INTO recipes(recipe_id,category,recipe_name,unlocked,total_ingredients,source,recipe_level,current_energy,updated_at,notes)
  VALUES(?,?,?,?,?,?,?,?,?,?)`,['curry_dream_eater','咖哩／濃湯','絕對睡眠奶油咖哩',1,55,'player',8,9999,'2026-08-02T00:00:00+08:00','keep-dream']);
...
assert(dreamMaster?.recipe_name==='夢食奶油咖哩','legacy_shared_name_not_canonicalized');
...
assert(Number(ninjaView?.unlocked)===1&&Number(ninjaView?.recipe_level)===12&&Number(ninjaView?.current_energy)===12345,'legacy_player_state_projection_mismatch');
const dreamView=one('SELECT * FROM recipe_catalog_state WHERE recipe_id=?',['curry_dream_eater']);
assert(dreamView?.player_recipe_id==='curry_dream_eater','shared_player_id_not_preserved');
assert(Number(dreamView?.unlocked)===1&&Number(dreamView?.recipe_level)===8&&Number(dreamView?.current_energy)===9999,'shared_player_state_projection_mismatch');
...
console.log(JSON.stringify({
  status:'PASS',
  schema:'pokemon-sleep-recipe-sync-preservation-contract/1.0',
  ...
  legacy_shared_name_canonicalized:true,

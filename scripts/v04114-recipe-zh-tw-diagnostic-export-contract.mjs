...
if(additions.length){
  ...
  assert.deepEqual(new Set(additions.map(row=>row.recipe_id)),new Set(['curry_greengrass_bun','curry_bounce_udon']));
}

const expectedCurrentNames=new Map([
  ['curry_soft_corn','柔軟玉米濃湯'],['recipe_curry_015','入口即化蛋捲咖哩'],['recipe_curry_010','單純白醬濃湯'],['recipe_curry_011','豆製肉排咖哩'],
...
const legacyDizzy=resolvePublicRecipeName('暈眩拳辣味咖哩');assert.equal(legacyDizzy?.recipe_id,'curry_dizzy_punch');assert.equal(legacyDizzy?.recipe_name,'迷昏拳辣味咖哩');assert.equal(legacyDizzy?.commit_allowed,true);
...
const diagnostic=buildUcImgDiagnosticBundle({appVersion:'v0.4.11.3',session:{session_id:'ucimg-test'},scenarioKey:'weekly',config:{scenario:'weekly_context_update'},coverage:'USER_CONFIRMED_COMPLETE',rawResponse:JSON.stringify({schema_version:'1.1'}),validation:{ok:true,errors:[],warnings:[],review:[],summary:{}},providerMeta:{provider:'gemini',model:'gemini-test',project_alias:'Project A',response_contract:'update-package-v1.1',image_count:2,platform_authority:authorityAudit}});
assert.deepEqual(diagnostic.platform_authority,authorityAudit);assert.equal(diagnostic.safety.api_key_included,false);assert.equal(diagnostic.safety.screenshot_bytes_included,false);assert.equal(diagnostic.safety.sqlite_export_included,false);

let downloadedBlob=null,clicked=false,downloadName=null,revoked=false;
const fakeDocument={body:{appendChild(){}},createElement(tag){assert.equal(tag,'a');return {style:{},set href(value){this._href=value;},get href(){return this._href;},set download(value){downloadName=value;},click(){clicked=true;},remove(){}};}};
...
assert.equal(clicked,true);assert.equal(downloadName,filename);assert.ok(filename.endsWith('.json'));
const downloaded=JSON.parse(await downloadedBlob.text());assert.equal(downloaded.schema,'pokemon-sleep-uc-img-ai-diagnostic/1.0');assert.equal(downloaded.safety.api_key_included,false);assert.equal(downloaded.safety.screenshot_bytes_included,false);assert.equal(downloaded.safety.sqlite_export_included,false);
await new Promise(resolve=>setTimeout(resolve,0));assert.equal(revoked,true);

...
const diagnostic=buildUcImgDiagnosticBundle({appVersion:'v0.4.13.3',session:{session_id:'ucimg-test',entries:[],scenario_state:{weekly:{last_ai_error:null}}},scenarioKey:'weekly',config:{scenario:'weekly_context_update'},coverage:'USER_CONFIRMED_COMPLETE',rawResponse:JSON.stringify({schema_version:'1.1'}),validation:{ok:true,errors:[],warnings:[],review:[],summary:{}},providerMeta:{provider:'gemini',model:'gemini-test',project_alias:'Project A',response_contract:'update-package-v1.1',image_count:2,platform_authority:authorityAudit},debugTrace:{events:[]}});
assert.equal(diagnostic.schema,UC_IMG_DIAGNOSTIC_SCHEMA);assert.deepEqual(diagnostic.platform_authority,authorityAudit);assert.equal(diagnostic.safety.api_key_included,false);assert.equal(diagnostic.safety.screenshot_bytes_included,false);assert.equal(diagnostic.safety.sqlite_export_included,false);
...
const filename=downloadUcImgDiagnosticJson(diagnostic,{documentRef:fakeDocument,urlApi:fakeUrl,BlobCtor:Blob,filename:buildUcImgDiagnosticFilename({appVersion:'v0.4.13.3',scenarioKey:'weekly',sessionId:'ucimg-test',generatedAt:'2026-08-12T07:00:00Z'})});
assert.equal(clicked,true);assert.equal(downloadName,filename);assert.ok(filename.endsWith('.json'));
const downloaded=JSON.parse(await downloadedBlob.text());assert.equal(downloaded.schema,UC_IMG_DIAGNOSTIC_SCHEMA);assert.equal(downloaded.safety.api_key_included,false);assert.equal(downloaded.safety.screenshot_bytes_included,false);assert.equal(downloaded.safety.sqlite_export_included,false);

...
assert.equal(version.match(/app_version:\s*'([^']+)'/)?.[1],'v0.4.8.2');
assert.equal(version.match(/app_build:\s*'([^']+)'/)?.[1],'20260810-v0482-evolution-coverage-evidence-diagnostic');
assert.equal(version.match(/cache_name:\s*'([^']+)'/)?.[1],'pokemon-sleep-ai-v0.4.8.2-v0482-evolution-coverage-evidence-diagnostic');

assert.equal(PUBLIC_POKEMON_KNOWLEDGE_VERSION,'pokemon-knowledge-2026-08-10-d');
assert.equal(PUBLIC_CANDY_MASTER_VERSION,'public-candy-master-2026-08-10-c');
...
assert.equal(bundle.manifest.evolution_route_rows,78);
assert.equal(bundle.manifest.evolution_from_species_rows,69);
assert.equal(bundle.manifest.evolution_verified_terminal_rows,67);
...
assert.equal(migrations.includes('VALUES(10,'),false,'v0.4.8.2 must not add SQLite migration 10');

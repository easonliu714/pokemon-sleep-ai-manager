...
}else assert.equal(PUBLIC_CANDY_DISPLAY_NAME_AUTHORITY_POLICY.exact_official_zh_tw_string_required,true);
if(p0b6Successor){
...
if(p0b6Successor){
  ...
  for(const displayName of p0b6RealDeviceNames){const row=rows.find(item=>item.candy_display_name===displayName);assert.ok(row);assert.match(row.source_ref,/^project-evidence:2026-09-01-p0b6-real-device-inventory-revalidation#/);}
  for(const species of ['喵喵','穿山鼠']){assert.equal(resolvePublicCandyDisplayNameForSpecies(species).status,'REVIEW_REQUIRED');assert.equal(resolveCandyFamilyStorageForSpecies(species).status,'REVIEW_REQUIRED');}
  withLocalAdmissions(['喵喵的糖果','穿山鼠的糖果'],()=>{
    for(const [species,name] of [['喵喵','喵喵的糖果'],['穿山鼠','穿山鼠的糖果']]){
      const display=resolvePublicCandyDisplayNameForSpecies(species);assert.equal(display.status,'MATCH');assert.equal(display.candy_display_name,name);assert.equal(display.local_admission_authority,true);assert.equal(display.reason,'EXACT_USER_CONFIRMED_LOCAL_ZH_TW_CANDY_DISPLAY_NAME');assert.equal(display.automatic_display_name_generation,false);
      const storage=resolveCandyFamilyStorageForSpecies(species);assert.equal(storage.status,'MATCH',`${species} local admission must reach P0-B6 canonical storage`);assert.equal(storage.canonical_candy_display_name,name);assert.match(storage.family_id,/^family_/);
    }
    assert.equal(PUBLIC_CANDY_DISPLAY_NAME_AUTHORITY_ROWS.some(row=>row.candy_display_name==='喵喵的糖果'),true);assert.equal(PUBLIC_CANDY_DISPLAY_NAME_AUTHORITY_ROWS.some(row=>row.candy_display_name==='穿山鼠的糖果'),true);
  });
...
assert.match(displayAuthoritySource,/species_name_concatenation_forbidden:true/);assert.match(displayAuthoritySource,/automatic_display_name_generation:false/);assert.match(displayAuthoritySource,/publicCandyLocalAdmissionRows/,'B4 may consume validated local admissions dynamically');
const professorSource=read('assets/js/pokemon-professor-transfer.js');assert.match(professorSource,/USER_DIRECT_OBSERVATION_ONLY/);assert.equal(professorSource.includes('public-candy-display-name-authority.js'),false);
...
else if(p0b6Successor){assert.ok(versionSource.includes("// app_version: 'v0.4.27.55'"));assert.ok(versionSource.includes("// app_build: '20260901-v042755-p0b6-candy-family-storage-reconciliation'"));assert.ok(versionSource.includes("// cache_name: 'pokemon-sleep-ai-v0.4.27.55-v042755-p0b6-candy-family-storage-reconciliation'"));assert.equal(PUBLIC_CANDY_MASTER_VERSION,'public-candy-master-2026-09-01-g');}

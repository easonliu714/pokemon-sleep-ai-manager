...
assert.equal(
  PUBLIC_CANDY_DISPLAY_NAME_AUTHORITY_VERSION,
  p0b6Successor?'public-candy-display-name-authority-2026-09-01-b':'public-candy-display-name-authority-2026-08-31-a',
...
if(p0b6Successor)assert.equal(PUBLIC_CANDY_DISPLAY_NAME_AUTHORITY_POLICY.ingame_screenshot_official_equivalent_exact_string_supported,true,'P0-B6 must admit only the already-governed .54 first-party in-game exact-string evidence');

const rows=currentPublicCandyDisplayNameAuthorityRows();
const predecessorNames=['伊布的糖果','妙蛙種子的糖果','皮卡丘的糖果'];
const p0b6IngameNames=['草苗龜的糖果','木守宮的糖果','小鍛匠的糖果','波加曼的糖果','水躍魚的糖果','摔角鷹人的糖果','火稚雞的糖果','菊草葉的糖果'];
const expectedNames=p0b6Successor?[...predecessorNames,...p0b6IngameNames]:predecessorNames;
...
if(p0b6Successor){
  ...
  for(const displayName of p0b6IngameNames){

...
assert.equal(potValid.ok,true);
assert.equal(potValid.observation.total_capacity,57);
assert.equal(potValid.count,2);
assert.equal(potValid.warnings.length,1);
...
assert.equal(potOperation.operation.entity,'account_capacity');
assert.deepEqual(potOperation.operation.key,{capacity_key:'pot'});
assert.deepEqual(potOperation.operation.data,{total_capacity:57});
assert.equal(recipeScenarioAcceptsPotCapacity(),true,'recipe UC.IMG scenario must permit account_capacity in the same formal Apply path');
...
assert.equal(RAW_PUBLIC_RECIPE_MASTER.length,76,'historical raw base stays 76; do not rewrite stable baseline');
assert.equal(PUBLIC_RECIPE_MASTER.length,78,'current canonical registered recipe master must be 78');
...
const rawIds=new Set(RAW_PUBLIC_RECIPE_MASTER.map(row=>row.recipe_id));
for(const row of RAW_PUBLIC_RECIPE_MASTER)assert.ok(PUBLIC_RECIPE_MASTER.some(current=>current.recipe_id===row.recipe_id),`existing stable recipe id disappeared: ${row.recipe_id}`);
...
assert.equal(coverage.upcoming_evidence_count,0);
assert.equal(coverage.promoted_historical_evidence_count,2);
assert.equal(PUBLIC_RECIPE_DISCOVERY.length,2);
assert.ok(PUBLIC_RECIPE_DISCOVERY.every(row=>row.active_canonical===true&&row.lifecycle==='PROMOTED_TO_CANONICAL_ACTIVE'));
const discoveryPlan=projectRecipeDiscoveryStockpile({inventory:[],scoringProjection:{candidates:[]},weeklyContext:{}});
assert.equal(discoveryPlan.summary.recipe_candidate_count,0,'promoted recipes must no longer appear as unknown Discovery Candidate A/B');
...
for(const row of PUBLIC_RECIPE_ACTIVATION_ADDITIONS)assert.ok(snapshot.rows.some(item=>item.recipe_id===row.recipe_id&&item.recipe_name===row.recipe_name));
const schema=buildPublicMasterRecognitionJsonSchema('recipes');
assert.ok(schema.properties.capacity_observations,'recipe Recognition schema must expose optional pot capacity observations');
...
const recognition={
  ...
  generated_at:'2026-08-12T02:00:00Z',visible_target_count:1,
  observations:[{observation_id:'obs-1',status:'MATCHED',observed_text:'萌綠咖哩麵包',observed_data:{unlocked:true,recipe_level:1},canonical_key:{recipe_id:newRecipe.recipe_id,recipe_name:newRecipe.recipe_name},canonical_name:newRecipe.recipe_name,source_image_ref:'image-001',confidence:0.99}],
  capacity_observations:[{capacity_key:'pot',total_capacity:57,source_image_ref:'image-001',confidence:0.99,observation_context:'RECIPE_SCREEN_BASE_POT_CAPACITY'}],
};
const compiled=compilePublicMasterRecognitionToUpdatePackage(recognition,'recipes',{allowedImageRefs:['image-001']});
assert.equal(compiled.ok,true);
assert.equal(compiled.errors.length,0);
assert.equal(compiled.summary.matched_count,1);
assert.equal(compiled.summary.pot_capacity_observed,57);
assert.equal(compiled.update_package.operations.length,2);
assert.ok(compiled.update_package.operations.some(op=>op.entity==='recipes'&&op.key.recipe_id==='curry_greengrass_bun'));
assert.ok(compiled.update_package.operations.some(op=>op.entity==='account_capacity'&&op.key.capacity_key==='pot'&&op.data.total_capacity===57));

const unified=read('assets/js/unified-screenshot-update-center.js');
assert.equal((unified.match(/applyPayload\(/g)||[]).length,1,'UC.IMG must retain exactly one Apply bridge');
const migrations=read('assets/js/migrations.js');
assert.equal(migrations.includes('VALUES(10,'),false,'v0.4.13.2 must remain SQLite-migration-free');
for(const privateToken of ['1000109624.png','1000109625.png','1000109626.png','1000109627.png','1000109628.png','1000109629.png']){
  for(const path of ['assets/js/public-recipe-canonical-authority.js','assets/js/public-recipe-provenance.js','assets/js/public-recipe-discovery-master.js'])assert.equal(read(path).includes(privateToken),false,`private screenshot filename leaked into ${path}`);
}

console.log(JSON.stringify({
  status:'PASS',gate:'V0.4.13.2_BEHAVIORAL_CONTRACT',central_release_unchanged:'v0.4.13.1',
  pot_capacity_authority:POT_CAPACITY_AUTHORITY_VERSION,account_capacity_primary:true,legacy_weekly_fallback:true,weekly_prompt_owns_base_pot:false,
  recipe_screen_capacity_compiles_same_update_package:true,conflicting_capacity_fail_closed:true,uc_img_apply_bridge_count:1,
  raw_recipe_count:RAW_PUBLIC_RECIPE_MASTER.length,canonical_recipe_count:PUBLIC_RECIPE_MASTER.length,category_counts:categoryCounts,
  activated_recipe_ids:PUBLIC_RECIPE_ACTIVATION_ADDITIONS.map(row=>row.recipe_id),discovery_candidates_remaining:discoveryPlan.summary.recipe_candidate_count,
  recognition_catalog_count:snapshot.row_count,sqlite_migration_added:false,private_screenshot_names_committed:false,
},null,2));

...
const snapshot=buildPublicMasterCatalogSnapshot('recipes');assert.equal(snapshot.row_count,78);assert.equal(snapshot.data_version,PUBLIC_RECIPE_MASTER_VERSION);for(const row of PUBLIC_RECIPE_ACTIVATION_ADDITIONS)assert.ok(snapshot.rows.some(item=>item.recipe_id===row.recipe_id&&item.recipe_name===row.recipe_name));

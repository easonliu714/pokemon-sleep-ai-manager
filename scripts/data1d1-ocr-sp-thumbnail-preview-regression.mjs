...
const paths={
  ...
  consent:'assets/js/data1d1-ocr-region-ai-consent.js',
...
for(const path of Object.values(paths)){
  ...
  assert.equal(checked.status,0,`syntax:${path}:${checked.stderr}`);
}
const source=Object.fromEntries(Object.entries(paths).map(([key,path])=>[key,fs.readFileSync(path,'utf8')]));

assert.match(CLASSIFIER_SCHEMA,/^pokemon-sleep-ocr-first-classifier\/1\.[5-9]\d*$/);
...
assert.ok(joint.classification_confidence>levelOnly.classification_confidence);
assert.equal(tokenMatches('display response screenshot inspection','sp'),false);
assert.equal(tokenMatches('pokemon sp 1789','sp'),true);
...
assert.equal(AI_CONSENT_SCHEMA,'pokemon-sleep-ai-consent-queue/1.2');
...
for(const token of ['ocr-ai-candidate','PREVIEW_ROW_SELECTOR',"archive.readImage(path,{type:'blob'})",'pokemon-sleep:ocr-overlay-preview-requested']){
  assert.match(source.wiring,new RegExp(token.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));
}
for(const token of [

...
const joint=classifyOcrText('Lv. 31 SP 1789'),levelOnly=classifyOcrText('Lv. 31'),spOnly=classifyOcrText('SP 1789'),strong=classifyOcrText('主技能 Lv. 31');
assert.equal(joint.suggested_category,'pokemon');assert.equal(levelOnly.suggested_category,'other');assert.equal(spOnly.suggested_category,'other');assert.equal(strong.suggested_category,'pokemon');assert.ok(joint.classification_confidence>levelOnly.classification_confidence);assert.equal(tokenMatches('display response screenshot inspection','sp'),false);assert.equal(tokenMatches('pokemon sp 1789','sp'),true);
const duplicate={sha256:'dup-sha',path:'duplicate.png',status:'duplicate',classification_status:'skipped'},review={sha256:'review-sha',path:'review.png',requires_review:true,classification_status:'suggested'};
assert.equal(buildAiConsentQueue([duplicate],{selectedIds:[]}).selected_count,0);const manual=buildAiConsentQueue([duplicate],{selectedIds:['dup-sha']});assert.equal(AI_CONSENT_SCHEMA,'pokemon-sleep-ai-consent-queue/1.2');assert.equal(manual.selected_count,1);assert.equal(manual.manual_duplicate_count,1);assert.equal(manual.items[0].manual_duplicate_override,true);assert.equal(buildAiConsentQueue([duplicate,review],{selectedIds:['dup-sha','review-sha']}).selected_count,2);
...
assert.match(authority.app_version,/^v\d+\.\d+\.\d+(?:\.\d+)*$/);assert.match(authority.app_build,/^\d{8}-[a-z0-9-]+$/);assert.equal(authority.cache_name,`pokemon-sleep-ai-${authority.app_version}-${authority.app_build.replace(/^\d{8}-/,'')}`);
assert.match(source.bootstrap,/authority\.app_version/);assert.match(source.worker,/cache_name:CACHE/);
console.log(JSON.stringify({ok:true,gate:'SP thumbnail preview compatibility contract',classifier_schema:CLASSIFIER_SCHEMA,app_version:authority.app_version,build:authority.app_build,incremental_dom:true,delegated_candidate_listener:true,android_raf_timeout_fallback:true,central_version_authority:true,hotfix_semver_supported:true}));

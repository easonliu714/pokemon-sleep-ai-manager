...
import {
  ...
  INGREDIENT_EVENT_RECONSTRUCTION_STATUS,
  resolveIngredientHelpEventCountFromAggregateItems,
...
const base={
  ...
  observation_mode:FIRST_PARTY_OBSERVATION_MODES.MULTI_SLOT_UNIQUE_EVENT_COUNT,
...
const unique=resolveIngredientHelpEventCountFromAggregateItems({ingredient_items_collected:4,slot_quantities:[2,3]});
assert.equal(unique.status,INGREDIENT_EVENT_RECONSTRUCTION_STATUS.UNIQUE_EVENT_COUNT);
assert.equal(unique.event_count,2);
assert.deepEqual(unique.possible_event_counts,[2]);

const ambiguous=resolveIngredientHelpEventCountFromAggregateItems({ingredient_items_collected:6,slot_quantities:[2,3]});
assert.equal(ambiguous.status,INGREDIENT_EVENT_RECONSTRUCTION_STATUS.AMBIGUOUS_EVENT_COUNT);
assert.equal(ambiguous.event_count,null);
assert.deepEqual(ambiguous.possible_event_counts,[2,3]);

const equalQuantity=resolveIngredientHelpEventCountFromAggregateItems({ingredient_items_collected:6,slot_quantities:[2,2]});
assert.equal(equalQuantity.status,INGREDIENT_EVENT_RECONSTRUCTION_STATUS.UNIQUE_EVENT_COUNT);
assert.equal(equalQuantity.event_count,3,'duplicate slot quantities are distinct slots but imply the same event count');

const unreachable=resolveIngredientHelpEventCountFromAggregateItems({ingredient_items_collected:1,slot_quantities:[2,3]});
assert.equal(unreachable.status,INGREDIENT_EVENT_RECONSTRUCTION_STATUS.NO_INTEGER_COMBINATION);
assert.equal(unreachable.event_count,null);
...
assert.equal(accepted.ingredient_help_event_count,2);
assert.equal(accepted.total_help_event_count,22);
assert.equal(accepted.ingredient_event_fraction,2/22);
...
assert.equal(accepted.safety.multi_slot_unique_integer_reconstruction,true);
assert.equal(accepted.safety.multi_slot_reconstruction_status,INGREDIENT_EVENT_RECONSTRUCTION_STATUS.UNIQUE_EVENT_COUNT);
...
const ambiguousObservation=evaluateFirstPartyIngredientHelpObservation({...base,observation_id:'fixture-ambiguous',ingredient_items_collected:6,inventory_items_before_collection:26});
assert.equal(ambiguousObservation.status,FIRST_PARTY_OBSERVATION_STATUS.REVIEW_REQUIRED);
assert.ok(ambiguousObservation.blockers.includes(FIRST_PARTY_OBSERVATION_BLOCKERS.MULTI_SLOT_INGREDIENT_EVENT_COUNT_AMBIGUOUS));
assert.equal(ambiguousObservation.ingredient_help_event_count,null);
assert.equal(ambiguousObservation.eligible_for_statistical_aggregation,false);

const unreachableObservation=evaluateFirstPartyIngredientHelpObservation({...base,observation_id:'fixture-unreachable',ingredient_items_collected:1,inventory_items_before_collection:21});
assert.equal(unreachableObservation.status,FIRST_PARTY_OBSERVATION_STATUS.REVIEW_REQUIRED);
assert.ok(unreachableObservation.blockers.includes(FIRST_PARTY_OBSERVATION_BLOCKERS.INGREDIENT_EVENT_COUNT_NOT_RECONSTRUCTABLE));

const wrongLevel=evaluateFirstPartyIngredientHelpObservation({...base,observation_id:'fixture-multi-mode-level20',level:20,ingredient_slots:[{unlock_level:1,ingredient_name:'HONEY',quantity:2}],ingredient_items_collected:4,inventory_items_before_collection:24});
...
assert.equal(ui.includes(FIRST_PARTY_OBSERVATION_MODES.MULTI_SLOT_UNIQUE_EVENT_COUNT),false,'multi-slot mode must not be exposed before the UI successor gate');

console.log(JSON.stringify({
  status:'PASS',
  gate:'V0428_G75E3C6C_MULTI_SLOT_UNIQUE_EVENT_RECONSTRUCTION',
  method:'AGGREGATE_ITEM_TOTAL_PLUS_OBSERVED_SLOT_QUANTITIES_UNIQUE_INTEGER_EVENT_COUNT',
  level30_unique_fixture:{slot_quantities:[2,3],ingredient_items:4,ingredient_help_events:accepted.ingredient_help_event_count},
  level30_ambiguous_fixture:{slot_quantities:[2,3],ingredient_items:6,status:ambiguousObservation.status},
  level60_unique_fixture:true,

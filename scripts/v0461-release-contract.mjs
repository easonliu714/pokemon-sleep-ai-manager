...
for(const file of ['assets/js/recipe-strategy-local.js','assets/js/pokemon-candidate-local.js','assets/js/recipe-discovery-stockpile-local.js','assets/js/strategy-context-local.js','assets/js/evaluation-lifecycle.js','assets/js/current-week-recipe-recommendation-bridge.js','assets/js/shared-knowledge-ui.js']){
  ...
  if(atLeast(app,'v0.4.27.5')&&effectiveWeeklySuccessorFiles.has(file)){
    assert.ok(source.includes('currentEffectiveWeeklyContext'),`${file} bypasses Effective Weekly Context authority`);
    assert.ok(source.includes("from './effective-weekly-context.js'"),`${file} must import Effective Weekly Context authority`);
  }else assert.ok(source.includes('currentWeeklyContext'),`${file} bypasses resolved Weekly Context authority`);
  assert.equal(source.includes("SELECT * FROM weekly_context ORDER BY updated_at DESC LIMIT 1"),false);
}

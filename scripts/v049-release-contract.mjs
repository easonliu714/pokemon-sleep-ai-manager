...
assert.ok(version.includes("// app_version: 'v0.4.8.5'"),'v0.4.9 must retain v0.4.8.5 legacy parser bridge');
...
for(const token of ['產生可信分析包','複製 AI 提示詞','下載 JSON','下載 Markdown','navigator.share','資料或分析要求已變更，請重新產生','source image ref','identity fingerprint'])assert.ok(ui.includes(token),`v0.4.9 UI missing ${token}`);
...
console.log(JSON.stringify({
  ...
  local_version:STRATEGY_ANALYSIS_LOCAL_VERSION,ui_version:STRATEGY_ANALYSIS_PACK_UI_VERSION,
  provider_neutral:true,offline_generation:true,ephemeral_candidate_refs:true,stable_pokemon_id_export:false,structural_privacy_guard:true,

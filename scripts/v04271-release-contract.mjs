...
assert.equal(authority.app_version,'v0.4.27.1');
...
assert.ok(versionSource.includes("// app_build: '20260814-v0427-g75e3b-ingredient-slot-distribution'"));

assert.equal(INGREDIENT_INVENTORY_INTEGRITY_VERSION,'ingredient-inventory-integrity-2026-08-16-a');
assert.equal(INGREDIENT_INVENTORY_INTEGRITY_MIGRATION_VERSION,12);
assert.equal(LEGACY_INGREDIENT_IDENTITY_RENAMES['特選酪梨'],'嫩亮酪梨');

const established=[{ingredient_name:'好眠番茄',quantity:21},{ingredient_name:'窩心洋芋',quantity:7}];
...
const ui=read('assets/js/ingredient-inventory-integrity-ui.js');assert.ok(ui.includes('完整庫存中未找到的已建立食材'));assert.ok(ui.includes('CONFIRMED_EXHAUSTED'));assert.ok(ui.includes('PRESERVE_EXISTING_NOT_CAPTURED'));assert.ok(ui.includes('stopImmediatePropagation'));
...
console.log(JSON.stringify({status:'PASS',gate:'V0.4.27.1_RELEASE_CONTRACT',app_version:authority.app_version,migration_version:INGREDIENT_INVENTORY_INTEGRITY_MIGRATION_VERSION,legacy_avocado_rekey:true,legacy_canonical_no_sum:true,complete_capture_absence_requires_confirmation:true,explicit_zero_only_after_user_confirmation:true,null_missing_preserve_existing:true,offline_precache:true,android_pwa_live_validation_required:true,workflow_topology_changed:false},null,2));
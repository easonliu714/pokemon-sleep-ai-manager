...
for(const row of PUBLIC_RECIPE_DISCOVERY){
  ...
  assert.ok(Array.isArray(row.reference_ingredient_set)&&row.reference_ingredient_set.length===4);
  assert.ok(Array.isArray(row.observed_quantity_signature)&&row.observed_quantity_signature.length===4);
  assert.equal(row.max_observed_quantity,Math.max(...row.observed_quantity_signature));
  assert.equal('planning_formula' in row,false,'Discovery evidence must not bind quantity to an ingredient before canonical verification');
}
...
assert.equal(result.summary.total_deficit,236);
...
assert.equal(result.discovery_candidates[0].quantity_assignment_status,'UNKNOWN_UNORDERED_SIGNATURE');
assert.equal(result.discovery_candidates[0].conservative_upper_bound_per_reference_ingredient,20);
assert.equal(result.discovery_candidates[1].conservative_upper_bound_per_reference_ingredient,39);
const target=Object.fromEntries(result.stockpile.map(row=>[row.ingredient_name,row.target]));
assert.deepEqual(target,{'暖暖薑':59,'火辣香草':59,'品鮮蘑菇':39,'豆製肉':39,'純粹油':20,'萌綠大豆':20});
...
for(const token of ['weekly_context_update','weekly_context','recipe_final_energy_multiplier','玩家當週狀態，不是公版 Master','固定三樹果營地','萌綠之島一般模式與 EX 動態營地'])assert.ok(prompt.includes(token),`weekly player JSON prompt missing: ${token}`);
const local=fs.readFileSync('assets/js/recipe-discovery-stockpile-local.js','utf8');
for(const token of ['currentWeeklyContext','ingredient_inventory','buildLocalPokemonCandidateScoring'])assert.ok(local.includes(token),`local planner input missing: ${token}`);
...
console.log(JSON.stringify({
  status:'PASS',gate:'WAR.2B_RECIPE_DISCOVERY_STOCKPILE',discovery_candidates:2,canonical_active_discovery_rows:0,total_stockpile_target:236,
  ...
  quantity_assignment:'UNKNOWN_UNORDERED_SIGNATURE',target_semantics:result.summary.target_semantics,

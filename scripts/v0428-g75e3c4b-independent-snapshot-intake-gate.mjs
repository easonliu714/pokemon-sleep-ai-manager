...
import {PUBLIC_SPECIES_FORM_ROSTER_ROWS,currentPublicSpeciesFormRoster} from '../assets/js/public-pokemon-species-form-roster.js';
...
const rosterKeys=PUBLIC_SPECIES_FORM_ROSTER_ROWS.map(row=>row.source_key);
...
assert.equal(new Set(rosterKeys).size,242);
...
assert.equal(partial.row_count,3);assert.equal(partial.valid_mapped_row_count,3);assert.equal(partial.unique_mapped_row_count,3);assert.equal(partial.coverage_ratio,3/242);assert.equal(partial.complete_coverage,false);assert.equal(partial.activation_authority_granted,false);assert.deepEqual(partial.problems,[]);
assert.equal(partial.normalized_rows[0].ingredient_percentage_published,'25.700');assert.equal(partial.normalized_rows[0].ingredient_percentage_normalized,'25.7');assert.equal(partial.normalized_rows[2].ingredient_percentage_normalized,'0.5');
...
assert.equal(complete.status,INDEPENDENT_SNAPSHOT_INTAKE_STATUS.ADMITTED_COMPLETE_CROSSCHECK_SNAPSHOT);assert.equal(complete.row_count,242);assert.equal(complete.unique_mapped_row_count,242);assert.equal(complete.coverage_ratio,1);assert.equal(complete.complete_coverage,true);assert.equal(complete.activation_authority_granted,false);
...
assert.equal(unreviewed.status,INDEPENDENT_SNAPSHOT_INTAKE_STATUS.SOURCE_ADMISSION_NOT_READY);assert.equal(unreviewed.reason,'HUMAN_REVIEWED_LINEAGE_INDEPENDENCE_MISSING');
const wrongSchema=validateIndependentIngredientProbabilitySnapshot({snapshot:{...partialSnapshot,schema:'other/1.0'},rosterKeys});assert.equal(wrongSchema.status,INDEPENDENT_SNAPSHOT_INTAKE_STATUS.SNAPSHOT_REVIEW_REQUIRED);assert.equal(wrongSchema.reason,'SNAPSHOT_SCHEMA_NOT_ACCEPTED');
...
const unknown=validateIndependentIngredientProbabilitySnapshot({snapshot:{schema:INDEPENDENT_INGREDIENT_PROBABILITY_SNAPSHOT_SCHEMA,source:admittedSource(1),rows:[row('NOT_IN_ROSTER')]},rosterKeys});assert.equal(unknown.status,INDEPENDENT_SNAPSHOT_INTAKE_STATUS.SNAPSHOT_REVIEW_REQUIRED);assert.ok(unknown.problems.includes('UNKNOWN_ROSTER_KEYS'));
const invalidValue=validateIndependentIngredientProbabilitySnapshot({snapshot:{schema:INDEPENDENT_INGREDIENT_PROBABILITY_SNAPSHOT_SCHEMA,source:admittedSource(1),rows:[row(rosterKeys[0],'25.7%')]},rosterKeys});assert.equal(invalidValue.status,INDEPENDENT_SNAPSHOT_INTAKE_STATUS.SNAPSHOT_REVIEW_REQUIRED);assert.ok(invalidValue.problems.some(value=>value.includes('PUBLISHED_NUMERIC_VALUE_INVALID')));
const missingRef=validateIndependentIngredientProbabilitySnapshot({snapshot:{schema:INDEPENDENT_INGREDIENT_PROBABILITY_SNAPSHOT_SCHEMA,source:admittedSource(1),rows:[{...row(rosterKeys[0]),source_row_ref:''}]},rosterKeys});assert.equal(missingRef.status,INDEPENDENT_SNAPSHOT_INTAKE_STATUS.SNAPSHOT_REVIEW_REQUIRED);assert.ok(missingRef.problems.some(value=>value.includes('SOURCE_ROW_REF_MISSING')));
const wrongMappedCount=validateIndependentIngredientProbabilitySnapshot({snapshot:{...partialSnapshot,source:admittedSource(4)},rosterKeys});assert.equal(wrongMappedCount.status,INDEPENDENT_SNAPSHOT_INTAKE_STATUS.SNAPSHOT_REVIEW_REQUIRED);assert.ok(wrongMappedCount.problems.includes('ADMISSION_MAPPED_ROW_COUNT_MISMATCH'));
const wrongRosterCount=validateIndependentIngredientProbabilitySnapshot({snapshot:{...partialSnapshot,source:{...admittedSource(3),roster_row_count:241}},rosterKeys});assert.equal(wrongRosterCount.status,INDEPENDENT_SNAPSHOT_INTAKE_STATUS.SNAPSHOT_REVIEW_REQUIRED);assert.ok(wrongRosterCount.problems.includes('ADMISSION_ROSTER_ROW_COUNT_MISMATCH'));

const registry=currentProductionAuthorityRegistry();assert.deepEqual(registry.active_verified_dimensions,['berry_output_per_help','berry_energy_per_berry','favorite_berry_multiplier','ingredient_slot_distribution']);assert.equal(registry.rules.ingredient_probability_per_help.status,'NOT_YET_VERIFIED');assert.equal(registry.rules.ingredient_probability_per_help.runtime_numeric_activation,false);assert.equal(registry.numeric_rate_model_status,'NOT_YET_VERIFIED');
const source=read('assets/js/ingredient-probability-independent-snapshot-contract.js');for(const forbidden of ['fetch(', 'raw.githubusercontent.com', 'localStorage', 'sessionStorage', 'indexedDB', 'INSERT INTO', 'UPDATE ingredient_inventory', 'DELETE FROM', 'applyPayload(', 'dryRun('])assert.equal(source.includes(forbidden),false,`snapshot intake contract owns forbidden path ${forbidden}`);
